...
- For required demonstrations, select the real capture source, caption renderer,
  frame sampling, artifact retention, and OpenCode response-media embed path. The
  plan must state that proof videos do not run proof-specific PII/sensitive-data
  detection or replacement, that the full video never enters model context, and
  that exact captions are written only after normal green gates.
