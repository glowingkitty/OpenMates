...
def disable_bot(bot_name=None,bot_id=None):
    try:
        ...
        url = f"{mattermost_domain}/api/v4/bots/{bot_id}/disable"
        headers = {
        ...
        if response.status_code != 200:
            raise Exception(f"Failed to disable bot {bot_name}: {response.text}")
