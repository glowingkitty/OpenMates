...
export const GET: RequestHandler = async ({ params, url, fetch }) => {
	...
	try {
		...
		const response = await fetch(
			`${backendUrl}/v1/share/embed/${embedId}/og-metadata?t=${cacheBuster}`,
			{
		...
		if (response.ok) {
			...
			console.warn(`[OG Tags] Fetched metadata for embed ${embedId}:`, {
				title: ogTitle.substring(0, 50),
				description: ogDescription.substring(0, 50),
				image: ogImage,
				isFallback:
					ogTitle === 'Shared Embed - OpenMates' &&
					ogDescription === 'View this shared content on OpenMates'
			});
		} else {
