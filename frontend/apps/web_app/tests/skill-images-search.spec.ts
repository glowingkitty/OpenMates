 ...
 *
 * Phase 1: Embed preview renders at /dev/preview/embeds/images
 * Phase 2: CLI direct skill command (openmates apps images search --json)
...
test.describe('App: Images / Skill: search', () => {
	...
	test('Phase 1: embed preview renders at /dev/preview/embeds/images', async ({ page }) => {
		const log = (msg: string) => console.log(`[P1] ${msg}`);
		await verifyEmbedPreviewPage(page, 'images', log);
	});
	...
	test('Phase 4: Web chat triggers images search with embed', async ({ page }: { page: any }) => {
		test.slow();
		test.setTimeout(300_000);
		test.skip(!getTestAccount().email, 'Test account credentials required.');

		const { logCheckpoint } = createSignupLogger('skill-images-search');
		await archiveExistingScreenshots('skill-images-search');
		const takeStepScreenshot = createStepScreenshotter('skill-images-search');
		...
		logCheckpoint(`Found ${await resultCards.count()} image result(s).`);

		await closeFullscreen(page, fullscreenOverlay);
		await deleteActiveChat(page, logCheckpoint, takeStepScreenshot, 'images-search');
	});
});
