...
test.describe('Proof video real CLI architecture', () => {
	...
	test('records the real OpenMates CLI app skill examples in a graphical terminal', async ({}, testInfo: any) => {
		...
		const outputDir = path.resolve(process.cwd(), 'test-results/cli-proof/travel-search-examples');
		...
		const result = await run('python3', [
			...
			'--display-number', '92',
			'--', 'node', cliPath, 'apps', 'examples', 'travel', 'search_connections'
		], repositoryRoot);
		...
		expect(fs.readFileSync(transcriptPath, 'utf8')).toContain('Example chats for travel/search_connections');
