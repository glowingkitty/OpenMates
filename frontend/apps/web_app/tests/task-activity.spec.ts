...
const TASK_ACTIVITY_PROOF = defineVideoProof({
	...
	transcript: [
		...
		{ id: 'attribution-tombstones', text: 'Web, CLI, SDK, and Codex entries have distinct attribution, while deleted comments retain only author and deleter identities.', checkpoint: 'attribution-tombstones', devices: ['web-laptop', 'web-phone'] }
	],
	assertions: [
...
test.describe('Task Activity component', () => {
    ...
    await recordReadableInterval(page, 'rich-composer');
		...
		await proof.action('demonstrate-attribution-and-tombstones', async () => {
			...
			await expect(page.getByTestId('task-activity-entry-mate')).toContainText('Codex');
			...
			await expect(page.getByTestId('task-activity-entry-deleted-mate')).toContainText('Comment by Codex deleted by Alice Weber');
