...
test('recording overlay keeps only its latest wrapped transcript line visible on mobile', async ({ page }) => {
  await page.goto(
  ...
  await expect.poll(async () => Number(await transcript.getAttribute('data-line-count'))).toBeGreaterThan(1);
