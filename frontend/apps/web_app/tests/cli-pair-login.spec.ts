...
/**
 * Login to the test account via the web app UI.
 * Reused from chat-search-flow.spec.ts pattern.
 */
	...
	for (let attempt = 1; attempt <= 3 && !loginSuccess; attempt++) {
		...
		try {
			...
			if (hasError && attempt < 3) {
				logCheckpoint(`OTP attempt ${attempt} failed, waiting for new TOTP window...`);
