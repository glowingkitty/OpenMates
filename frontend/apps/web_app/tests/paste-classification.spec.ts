...
test('short plain multi-line prose stays editable text', async ({ page }: { page: any }) => {
...
test('formatted pasted text becomes a docs embed with Paste as text recovery', async ({ page }: { page: any }) => {
	test.setTimeout(60000);
	const editor = await openUnauthenticatedNewChat(page);
