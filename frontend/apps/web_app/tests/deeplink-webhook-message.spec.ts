...
/**
 * Deeplink Webhook Message — End-to-End
 *
 * Validates that a webhook-triggered system message renders correctly
 * when the user navigates to the chat via a deep link (page.goto):
 *   1. Login
 *   2. Create a webhook key in Settings › Developers › Webhooks
 *   3. POST a GitHub-shaped payload to /v1/webhooks/incoming
 *   4. Wait for the webhook_chat WS event to be received and processed
 *   5. Navigate via page.goto(`/#chat-id=<expectedChatId>`)
 *   6. Assert the system message is visible and rendered correctly
 *   7. Assert the AI assistant response arrives
 *
 * REQUIRED ENV VARS:
...
test('deeplink-webhook-message — deep link to webhook chat shows system message + AI response', async ({
	page,
	request
}: {
	page: any;
	...
	const log = createSignupLogger('DEEPLINK_WEBHOOK_MESSAGE');
	...
	const keyName = `E2E-Deeplink-${Date.now()}`;
	const nameInput = page.getByTestId('webhook-name-input');
	await expect(nameInput).toBeVisible({ timeout: 5000 });
	await nameInput.fill(keyName);
	log(`Entered webhook name: "${keyName}"`);

	// Fill in a Jinja2 template that extracts fields via dotted paths
	const messageTemplate =
		'E2E deeplink webhook test — please reply with "acknowledged".\n\n' +
	...
	const uniqueMarker = `e2e-deeplink-${Date.now()}`;
	const githubPayload = {
		action: 'opened',
		pull_request: {
			title: `E2E deeplink PR ${uniqueMarker}`,
			user: { login: 'e2e-tester' },
			html_url: `https://github.com/openmates/webhooks/pull/${Date.now()}`
		},
		repository: { full_name: 'openmates/webhooks-e2e-deeplink' }
	};
	...
	// ── Phase 5: Assert the system message is visible ────────────────────────
	...
	expect(sysText || '').toContain('openmates/webhooks-e2e-deeplink');
	expect(sysText || '').toContain(`E2E deeplink PR ${uniqueMarker}`);
	expect(sysText || '').toContain('e2e-tester');
	log('System message verified — template substitutions rendered correctly.');

	// ── Phase 6: Assert the AI assistant response streams in ─────────────────
