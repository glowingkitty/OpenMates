...
async function waitForResponseAndVerifyModel(
	...
	// Wait for the message to contain actual content (not just a streaming placeholder).
	// Some models (e.g. Kimi K2.5 via Together AI) can be slow to start streaming.
	// Poll the message text until it has substantive content (>20 chars), which confirms
	// the model has started generating a real response.
	await expect(async () => {
		const msgText = await assistantMessage.textContent();
		const textLen = (msgText || '').trim().length;
		logCheckpoint(`Waiting for response content... length=${textLen}`);
		expect(textLen).toBeGreaterThan(20);
	}).toPass({ timeout: 90000, intervals: [2000, 3000, 5000] });
	logCheckpoint('Response has substantive content.');
