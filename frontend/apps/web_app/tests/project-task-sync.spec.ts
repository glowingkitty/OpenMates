...
function events(child) {
  ...
  child.stdout.on('data', data => {
    ...
    while (buffer.includes('\n')) {
      ...
      try { const event = JSON.parse(line); received.push(event); pending.get(event.event)?.resolve(event); } catch { /* Only complete JSON protocol events are actionable. */ }
    }
  });
  child.once('exit', code => { for (const wait of pending.values()) wait.reject(new Error(`Task sync fixture exited (${code})`)); });
  return name => {
    const previous = received.find(event => event.event === name);
    if (previous) return Promise.resolve(previous);
    return new Promise((resolve, reject) => {
      const timer = setTimeout(() => { pending.delete(name); reject(new Error(`Task sync fixture timed out waiting for ${name}`)); }, 60_000);
...
test('web edits reach every cached Task and disconnected updates replay', async ({ page }) => {
  ...
  try {
    ...
    child = spawn('node', ['--experimental-strip-types', '--loader', './frontend/packages/openmates-cli/tests/loader.mjs', 'scripts/project_task_sync_live.mjs'], {
      cwd: ROOT, env: workflowCliEnv('http://localhost:8000', home), stdio: ['pipe', 'pipe', 'pipe'],
    });
    ...
    expect((await verified).checks).toEqual(['twelve_tasks', 'reconnect_update', 'reconnect_deletion']);
