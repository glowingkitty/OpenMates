...
test('native mindmap upload renders preview and fullscreen controls', async ({ page }: { page: any }) => {
	...
	await expect(chatMindMapPreview).toContainText('Launch Plan');
	await expect(chatMindMapPreview.getByTestId('embed-app-icon-mask')).toHaveCSS('background-color', 'rgb(255, 255, 255)');
