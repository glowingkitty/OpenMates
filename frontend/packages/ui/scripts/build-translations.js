...
function setNestedValue(obj, path, value) {
  ...
  // Set the final value
  // If the target already exists as an object and the new value is also an object,
  // merge them instead of overwriting. This handles the case where a shallow key
  // (e.g. "software_development" → { text: "Sophia" }) is set after deeper keys
  // (e.g. "software_development.systemprompt" → { text: "..." }) have already
  // created a nested structure under the same key.
...
function loadYamlFilesRecursive(dir, namespacePrefix = "") {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      const subNamespace = namespacePrefix
