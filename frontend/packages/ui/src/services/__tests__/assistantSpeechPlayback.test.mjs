...
// contract-test: supporting surface=gui.web assertions=assistant-speech.on-demand.generate-missing-only
test('metadata-only ready updates preserve already hydrated audio', async () => {
  const { queue } = harness();
  queue.start('response', [segment(0)]);
  await Promise.resolve();
  queue.upsertSegment({ ...segment(0), audioUrl: undefined });
  await queue.play();
  assert.equal(queue.state.status, 'playing');
  queue.stop();
});

// contract-test: supporting surface=gui.web assertions=assistant-speech.failure.nonblocking-visible-resumable
test('selecting a failed chapter shows failure rather than perpetual loading', async () => {
  const { queue } = harness();
  queue.start('response', [segment(0), segment(1, 'failed')]);
...
// contract-test: supporting surface=gui.web assertions=assistant-speech.playback.single-queue-segment-control
test('resuming between segments waits for the next instead of replaying the last', async () => {
  const { queue, emit } = harness();
  queue.start('response', [segment(0), segment(1, 'generating')]);
  await Promise.resolve();
  emit('ended');
  queue.pause();
  await queue.play();
  assert.equal(queue.state.activeSegmentId, 'segment-1');
  assert.equal(queue.state.status, 'waiting_for_segment');
  queue.stop();
});

// contract-test: supporting surface=gui.web assertions=assistant-speech.playback.single-queue-segment-control
test('paused navigation and delayed failure recovery preserve pause intent', async () => {
  const { queue } = harness();
  queue.start('response', [segment(0), segment(1, 'generating'), segment(2)]);
  await Promise.resolve();
  queue.pause();
  await queue.selectSegment('segment-1');
  await queue.selectSegment('segment-2');
  assert.equal(queue.state.status, 'paused');
  queue.upsertSegment(segment(2, 'failed'));
