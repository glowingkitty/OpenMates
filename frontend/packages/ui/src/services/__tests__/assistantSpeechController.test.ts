...
// contract-test: supporting surface=gui.web assertions=assistant-speech.playback.deterministic-chapter-labels
it("retains normalized manual chapters when later worker status uses an offset", async () => {
  const { assistantSpeechController: controller } = await import("../assistantSpeechController");
  await controller.request("chat", "message", "## First\nParagraph one.");
  await accepted([{ segment_id: "segment-0", sequence: 1, status: "queued" }]);
  mocks.handlers.get("assistant_speech_status")?.({ ...ready, sequence: 1 });
  await vi.waitFor(() => expect(get(controller.player).regions[0]?.status).toBe("ready"));
