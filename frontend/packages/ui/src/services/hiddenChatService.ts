...
class HiddenChatService {
  ...
  private async encryptChatKeyWithSecret(
    chatKey: Uint8Array,
    secret: Uint8Array,
  ): Promise<string> {
  ...
  private async getOrCreateSalt(): Promise<Uint8Array> {
    ...
    if (!emailSalt) {
      ...
      console.error(
        "[HiddenChatService] Email salt not found. User may need to log in.",
      );
      throw new Error(
        "Email salt required for hidden chat salt generation. Please log in.",
      );
    }
    ...
    console.debug(
      "[HiddenChatService] Using email salt for hidden chats (user-specific, cross-device compatible)",
    );
    return salt;
  }
  ...
  async deriveCombinedSecret(password: string): Promise<Uint8Array | null> {
    ...
    if (password.length < 4 || password.length > 30) {
      console.error(
        "[HiddenChatService] Invalid password format: length must be 4-30 characters",
      );
      return null;
    }
    ...
    try {
      ...
      const combinedInput = new Uint8Array(
        masterKeyArray.length + passwordBytes.length,
      );
      ...
      console.error(
        "[HiddenChatService] Error deriving combined secret:",
        error,
      );
  ...
  async unlockHiddenChats(
    ...
    try {
      ...
      if (verifiedEncryptedChatKey) {
        const verified = await this.decryptChatKeyWithCombinedSecret(
          verifiedEncryptedChatKey,
        );
        if (verified) {
      ...
      for (const chat of allChats) {
        if (chat.encrypted_chat_key) {
          ...
          const decrypted = await this.decryptChatKeyWithCombinedSecret(
            chat.encrypted_chat_key,
          );
          if (decrypted) {
      ...
      if (decryptedCount > 0) {
        ...
        console.debug(
          `[HiddenChatService] Hidden chats unlocked successfully: ${decryptedCount} chat(s) decrypted`,
        );
        ...
        console.debug(
          "[HiddenChatService] Hidden chats unlocked (no chats decrypted, but password is valid)",
        );
  ...
  async encryptChatKeyWithCode(
    ...
    try {
      ...
      console.error(
        "[HiddenChatService] Error encrypting chat key with code:",
        error,
      );
      return null;
    } finally {
  ...
  async lockHiddenChats(): Promise<void> {
    ...
    try {
      ...
      console.error(
        "[HiddenChatService] Error clearing chat keys from cache:",
        error,
      );
   ...
   */
  async encryptChatKeyWithCombinedSecret(
    chatKey: Uint8Array,
  ): Promise<string | null> {
    if (!this.combinedSecret) {
      console.error(
        "[HiddenChatService] Cannot encrypt: hidden chats are locked",
      );
      return null;
    }

    try {
      return await this.encryptChatKeyWithSecret(
        chatKey,
        new Uint8Array(this.combinedSecret),
      );
    } catch (error) {
   ...
   * @param chatId - Optional chat ID for debug logging
   * @returns Promise<Uint8Array | null> - Decrypted chat key or null if decryption fails
   */
  async decryptChatKeyWithCombinedSecret(
    ...
    try {
      ...
      console.error(
        `[HiddenChatService] Error decrypting chat key: chat_id=${chatIdStr} field=encrypted_chat_key`,
        error,
      );
      return null;
    }
   ...
   */
  async tryDecryptChatKey(encryptedChatKeyWithIV: string): Promise<{
    ...
    if (this.combinedSecret) {
      const hiddenKey = await this.decryptChatKeyWithCombinedSecret(
        encryptedChatKeyWithIV,
      );
      if (hiddenKey) {
  ...
  private recordFailedAttempt(): void {
    ...
    if (this.failedAttempts >= MAX_FAILED_ATTEMPTS) {
      ...
      console.warn(
        "[HiddenChatService] Rate limit triggered after",
        this.failedAttempts,
        "failed attempts",
      );
    }
  ...
  async unhideChat(chatId: string): Promise<boolean> {
    ...
    if (!this.combinedSecret) {
      console.error(
        "[HiddenChatService] Cannot unhide: hidden chats are locked",
      );
      return false;
    }
    ...
    try {
      ...
      if (!chat || !chat.encrypted_chat_key) {
        console.error(
          "[HiddenChatService] Chat not found or missing encrypted_chat_key:",
          chatId,
        );
        return false;
      }
      ...
      const decryptedChatKey = await this.decryptChatKeyWithCombinedSecret(
        chat.encrypted_chat_key,
        chatId,
      );
      if (!decryptedChatKey) {
        console.error(
          `[HiddenChatService] Failed to decrypt chat key with combined secret: chat_id=${chatId} field=encrypted_chat_key`,
        );
        return false;
      }
      ...
      if (!reEncryptedChatKey) {
        console.error(
          "[HiddenChatService] Failed to re-encrypt chat key with master key:",
          chatId,
        );
        return false;
      }
      ...
      try {
        ...
        await chatSyncService.sendUpdateEncryptedChatKey(
          chatId,
          reEncryptedChatKey,
        );
        console.debug(
          "[HiddenChatService] Synced unhidden chat to server:",
          chatId,
        );
      } catch (syncError) {
        console.warn(
          "[HiddenChatService] Failed to sync unhidden chat to server:",
          syncError,
        );
