...
export async function handlePhase2RecentChatsImpl(
    ...
    try {
        ...
        const { chats, chat_count, embeds, embed_keys } = payload as Phase2RecentChatsPayload & {
            embeds?: SyncEmbed[];
        ...
        serviceInstance.dispatchEvent(new CustomEvent('phase_2_last_20_chats_ready', {
            detail: { chat_count }
        }));
...
export async function handlePhase3FullSyncImpl(
    ...
    try {
        ...
        const { chats, chat_count, new_chat_suggestions, embeds, embed_keys } = payload as Phase3FullSyncPayload & {
            embeds?: SyncEmbed[];
            embed_keys?: EmbedKeyEntry[];
        };
        ...
        serviceInstance.dispatchEvent(new CustomEvent('phase_3_last_100_chats_ready', {
        ...
        serviceInstance.dispatchEvent(new CustomEvent('fullSyncReady', {
            detail: { chat_count, suggestions_count: new_chat_suggestions?.length || 0 }
        }));
...
export async function handleSyncStatusResponseImpl(
    ...
    if (is_primed && !serviceInstance.initialSyncAttempted_FOR_HANDLERS_ONLY && serviceInstance.webSocketConnected_FOR_SENDERS_ONLY) {
...
async function storeRecentChats(serviceInstance: ChatSynchronizationService, chats: any[]): Promise<void> {
    try {
        for (const chatItem of chats) {
            ...
            if (!existingChat && (!shouldSyncMessages || !messages || messages.length === 0)) {
            ...
            const preparedMessages = prepareMessagesForStorage(messages, chatId, 'Phase 2');
            ...
            try {
                ...
                if (shouldSyncMessages && preparedMessages.length > 0) {
                    ...
                    console.log(`[CLIENT_SYNC] ✅ Phase 2 - Successfully saved ${preparedMessages.length} messages for chat ${chatId}`);
                }
            } catch (saveError) {
        ...
        for (const chatItem of chats) {
            ...
            try {
                ...
                if (shouldSyncMessages && messages && messages.length > 0) {
                    const preparedMessages = prepareMessagesForStorage(messages, chatId, 'Phase 3');
                    ...
                    if (preparedMessages.length > 0) {
                        ...
                        await chatDB.batchSaveMessages(preparedMessages);
                        console.log(`[CLIENT_SYNC] ✅ Phase 3 - Successfully saved ${preparedMessages.length} messages for chat ${chatId}`);
                    }
                }
            } catch (saveError) {
...
function prepareMessagesForStorage(messages: any[], chatId: string, phaseName: string): any[] {
    ...
    for (const messageData of messages) {
        ...
        if (!message.message_id) {
            console.error(`[ChatSyncService] ${phaseName} - Message missing message_id after parsing, skipping:`, message);
            continue;
        }
 ...
 */
async function storeEmbedsBatch(embeds: SyncEmbed[], phaseName: string): Promise<void> {
    try {
 ...
 * @param phaseName - Phase name for logging (e.g., "Phase 1", "Phase 2", "Phase 3")
 */
async function storeEmbedKeysBatch(embed_keys: EmbedKeyEntry[], phaseName: string): Promise<void> {
    try {
        const { embedStore } = await import('./embedStore');
...
function mergeServerChatWithLocal(serverChat: Partial<Chat> & { id: string }, localChat: Chat | null): Chat {
    ...
    if (!localChat) {
        ...
        return {
            ...
            last_edited_overall_timestamp: serverChat.last_edited_overall_timestamp ?? serverChat.updated_at ?? nowTimestamp,
    ...
    const merged: Chat = {
        ...
        last_edited_overall_timestamp: serverChat.last_edited_overall_timestamp ?? localChat.last_edited_overall_timestamp ?? nowTimestamp,
        encrypted_draft_md: serverChat.encrypted_draft_md ?? localChat.encrypted_draft_md,
        encrypted_draft_preview: serverChat.encrypted_draft_preview ?? localChat.encrypted_draft_preview,
        encrypted_chat_key: serverChat.encrypted_chat_key ?? localChat.encrypted_chat_key,
