 ...
 * so they are reported with the correct key in metadata sent to the server.
 *
 ...
 */
function migrateItemType(appId: string, itemType: string): string {
  const appMigrations = ITEM_TYPE_MIGRATIONS[appId];
  return appMigrations && appMigrations[itemType]
    ? appMigrations[itemType]
    : itemType;
}
...
export async function storeAppSettingsMemoriesEntries(
  ...
  try {
    const transaction = dbInstance.db.transaction(
      [dbInstance.APP_SETTINGS_MEMORIES_STORE_NAME],
      "readwrite",
    );
    ...
    for (const entry of entries) {
      try {
        ...
        const existingEntry = await new Promise<
          AppSettingsMemoriesEntry | undefined
        >((resolve, reject) => {
        ...
        if (
          entryError instanceof DOMException &&
          entryError.name === "ConstraintError"
        ) {
          store.put({
            id: entry.id,
            app_id: entry.app_id,
          ...
          console.error(
            `[ChatDatabase] Error storing app settings/memories entry ${entry.id}:`,
            entryError,
          );
        }
      }
    ...
    console.info(
      `[ChatDatabase] App settings/memories sync complete: stored ${storedCount}, skipped ${skippedCount}, conflicts resolved ${conflictResolvedCount}`,
    );
  } catch (error) {
    console.error(
      "[ChatDatabase] Error storing app settings/memories entries:",
      error,
    );
    throw error;
  }
...
export async function getAppSettingsMemoriesEntry(
  ...
  try {
    ...
    return await new Promise<AppSettingsMemoriesEntry | null>(
      (resolve, reject) => {
    ...
    console.error(
      `[ChatDatabase] Error getting app settings/memories entry ${entryId}:`,
      error,
    );
    return null;
  }
...
export async function getAllAppSettingsMemoriesEntries(
  ...
  try {
    ...
    console.error(
      "[ChatDatabase] Error getting all app settings/memories entries:",
      error,
    );
    return [];
  }
...
export async function getAppSettingsMemoriesEntriesByApp(
  ...
  try {
    ...
    console.error(
      `[ChatDatabase] Error getting app settings/memories entries for app ${appId}:`,
      error,
    );
    return [];
  }
...
export async function deleteAppSettingsMemoriesEntry(
  ...
  try {
    ...
    console.debug(
      `[ChatDatabase] Successfully deleted app settings/memories entry: ${entryId}`,
    );
    return true;
  } catch (error) {
    console.error(
      `[ChatDatabase] Error deleting app settings/memories entry ${entryId}:`,
      error,
    );
    return false;
  }
...
export async function deleteAppSettingsMemoriesEntriesByApp(
  ...
  try {
    ...
    if (entries.length === 0) {
      console.debug(
        `[ChatDatabase] No app settings/memories entries found for app ${appId}`,
      );
      return 0;
    }
    ...
    console.info(
      `[ChatDatabase] Deleted ${entries.length} app settings/memories entries for app ${appId}`,
    );
    return entries.length;
  } catch (error) {
    console.error(
      `[ChatDatabase] Error deleting app settings/memories entries for app ${appId}:`,
      error,
    );
    return 0;
  }
...
export async function getAppSettingsMemoriesMetadataKeys(
  ...
  try {
    ...
    const entries = await new Promise<AppSettingsMemoriesEntry[]>(
      (resolve, reject) => {
        request.onsuccess = () => resolve(request.result || []);
        request.onerror = () => reject(request.error);
      },
    ...
    // Apply migrations for renamed category IDs so the server sees the current key names
    const keysSet = new Set<string>();
    for (const entry of entries) {
      if (entry.app_id && entry.item_type) {
        const migratedType = migrateItemType(entry.app_id, entry.item_type);
        keysSet.add(`${entry.app_id}-${migratedType}`);
      }
    }

    const keys = Array.from(keysSet);
    console.debug(
      `[ChatDatabase] Got ${keys.length} unique app settings/memories metadata keys from ${entries.length} entries`,
    );
    return keys;
  } catch (error) {
    console.error(
      "[ChatDatabase] Error getting app settings/memories metadata keys:",
      error,
    );
    return [];
  }
...
export async function getAppSettingsMemoriesEntryCounts(
  ...
  try {
    ...
    console.error(
      "[ChatDatabase] Error getting app settings/memories entry counts:",
      error,
    );
    return new Map();
  }
...
export async function getAppSettingsMemoriesEntriesByAppAndType(
  ...
  try {
    ...
    const allAppEntries = await getAppSettingsMemoriesEntriesByApp(
      dbInstance,
      appId,
    );

    // Filter by item_type
    const filteredEntries = allAppEntries.filter(
      (entry) => entry.item_type === itemType,
    );

    console.debug(
      `[ChatDatabase] Found ${filteredEntries.length} entries for ${appId}-${itemType}`,
    );
    return filteredEntries;
  } catch (error) {
    console.error(
      `[ChatDatabase] Error getting entries for ${appId}-${itemType}:`,
      error,
    );
    return [];
  }
