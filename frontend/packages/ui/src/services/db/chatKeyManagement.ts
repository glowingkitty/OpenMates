...
export async function encryptMessageFields(
    ...
    if (message.thinking_content) {
        encryptedMessage.encrypted_thinking_content = await encryptWithChatKey(message.thinking_content, chatKey);
    }
    ...
    if (message.thinking_signature) {
        encryptedMessage.encrypted_thinking_signature = await encryptWithChatKey(message.thinking_signature, chatKey);
    }
...
export async function decryptMessageFields(
    ...
    if (message.encrypted_thinking_content) {
        try {
            const decryptedThinkingContent = await decryptWithChatKey(message.encrypted_thinking_content, chatKey);
            ...
            console.error(`[ChatDatabase] Error decrypting thinking_content for message ${message.message_id}:`, error);
    ...
    if (message.encrypted_thinking_signature) {
        try {
            const decryptedThinkingSignature = await decryptWithChatKey(message.encrypted_thinking_signature, chatKey);
            ...
            console.error(`[ChatDatabase] Error decrypting thinking_signature for message ${message.message_id}:`, error);
