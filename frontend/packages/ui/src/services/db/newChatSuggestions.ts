...
interface ChatDatabaseInstance {
  ...
  getAllNewChatSuggestions(
    includeHidden?: boolean,
  ): Promise<NewChatSuggestion[]>;
}

// Maximum number of new chat suggestions kept per user (rolling window — oldest deleted on overflow).
// Must match MAX_NEW_CHAT_SUGGESTIONS in backend/core/api/app/tasks/persistence_tasks.py.
const MAX_SUGGESTIONS = 30;

/**
 * Save new chat suggestions (keeps last 30, encrypted with master key)
 */
export async function saveNewChatSuggestions(
  ...
  try {
    ...
    const existingEncryptedSet = new Set(
      existingSuggestions.map((s) => s.encrypted_suggestion),
    );
    ...
    for (const suggestion of suggestions) {
      ...
      if (
        encryptedSuggestion &&
        !existingEncryptedSet.has(encryptedSuggestion)
      ) {
    ...
    const transaction = dbInstance.db.transaction(
      [dbInstance.NEW_CHAT_SUGGESTIONS_STORE_NAME],
      "readwrite",
    );
    ...
    if (allSuggestions.length > MAX_SUGGESTIONS) {
      const transaction2 = dbInstance.db.transaction(
        [dbInstance.NEW_CHAT_SUGGESTIONS_STORE_NAME],
        "readwrite",
      );
      const store2 = transaction2.objectStore(
        dbInstance.NEW_CHAT_SUGGESTIONS_STORE_NAME,
      );

      // Delete oldest suggestions
      const suggestionsToDelete = allSuggestions.slice(MAX_SUGGESTIONS);
...
export async function saveEncryptedNewChatSuggestions(
  ...
  try {
    ...
    for (const suggestion of normalizedSuggestions) {
      ...
      if (
        suggestion.id &&
        !existingIdSet.has(suggestion.id) &&
        suggestion.encrypted_suggestion &&
        !existingEncryptedSet.has(suggestion.encrypted_suggestion)
      ) {
    ...
    if (currentCount + newSuggestionsToAdd.length > LIMIT) {
      ...
      await trimSuggestionsToLimit(
        dbInstance,
        LIMIT - newSuggestionsToAdd.length,
      );
    }
    ...
    const records: NewChatSuggestion[] = newSuggestionsToAdd.map(
      (suggestion) => ({
        id: suggestion.id,
    ...
    const transactionComplete = new Promise<void>((resolve, reject) => {
      ...
      transaction.onerror = (event) => {
        ...
        console.error(
          "[ChatDatabase] Transaction error saving suggestions:",
          error,
        );
        ...
        if (
          ...
          console.error(
            "[ChatDatabase] ❌ QuotaExceededError: IndexedDB storage quota exceeded. Cannot save suggestions.",
          );
          resolve();
        } else {
      ...
      transaction.onabort = (event) => {
        ...
        console.error(
          `[ChatDatabase] Transaction aborted while saving suggestions. Error: ${errorName} - ${errorMessage}`,
        );
    ...
    if (error instanceof DOMException && error.name === "QuotaExceededError") {
      console.error(
        "[ChatDatabase] ❌ QuotaExceededError saving suggestions (non-fatal): IndexedDB quota exceeded. Sync will continue without suggestions.",
      );
    } else {
      console.error(
        "[ChatDatabase] Error saving encrypted new chat suggestions (non-fatal):",
        error,
      );
    }
  }
 ...
 */
async function trimSuggestionsToLimit(
  dbInstance: ChatDatabaseInstance,
  limit: number,
): Promise<void> {
  ...
  try {
    ...
    console.error(
      "[ChatDatabase] Error trimming suggestions (non-fatal):",
      error,
    );
  }
}
...
export async function getRandomNewChatSuggestions(
  ...
  const decryptionPromises = allSuggestions.map((s) =>
    decryptWithMasterKey(s.encrypted_suggestion),
  );
  const decryptedResults = await Promise.all(decryptionPromises);

  // Filter out null results (failed decryptions)
  const decryptedSuggestions = decryptedResults.filter(
...
export async function deleteNewChatSuggestionByText(
  ...
  try {
    ...
    if (!encryptedText) {
      console.error(
        "[ChatDatabase] Failed to encrypt suggestion text for deletion",
      );
      return false;
    }
    ...
    const suggestionToDelete = allSuggestions.find(
      (s) => s.encrypted_suggestion === encryptedText,
    );

    if (!suggestionToDelete) {
      console.warn(
        "[ChatDatabase] Suggestion not found for deletion:",
        suggestionText,
      );
      return false;
    }
...
export async function deleteNewChatSuggestionById(
  ...
  try {
    ...
    console.error(
      "[ChatDatabase] Error deleting new chat suggestion by ID:",
      error,
    );
    return false;
  }
...
export async function deleteNewChatSuggestionByEncrypted(
  ...
  if (!encryptedSuggestion || encryptedSuggestion.trim() === "") {
    console.warn(
      "[ChatDatabase] Cannot delete suggestion with empty encrypted value (default suggestions cannot be deleted)",
    );
    return false;
  }
  ...
  try {
    ...
    const suggestionToDelete = allSuggestions.find(
      (s) => s.encrypted_suggestion === encryptedSuggestion,
    );

    if (!suggestionToDelete) {
      console.warn(
        "[ChatDatabase] Suggestion with encrypted content not found for deletion",
      );
      return false;
    }
    ...
    console.error(
      "[ChatDatabase] Error deleting new chat suggestion by encrypted content:",
      error,
    );
    return false;
  }
...
export async function hideNewChatSuggestionsForChat(
  ...
  try {
    ...
    const suggestionsToUpdate = allSuggestions.filter(
      (s) => s.chat_id === chatId,
    );
