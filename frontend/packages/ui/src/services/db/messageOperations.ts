 ...
 */
export function shouldUpdateMessage(existing: Message, incoming: Message): boolean {
 ...
 */
export function hasNewEncryptedFields(existing: Message, incomingEncrypted: Message): boolean {
 ...
 */
export function isContentDuplicate(existing: Message, incoming: Message): boolean {
...
export async function findContentDuplicate(
    ...
    try {
        ...
        const chatMessages = await getMessagesForChat(dbInstance, message.chat_id, transaction);
...
export async function getMessage(
    ...
    const currentTransaction = transaction || await dbInstance.getTransaction(MESSAGES_STORE_NAME, 'readonly');
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = () => {
            ...
            (async () => {
                try {
                    const decryptedMessage = await dbInstance.decryptMessageFields(encryptedMessage, encryptedMessage.chat_id);
                    resolve(decryptedMessage);
                } catch (error) {
                    console.error(`[ChatDatabase] Error decrypting message ${message_id}:`, error);
                    reject(error);
                }
            })();
        };
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = async () => {
            ...
            const decryptedMessages = await Promise.all(
                encryptedMessages.map(msg => dbInstance.decryptMessageFields(msg, chat_id))
            );
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = async () => {
            ...
            if (cursor) {
                ...
                try {
                    const decryptedMessage = await dbInstance.decryptMessageFields(encryptedMessage, chat_id);
                    resolve(decryptedMessage);
                } catch (error) {
...
export async function getAllMessages(
    ...
    return new Promise((resolve, reject) => {
        ...
        request.onsuccess = async () => {
            ...
            // CRITICAL FIX: await all decryption operations since decryptMessageFields is now async
            const decryptedMessages = await Promise.all(
                encryptedMessages.map(msg => dbInstance.decryptMessageFields(msg, msg.chat_id))
            );
            resolve(decryptedMessages);
        };
        request.onerror = () => {
...
export async function saveMessage(
    ...
    const encryptedMessage = await dbInstance.encryptMessageFields(message, message.chat_id);
    ...
    try {
        ...
        const checkTransaction = await dbInstance.getTransaction(MESSAGES_STORE_NAME, 'readonly');
        existingMessage = await getMessage(dbInstance, message.message_id, checkTransaction);
        
        if (!existingMessage) {
            contentDuplicate = await findContentDuplicate(dbInstance, message, checkTransaction);
        }
    } catch (checkError) {
    ...
    if (existingMessage) {
        const shouldUpdateDueToNewEncryptedFields = hasNewEncryptedFields(existingMessage, encryptedMessage);
        // Only update if the new message has higher priority status
        if (shouldUpdateMessage(existingMessage, message) || shouldUpdateDueToNewEncryptedFields) {
        ...
        if (shouldUpdateMessage(contentDuplicate, message)) {
            ...
            try {
                ...
                await deleteMessage(dbInstance, contentDuplicate.message_id, deleteTransaction);
            } catch (deleteError) {
    ...
    const isInvalidStateError = (error: unknown): boolean => {
        return error instanceof Error && (error.name === 'InvalidStateError' || error.message.includes('transaction'));
    };
    ...
    const saveWithRetry = async (): Promise<void> => {
        ...
        if (usesExternalTransaction && transaction) {
            try {
                ...
                const newTransaction = await dbInstance.getTransaction(MESSAGES_STORE_NAME, 'readwrite');
                return putEncryptedMessage(newTransaction, true);
            }
        }
        
        const currentTransaction = transaction || await dbInstance.getTransaction(MESSAGES_STORE_NAME, 'readwrite');
...
export async function batchSaveMessages(
    ...
    const existingMessageChecks = await Promise.all(
        validMessages.map(async (message) => {
            try {
                ...
                const checkTransaction = await dbInstance.getTransaction(MESSAGES_STORE_NAME, 'readonly');
                const existingMessage = await getMessage(dbInstance, message.message_id, checkTransaction);
    ...
    const encryptionPromises = messagesToEncrypt.map(async (message) => {
        const encrypted = await dbInstance.encryptMessageFields(message, message.chat_id);
    ...
    try {
        writeTransaction = await dbInstance.getTransaction(MESSAGES_STORE_NAME, 'readwrite');
    } catch (error) {
...
export async function deleteMessage(
    ...
    return new Promise((resolve, reject) => {
        ...
        const currentTransaction = transaction || dbInstance.db.transaction(MESSAGES_STORE_NAME, 'readwrite');
        const store = currentTransaction.objectStore(MESSAGES_STORE_NAME);
 ...
 */
export async function cleanupDuplicateMessages(dbInstance: ChatDatabaseInstance): Promise<void> {
