  ...
  try {
    ...
    try {
      ...
      try {
        ...
        console.warn("[DeepLink] Failed to parse prefill JSON, link will work without prefill:", rawJson.substring(0, 100));
        return;
      }
    }
    ...
    for (const [key, value] of Object.entries(parsed)) {
      if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") {
        ...
        console.warn(`[DeepLink] Skipping prefill field "${key}" — unsupported type: ${typeof value}`);
      }
    }
    ...
    if (isCreate) {
      ...
      if (parts.length >= 5 && parts[appIdIdx] && parts[categoryIdx]) {
        createEntryPrefillStore.set({
          ...
          suggested_title: (validatedPrefill.name as string) || (validatedPrefill.title as string) || "",
          item_value: validatedPrefill,
        });
        console.info(`[DeepLink] Stored create prefill for ${parts[appIdIdx]}/${parts[categoryIdx]}:`, Object.keys(validatedPrefill));
      }
    } else if (isEdit) {
      ...
      if (entryIdIdx > 0 && entryIdIdx < parts.length) {
        ...
        console.info(`[DeepLink] Stored update prefill for entry ${parts[entryIdIdx]}:`, Object.keys(validatedPrefill));
      }
    }
  } catch (error) {
...
export function processSettingsDeepLink(
  ...
  if (settingsPath.startsWith("/")) {
    ...
    const queryString = queryIdx >= 0 ? pathWithParams.substring(queryIdx + 1) : "";
