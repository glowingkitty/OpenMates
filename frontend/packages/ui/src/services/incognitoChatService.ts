...
class IncognitoChatService {
    ...
    async init(): Promise<void> {
        ...
        try {
            ...
            if (stored) {
                ...
                console.debug(`[IncognitoChatService] Loaded ${chatsArray.length} incognito chats from sessionStorage`);
            }

            // Load messages from sessionStorage
            const storedMessages = sessionStorage.getItem(this.MESSAGES_STORAGE_KEY);
            if (storedMessages) {
                const messagesMap = JSON.parse(storedMessages) as Record<string, Message[]>;
                Object.entries(messagesMap).forEach(([chatId, messages]) => {
                    this.messages.set(chatId, messages);
                });
                console.debug(`[IncognitoChatService] Loaded messages for ${Object.keys(messagesMap).length} incognito chats`);
            }
    ...
    /**
     * Persist chat metadata to sessionStorage
     * Only stores metadata, not full message content
     */
    private persistToSessionStorage(): void {
        ...
        try {
            ...
            console.error('[IncognitoChatService] Error persisting chats to sessionStorage:', error);
            // If storage quota exceeded, clear old data
            if (error instanceof DOMException && error.name === 'QuotaExceededError') {
                console.warn('[IncognitoChatService] SessionStorage quota exceeded, clearing old data');
                this.clearStorage();
            }
        }
    }
    ...
    private persistMessagesToSessionStorage(): void {
        ...
        try {
            ...
            sessionStorage.setItem(this.MESSAGES_STORAGE_KEY, JSON.stringify(messagesMap));
        } catch (error) {
            console.error('[IncognitoChatService] Error persisting messages to sessionStorage:', error);
            // If storage quota exceeded, clear old data
            if (error instanceof DOMException && error.name === 'QuotaExceededError') {
                console.warn('[IncognitoChatService] SessionStorage quota exceeded, clearing old messages');
                this.clearStorage();
            }
        }
    }
