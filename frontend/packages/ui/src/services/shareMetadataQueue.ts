...
/**
 * Interface for pending OG metadata update
 */
...
class ShareMetadataQueueService {
    ...
    init(): void {
        ...
        if (typeof window !== 'undefined') {
            window.addEventListener('online', () => {
                console.debug('[ShareMetadataQueue] Network online event detected - retrying pending OG metadata updates');
        ...
        this.unsubscribeWebSocketStatus = websocketStatus.subscribe((status) => {
            ...
            if (isConnected && !this.wasConnected) {
                console.debug('[ShareMetadataQueue] WebSocket reconnected - retrying pending OG metadata updates');
        ...
        webSocketService.on('open', () => {
            console.debug('[ShareMetadataQueue] WebSocket open event - retrying pending OG metadata updates');
            setTimeout(() => {
                this.retryPendingUpdates();
            }, 1000);
        });
     ...
     */
    async queueUpdate(chatId: string, title: string | null, summary: string | null): Promise<void> {
        try {
            ...
            console.debug('[ShareMetadataQueue] Queued OG metadata update for chat:', chatId);
        } catch (error) {
            console.error('[ShareMetadataQueue] Error queueing OG metadata update:', error);
        }
    }
     ...
     */
    private async storePendingUpdate(update: PendingOGMetadataUpdate): Promise<void> {
        try {
            await chatDB.init();
            const transaction = await chatDB.getTransaction([this.STORE_NAME], 'readwrite');
            ...
            console.debug('[ShareMetadataQueue] Stored pending OG metadata update:', update.update_id);
        } catch (error) {
            console.error('[ShareMetadataQueue] Error storing pending update:', error);
            throw error;
        }
    }
    ...
    private async getPendingUpdates(): Promise<PendingOGMetadataUpdate[]> {
        try {
            await chatDB.init();
            const transaction = await chatDB.getTransaction([this.STORE_NAME], 'readonly');
            const store = transaction.objectStore(this.STORE_NAME);
            ...
            console.error('[ShareMetadataQueue] Error getting pending updates:', error);
            return [];
        }
    }
    ...
    private async deletePendingUpdate(updateId: string): Promise<void> {
        try {
            ...
            console.error('[ShareMetadataQueue] Error deleting pending update:', error);
        }
    }
    ...
    async retryPendingUpdates(): Promise<void> {
        ...
        if (!get(authStore).isAuthenticated) {
            console.debug('[ShareMetadataQueue] User not authenticated, skipping retry');
            return;
        }
        ...
        try {
            ...
            if (pendingUpdates.length === 0) {
                console.debug('[ShareMetadataQueue] No pending OG metadata updates to retry');
                this.isRetrying = false;
                return;
            }

            console.info(`[ShareMetadataQueue] Retrying ${pendingUpdates.length} pending OG metadata update(s)...`);
            ...
            for (const update of pendingUpdates) {
                try {
                    ...
                    if (update.retry_count >= this.MAX_RETRY_COUNT) {
                        console.warn(`[ShareMetadataQueue] Update ${update.update_id} exceeded max retry count, removing`);
                    ...
                    if (success) {
                        ...
                        console.debug(`[ShareMetadataQueue] Successfully sent OG metadata update for chat ${update.chat_id}`);
                        await this.deletePendingUpdate(update.update_id);
                    } else {
                        // Failed - increment retry count and update
                        update.retry_count++;
                        await this.storePendingUpdate(update);
                        console.debug(`[ShareMetadataQueue] Failed to send update ${update.update_id}, retry count: ${update.retry_count}`);
                    }
                } catch (error) {
                    console.error(`[ShareMetadataQueue] Error retrying update ${update.update_id}:`, error);
            ...
            console.error('[ShareMetadataQueue] Error in retryPendingUpdates:', error);
        } finally {
            this.isRetrying = false;
        }
    }
    ...
    private async sendOGMetadataUpdate(chatId: string, title: string | null, summary: string | null): Promise<boolean> {
        try {
            ...
            console.debug('[ShareMetadataQueue] Network error sending OG metadata update:', error);
            return false;
        }
    }
    ...
    private startPeriodicRetry(): void {
        ...
        console.debug('[ShareMetadataQueue] Started periodic retry for pending OG metadata updates');
    }
