...
async function initToonDecoder() {
  if (!toonDecode) {
    try {
      ...
      console.warn(
        "[EmbedStore] TOON decoder not available, will use JSON fallback:",
        error,
      );
    }
  }
 ...
 */
async function decodeToonContentLocal(
...
export class EmbedStore {
  ...
  private async extractAppMetadata(
    ...
    try {
      ...
      console.debug(
        "[EmbedStore] Could not extract app metadata from content:",
        error,
      );
    }
  ...
  async put(
    ...
    if (typeof data === "object" && data !== null) {
      for (const [key, value] of Object.entries(data)) {
        if (value instanceof Promise) {
          ...
          throw new Error(
            `EmbedStore.put() received data with Promise in field: ${key}`,
          );
        }
      }
    }
    ...
    if (typeof dataString !== "string") {
      ...
      throw new Error(
        `JSON.stringify returned ${typeof dataString} instead of string`,
      );
    }
    ...
    if (encryptedData !== null && typeof encryptedData !== "string") {
      console.error(
        "[EmbedStore] encryptWithMasterKey returned unexpected type!",
        {
          type: typeof encryptedData,
          // eslint-disable-next-line @typescript-eslint/no-explicit-any -- runtime type check for debugging Promise leak
          isPromise: (encryptedData as any) instanceof Promise,
          value: encryptedData,
        },
      );
      throw new Error(
        `encryptWithMasterKey returned ${typeof encryptedData} instead of string or null`,
      );
    }

    if (!encryptedData) {
      console.warn(
        "[EmbedStore] Master key not available, storing unencrypted data",
      );
    }
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        request.onsuccess = () => {
          console.info(
            "[EmbedStore] ✅ Successfully stored embed in IndexedDB:",
            contentRef,
            type,
          );
          resolve();
        ...
        request.onerror = () => {
          console.error(
            "[EmbedStore] ❌ IndexedDB store.put failed:",
            contentRef,
            request.error,
          );
      ...
      console.error(
        "[EmbedStore] ❌ Failed to store in IndexedDB:",
        contentRef,
        error,
      );
  ...
  async putEncrypted(
      ...
      try {
        ...
        if (preExtractedMetadata?.app_id || preExtractedMetadata?.skill_id) {
          ...
          if (embedKey) {
            ...
            console.debug(
              "[EmbedStore] Embed key not available yet, will extract metadata later",
            );
          }
        }
      } catch (error) {
    ...
    const entry: EmbedStoreEntry = {
      ...
      hashed_task_id: encryptedData.hashed_task_id as string | undefined,
      hashed_user_id: encryptedData.hashed_user_id as string | undefined,
      embed_ids: encryptedData.embed_ids as string[] | undefined,
      parent_embed_id: encryptedData.parent_embed_id as string | undefined,
      version_number: encryptedData.version_number as number | undefined,
      file_path: encryptedData.file_path as string | undefined,
      content_hash: encryptedData.content_hash as string | undefined,
      text_length_chars: encryptedData.text_length_chars as number | undefined,
      is_private: (encryptedData.is_private as boolean | undefined) ?? false,
      is_shared: (encryptedData.is_shared as boolean | undefined) ?? false,
    ...
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        request.onsuccess = () => {
          console.debug(
            "[EmbedStore] ✅ Successfully stored encrypted embed in IndexedDB:",
            contentRef,
            { type: normalizedType, hasEmbedId: !!encryptedData.embed_id },
          );
          resolve();
        };
        request.onerror = () => reject(request.error);
   ...
   */
  private async getFromSeparateFields(
    entry: EmbedStoreEntry,
    contentRef: string,
    ...
    if (entry.encrypted_content) {
      ...
      try {
        ...
        console.debug(
          "[EmbedStore] Could not decrypt data field as master-key encrypted:",
          error,
        );
      }
    }
  ...
  async get(
    ...
    if (!entry) {
      try {
        ...
        entry = await new Promise<EmbedStoreEntry | undefined>(
          (resolve, reject) => {
    ...
    if ((storedData as any) instanceof Promise) {
      console.warn(
        "[EmbedStore] Stored data is a Promise, awaiting resolution",
      );
      // eslint-disable-next-line @typescript-eslint/no-explicit-any -- awaiting leaked Promise at runtime
      storedData = await (storedData as any);
    }

    if (!storedData) {
      console.warn(
        "[EmbedStore] Entry has no data field and no separate fields",
      );
      return undefined;
    }
    ...
    const looksLikeJSON =
      storedData.trim().startsWith("{") || storedData.trim().startsWith("[");
    ...
    if (looksLikeBase64 && !looksLikeJSON) {
      ...
      try {
        ...
        console.debug(
          "[EmbedStore] Decryption attempt failed (might be plain JSON):",
          error,
        );
        decryptedData = null;
      }
    ...
    if (decryptedData) {
      ...
      if (
        ...
        const embedIdForDecrypt =
          (parsed.embed_id as string) || contentRef.replace("embed:", "");
        if ((parsed.encrypted_content as string).trim().length === 0) {
          console.warn(
          ...
          try {
            ...
            const embedKey = await this.getEmbedKey(
              embedId,
              parsed.hashed_chat_id as string | undefined,
            );
      ...
      if (
        ...
        try {
          ...
          if (embedKey) {
            const decryptedType = await decryptWithEmbedKey(
              parsed.encrypted_type as string,
              embedKey,
    ...
    if (parsed && typeof parsed.content === "string") {
      ...
      console.debug(
        "[EmbedStore] Returning embed with TOON content:",
        contentRef,
        { hasContent: true, hasEmbedId: !!parsed.embed_id },
      );
      return parsed;
    }
    ...
    if (parsed) {
      console.debug(
        "[EmbedStore] Returning embed (may have decryption errors):",
        contentRef,
        {
    ...
    console.warn(
      "[EmbedStore] Failed to parse embed data, returning undefined:",
      contentRef,
    );
    return undefined;
  }
  ...
  setInMemoryOnly(
    ...
    const entry: EmbedStoreEntry = {
      ...
      embed_id: embedData.embed_id as string | undefined,
      status: embedData.status as EmbedStoreEntry["status"],
      parent_embed_id: embedData.parent_embed_id as string | undefined,
  ...
  async getRawEntry(
    ...
    if (!entry) {
      try {
        ...
        console.debug(
          "[EmbedStore] getRawEntry failed to load from IndexedDB:",
          error,
        );
        return null;
      }
  ...
  async getEmbedKey(
    ...
    const normalizedEmbedId = embedId.startsWith("embed:")
      ? embedId.slice("embed:".length)
      : embedId;
    ...
    if (visited.has(normalizedEmbedId)) {
      console.warn(
        "[EmbedStore] Detected embed key lookup cycle, aborting:",
        embedId,
      );
      return null;
    }
    ...
    try {
      ...
      if (parentEmbedId && parentEmbedId !== normalizedEmbedId) {
        const normalizedParentEmbedId = parentEmbedId.startsWith("embed:")
          ? parentEmbedId.slice("embed:".length)
          : parentEmbedId;
        const parentKey = await this.getEmbedKey(
    ...
    try {
      ...
      if (result.length === 0) {
        ...
        console.debug(
          "[EmbedStore] Found",
          result.length,
          "embed key entries for hashedEmbedId:",
          hashedEmbedId.substring(0, 16) + "...",
        );
      }
  ...
  setEmbedKeyInCache(
    ...
    embedKeyCache.set(cacheKey, embedKey);
    console.debug(
      "[EmbedStore] Set embed key in cache:",
      embedId,
      "cacheKey:",
      cacheKey,
      "hashedChatId:",
  ...
  async getEmbedsByAppId(appId: string): Promise<EmbedStoreEntry[]> {
    try {
      ...
      console.debug(
        `[EmbedStore] Found ${result.length} embeds for app_id: ${appId}`,
      );
      return result;
    } catch (error) {
   ...
   * @returns Array of embed entries (with encrypted content, needs decryption for use)
   */
  async getEmbedsByAppAndSkill(
    appId: string,
    skillId?: string,
  ): Promise<EmbedStoreEntry[]> {
    try {
      ...
      console.error(
        "[EmbedStore] Error querying embeds by app and skill:",
        error,
      );
      return [];
    }
  ...
  async getAllEmbedsByType(type: EmbedType): Promise<EmbedStoreEntry[]> {
    try {
      ...
      console.debug(
        `[EmbedStore] Found ${result.length} embeds for type: ${type}`,
      );
      return result;
    } catch (error) {
  ...
  async rekeyStreamToCid(streamKey: string): Promise<string> {
    ...
    const embedString =
      typeof embed === "string" ? embed : JSON.stringify(embed);
  ...
  async getEmbedsByHashedChatId(
    ...
    try {
      ...
      console.debug(
        `[EmbedStore] Found ${result.length} embeds for hashed_chat_id: ${hashedChatId.substring(0, 16)}...`,
      );
      return result;
    } catch (error) {
      console.error(
        "[EmbedStore] Error querying embeds by hashed_chat_id:",
        error,
      );
      return [];
    }
   ...
   */
  async getEmbedKeysByHashedChatId(
    hashedChatId: string,
  ): Promise<EmbedKeyEntry[]> {
    try {
      const transaction = await chatDB.getTransaction(
        [EMBED_KEYS_STORE_NAME],
        "readonly",
      ...
      console.debug(
        `[EmbedStore] Found ${result.length} embed keys for hashed_chat_id: ${hashedChatId.substring(0, 16)}...`,
      );
      return result;
    } catch (error) {
      console.error(
        "[EmbedStore] Error querying embed keys by hashed_chat_id:",
        error,
      );
      return [];
    }
   ...
   */
  async isEmbedUsedByOtherChats(
    hashedEmbedId: string,
    excludeHashedChatId: string,
  ): Promise<boolean> {
    try {
      const transaction = await chatDB.getTransaction(
        [EMBED_KEYS_STORE_NAME],
      ...
      const hasOtherChatKeys = allKeys.some(
        (key) =>
          key.key_type === "chat" &&
          key.hashed_chat_id &&
      ...
      console.debug(
        `[EmbedStore] Embed ${hashedEmbedId.substring(0, 16)}... used by other chats: ${hasOtherChatKeys}`,
      );
      return hasOtherChatKeys;
    } catch (error) {
      console.error(
        "[EmbedStore] Error checking if embed is used by other chats:",
        error,
      );
  ...
  async deleteEmbed(embedId: string, hashedEmbedId?: string): Promise<void> {
    try {
      ...
      const embedTransaction = await chatDB.getTransaction(
        [EMBEDS_STORE_NAME],
        "readwrite",
      );
      ...
      const keysToDelete = await new Promise<EmbedKeyEntry[]>(
        (resolve, reject) => {
      ...
      for (const keyEntry of keysToDelete) {
        ...
        embedKeyCache.delete(
          `${embedId}:${keyEntry.hashed_chat_id || "master"}`,
        );
      }

      console.debug(
        `[EmbedStore] Deleted embed ${embedId} and ${keysToDelete.length} associated keys`,
      );
    } catch (error) {
  ...
  async deleteEmbedsForChat(chatId: string): Promise<string[]> {
    ...
    try {
      ...
      console.debug(
        `[EmbedStore] Found ${chatEmbeds.length} embeds for chat ${chatId}`,
      );
      ...
      for (const embed of chatEmbeds) {
        if (!embed.embed_id) {
          console.warn(
            "[EmbedStore] Embed has no embed_id, skipping:",
            embed.contentRef,
          );
          continue;
        }
        ...
        const isShared = await this.isEmbedUsedByOtherChats(
          hashedEmbedId,
          hashedChatId,
        );
        ...
        if (isShared) {
          console.debug(
            `[EmbedStore] Embed ${embed.embed_id} is shared with other chats, only removing chat key`,
          );
          // Only remove the chat-specific key, not the embed itself
          await this.deleteEmbedKeyForChat(hashedEmbedId, hashedChatId);
        } else {
          console.debug(
            `[EmbedStore] Embed ${embed.embed_id} is not shared, deleting completely`,
          );
      ...
      console.info(
        `[EmbedStore] Deleted ${deletedEmbedIds.length} embeds for chat ${chatId}`,
      );
      return deletedEmbedIds;
    } catch (error) {
      console.error(
        `[EmbedStore] Error deleting embeds for chat ${chatId}:`,
        error,
      );
      return deletedEmbedIds;
    }
  ...
  async debugGetDecryptedEmbed(embedId: string): Promise<{
    ...
    const contentRef = embedId.startsWith("embed:")
      ? embedId
      : `embed:${embedId}`;
    ...
    try {
      ...
      if (!rawEntry) {
        try {
          ...
          rawEntry = await new Promise<EmbedStoreEntry | undefined>(
            (resolve, reject) => {
              const request = store.get(contentRef);
              request.onsuccess = () => resolve(request.result);
              request.onerror = () => reject(request.error);
          ...
          console.error(
            "[EmbedStore DEBUG] Error loading from IndexedDB:",
            error,
          );
        }
      }
      ...
      const result = {
        ...
        metadata: {
          ...
          decryptionSuccess:
            !!decryptedEmbed?.content && !decryptedEmbed?._decryptionFailed,
      ...
      console.log(
        "[EmbedStore DEBUG] TOON decoded (full content):",
        result.toonDecoded,
      );
      ...
      console.error(
        "[EmbedStore DEBUG] Error in debugGetDecryptedEmbed:",
        error,
      );
      return null;
    }
   ...
   * @param hashedChatId - SHA256 hash of the chat_id
   */
  private async deleteEmbedKeyForChat(
    hashedEmbedId: string,
    hashedChatId: string,
  ): Promise<void> {
    try {
      ...
      console.debug(
        `[EmbedStore] Deleted chat key for embed ${hashedEmbedId.substring(0, 16)}... in chat ${hashedChatId.substring(0, 16)}...`,
      );
    } catch (error) {
