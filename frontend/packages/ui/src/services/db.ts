...
import type { Chat, Message, ChatComponentVersions, OfflineChange, NewChatSuggestion } from '../types/chat';
...
class ChatDatabase {
    ...
    private readonly PENDING_OG_METADATA_STORE_NAME = 'pending_og_metadata_updates';
    ...
    private migrateMessagesFromChats(transaction: IDBTransaction): void {
        console.info(`[ChatDatabase] Migrating messages from chats to messages store`);
        const chatStore = transaction.objectStore(this.CHATS_STORE_NAME);
        const messagesStore = transaction.objectStore(this.MESSAGES_STORE_NAME);
        ...
        cursorRequest.onerror = (e) => {
            console.error("[ChatDatabase] CRITICAL: Error during message migration cursor:", (e.target as IDBRequest).error);
        };
    }
    ...
    private migrateMessageTimestamps(transaction: IDBTransaction): void {
        console.info(`[ChatDatabase] Migrating message timestamps: renaming timestamp to created_at`);
        const messagesStore = transaction.objectStore(this.MESSAGES_STORE_NAME);
        ...
        cursorRequest.onerror = (e) => {
            console.error("[ChatDatabase] CRITICAL: Error during message timestamp migration cursor:", (e.target as IDBRequest).error);
        };
    }
     ...
     */
    private migrateEmbedData(transaction: IDBTransaction, embedsStoreName: string): void {
        console.info(`[ChatDatabase] Migrating embeds: converting JSON string to separate fields`);
        ...
        cursorRequest.onerror = (e) => {
            console.error("[ChatDatabase] CRITICAL: Error during embed migration cursor:", (e.target as IDBRequest).error);
        };
    }
    ...
    async saveCurrentUserChatDraft(chat_id: string, draft_content: string | null, draft_preview: string | null = null): Promise<Chat | null> {
        return chatCrudOps.saveCurrentUserChatDraft(this, chat_id, draft_content, draft_preview);
    }
    
    async createNewChatWithCurrentUserDraft(draft_content: string, draft_preview: string | null = null): Promise<Chat> {
        return chatCrudOps.createNewChatWithCurrentUserDraft(this, draft_content, draft_preview);
    }
    ...
    async updateChatComponentVersion(chat_id: string, component: keyof ChatComponentVersions, version: number): Promise<void> {
        return offlineOps.updateChatComponentVersion(this, chat_id, component, version);
    }
    ...
    async saveNewChatSuggestions(suggestions: string[], chatId: string): Promise<void> {
        return newChatSuggestionsOps.saveNewChatSuggestions(this, suggestions, chatId);
    }

    async saveEncryptedNewChatSuggestions(suggestions: NewChatSuggestion[] | string[], chatId: string): Promise<void> {
        return newChatSuggestionsOps.saveEncryptedNewChatSuggestions(this, suggestions, chatId);
    }
    ...
    async deleteNewChatSuggestionByText(suggestionText: string): Promise<boolean> {
        return newChatSuggestionsOps.deleteNewChatSuggestionByText(this, suggestionText);
    }

    async deleteNewChatSuggestionById(suggestionId: string): Promise<boolean> {
        return newChatSuggestionsOps.deleteNewChatSuggestionById(this, suggestionId);
    }

    async deleteNewChatSuggestionByEncrypted(encryptedSuggestion: string): Promise<boolean> {
        return newChatSuggestionsOps.deleteNewChatSuggestionByEncrypted(this, encryptedSuggestion);
    }
    ...
    async storeAppSettingsMemoriesEntries(entries: appSettingsMemoriesOps.AppSettingsMemoriesEntry[]): Promise<void> {
        return appSettingsMemoriesOps.storeAppSettingsMemoriesEntries(this, entries);
    }

    async getAppSettingsMemoriesEntry(entryId: string): Promise<appSettingsMemoriesOps.AppSettingsMemoriesEntry | null> {
        return appSettingsMemoriesOps.getAppSettingsMemoriesEntry(this, entryId);
    }

    async getAllAppSettingsMemoriesEntries(): Promise<appSettingsMemoriesOps.AppSettingsMemoriesEntry[]> {
