 ...
 */
const fullscreenComponentRegistry = new Map<
  string,
  FullscreenComponentResolver
>();
...
export async function getFullscreenComponent(
  ...
  if (!resolver) {
    console.warn(
      `[EmbedFullscreenHandler] No resolver found for embed type: ${embedType}`,
    );
    return null;
  }
  ...
  try {
    ...
    console.error(
      `[EmbedFullscreenHandler] Error resolving fullscreen component for ${embedType}:`,
      error,
    );
    return null;
  }
