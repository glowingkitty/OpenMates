...
export async function sendUpdateTitleImpl(
  ...
  const tx = await chatDB.getTransaction(
...
export async function sendDeleteDraftImpl(
  ...
  try {
    ...
    if (clearedDraftChat) {
      ...
      console.debug(
        "[sendDeleteDraftImpl] Invalidated cache for chat:",
        chat_id,
      );
...
export async function sendNewMessageImpl(
  ...
  if (processedContent && processedContent.includes("```")) {
    ...
    while ((match = codeBlockPattern.exec(processedContent)) !== null) {
      ...
      if (
        ...
        language.toLowerCase() === "json_embed"
      ) {
        try {
  ...
  const { extractEmbedReferences, loadEmbeds } =
  ...
  if (isIncognitoChat) {
    try {
      messageHistory = await incognitoChatService.getMessagesForChat(
        message.chat_id,
      );
      console.debug(
        ...
        if (chatKey) {
          ...
          for (const embed of embeds) {
            try {
              ...
              const encryptedType = await encryptWithEmbedKey(
                embed.type,
                embedKey,
              );
...
export async function sendStoreAppSettingsMemoriesEntryImpl(
  ...
  try {
    await webSocketService.sendMessage(
      "store_app_settings_memories_entry",
      payload,
    );
    console.info(
...
export async function queueOfflineChangeImpl(
  ...
  notificationStore.info(
    `Change saved offline. Will sync when reconnected.`,
    3000,
  );
}
  ...
  try {
    ...
    const encryptedUserContent = user_message.content
      ? typeof user_message.content === "string"
        ...
        : await encryptWithChatKey(
            JSON.stringify(user_message.content),
            chatKey,
          )
      : null;
