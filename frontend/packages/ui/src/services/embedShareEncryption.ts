...
export async function generateEmbedShareKeyBlob(
  ...
  console.debug(
    "[EmbedShareEncryption] Serialized blob length:",
    serialized.length,
  );
  ...
  console.debug(
    "[EmbedShareEncryption] Encrypted blob length:",
...
export async function decryptEmbedShareKeyBlob(
  ...
  try {
    ...
    if (blob.pwd === 1) {
      if (!password) {
        console.warn(
          "[EmbedShareEncryption] Password required for protected embed share",
        );
        return {
          success: false,
    ...
    console.debug(
      "[EmbedShareEncryption] Embed share key blob decrypted successfully",
    );
    return {
      success: true,
      embedKey,
    ...
    console.error(
      `[EmbedShareEncryption] Error decrypting embed share key blob: embed_id=${embedId}`,
      error,
    );
    return {
      success: false,
 ...
 */
export async function getEmbedKeyForSharing(
  embedId: string,
  ...
  try {
    ...
    if (!embedKey) {
      console.warn(
        "[EmbedShareEncryption] Embed key not found for sharing:",
        embedId,
      );
      return null;
    }
    ...
    console.error(
      "[EmbedShareEncryption] Error getting embed key for sharing:",
      error,
    );
    return null;
  }
