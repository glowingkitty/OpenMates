...
/**
 * Migration map for renamed settings/memories category IDs.
 * When a category is renamed (e.g., "interests" → "preferred_activities"),
 * existing entries stored under the old item_type need to be mapped to the new ID
 * so they appear in the correct category after the rename.
 *
 * Format: { "app_id": { "old_item_type": "new_item_type" } }
 */
const ITEM_TYPE_MIGRATIONS: Record<string, Record<string, string>> = {
  travel: {
...
function createAppSettingsMemoriesStore() {
  ...
  async function decryptEntries(
    entries: AppSettingsMemoriesEntry[],
  ): Promise<Map<string, DecryptedEntry>> {
    ...
    if (!masterKey) {
      console.error(
        "[AppSettingsMemoriesStore] No master key available for decryption",
      );
      return decrypted;
    }
    ...
    for (const entry of entries) {
      try {
        ...
        try {
          ...
          console.warn(
            `[AppSettingsMemoriesStore] Could not decrypt item for entry ${entry.id}, using hash as fallback:`,
            itemError,
          );
        ...
        // Apply migration for renamed category IDs (e.g., "interests" → "preferred_activities")
        const appMigrations = ITEM_TYPE_MIGRATIONS[entry.app_id];
        const settingsGroup =
          appMigrations && appMigrations[entry.item_type]
            ? appMigrations[entry.item_type]
            : entry.item_type;
        ...
        console.error(
          `[AppSettingsMemoriesStore] Error processing entry ${entry.id}:`,
          error,
        );
      }
    }
  ...
  return {
    ...
    async loadEntries() {
      ...
      try {
        ...
        console.info(
          `[AppSettingsMemoriesStore] Loaded ${entries.length} app settings/memories entries`,
        );
      } catch (error) {
        const errorMsg =
          error instanceof Error ? error.message : "Unknown error";
        console.error(
          "[AppSettingsMemoriesStore] Error loading entries:",
          error,
        );
        update((state) => ({
          ...state,
    ...
    async loadEntriesForApp(appId: string) {
      ...
      try {
        ...
        console.info(
          `[AppSettingsMemoriesStore] Loaded ${entries.length} entries for app ${appId}`,
        );
      } catch (error) {
        const errorMsg =
          error instanceof Error ? error.message : "Unknown error";
        console.error(
          `[AppSettingsMemoriesStore] Error loading entries for app ${appId}:`,
          error,
        );
        update((state) => ({
          ...state,
    ...
    async createEntry(
      ...
      try {
        ...
        const hashedItemKey = await hashString(
          `${appId}-${entryData.item_key}-${now}`,
        );
        ...
        const itemValueWithMetadata = {
          ...(typeof entryData.item_value === "object" &&
          entryData.item_value !== null
        ...
        if (!encryptedItemJson) {
          throw new Error(
            "Failed to encrypt entry data - master key may not be available",
          );
        }
        ...
        console.debug(
          `[AppSettingsMemoriesStore] Stored encrypted entry ${itemId} in IndexedDB`,
        );
        ...
        console.info(
          `[AppSettingsMemoriesStore] Created entry ${itemId} for app ${appId}`,
        );
        ...
        try {
          const syncSuccess =
            await chatSyncService.sendStoreAppSettingsMemoriesEntry(
              encryptedEntry,
            );
          if (syncSuccess) {
            console.info(
              `[AppSettingsMemoriesStore] Synced entry ${itemId} to server`,
            );
          } else {
            console.warn(
              `[AppSettingsMemoriesStore] Failed to sync entry ${itemId} to server (will retry on reconnect)`,
            );
          }
        } catch (syncError) {
          // Don't fail the whole operation if sync fails - entry is still stored locally
          console.error(
            `[AppSettingsMemoriesStore] Error syncing entry ${itemId} to server:`,
            syncError,
          );
        }
      } catch (error) {
        console.error(
          "[AppSettingsMemoriesStore] Error creating entry:",
          error,
        );
        throw error;
      }
    ...
    async updateEntry(
      ...
      try {
        ...
        // PRIVACY: Hash the item_key so cleartext content is NOT exposed in IndexedDB/Directus
        const hashedItemKey = await hashString(
          `${appId}-${entryData.item_key}-${existingEntry.created_at}`,
        );
        ...
        await chatDB.storeAppSettingsMemoriesEntries([encryptedEntry]);
        console.debug(
          `[AppSettingsMemoriesStore] Updated encrypted entry ${entryId} in IndexedDB`,
        );
        ...
        update((state) => {
          ...
          if (oldGroup !== entryData.settings_group && appGroups[oldGroup]) {
            appGroups[oldGroup] = appGroups[oldGroup].filter(
              (e) => e.id !== entryId,
            );
          ...
          appGroups[newGroup] = appGroups[newGroup].filter(
            (e) => e.id !== entryId,
          );
          // Add updated entry
        ...
        console.info(
          `[AppSettingsMemoriesStore] Updated entry ${entryId} for app ${appId}`,
        );
        ...
        try {
          ...
          if (syncSuccess) {
            console.info(
              `[AppSettingsMemoriesStore] Synced updated entry ${entryId} to server`,
            );
          } else {
            console.warn(
              `[AppSettingsMemoriesStore] Failed to sync updated entry ${entryId} to server`,
            );
          }
        } catch (syncError) {
          console.error(
            `[AppSettingsMemoriesStore] Error syncing updated entry ${entryId} to server:`,
            syncError,
          );
        }
      } catch (error) {
        console.error(
          "[AppSettingsMemoriesStore] Error updating entry:",
          error,
        );
        throw error;
      }
    ...
    async deleteEntry(entryId: string, appId: string) {
      try {
        ...
        if (!existingEntry) {
          console.warn(
            `[AppSettingsMemoriesStore] Entry ${entryId} not found for deletion`,
          );
          return;
        }
        ...
        console.debug(
          `[AppSettingsMemoriesStore] Deleted entry ${entryId} from IndexedDB`,
        );
...
export const appSettingsMemoriesForApp = (appId: string) => {
  return derived(
    ...
    (state) => state.entriesByApp.get(appId) || {},
  );
};
