...
type ApiUserProfile = UserProfile & { id?: string | null };
...
export interface SessionCheckResult {
  success: boolean;
  user?: ApiUserProfile; // Backend returns id; the local profile stores it as user_id.
  message?: string;
...
export interface LoginResult {
  ...
  user?: ApiUserProfile; // User profile data on successful login
...
export interface LogoutCallbacks {
  ...
  onError?: (error: unknown) => void | Promise<void>; // Called on any error during logout
