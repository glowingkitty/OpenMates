...
export interface VideoIframeState {
  ...
  /** Fullscreen anchor rect (relative to active-chat-container) used to align iframe with placeholder */
  fullscreenAnchor?: {
    top: number;
    left: number;
    width: number;
...
export const videoIframeStore = {
  ...
  playVideo: (data: {
    ...
    set({
      ...
      iframeElement: undefined,
      iframeWrapperElement: undefined,
      fullscreenAnchor: data.fullscreenAnchor,
    });
  },
  ...
  open: (data: {
    ...
    console.debug(
      "[VideoIframeStore] Opening (preparing video data):",
      data.url,
    );
    update((state) => ({
      ...state,
  ...
  updateIframeRefs: (
    ...
    iframeWrapperElement: HTMLDivElement | null | undefined,
  ) => {
    update((state) => ({
      ...state,
  ...
  /**
   * Update fullscreen anchor while fullscreen embed layout is active.
   * This keeps the standalone iframe aligned with the placeholder slot.
   */
  setFullscreenAnchor: (
    anchor: { top: number; left: number; width: number } | undefined,
  ) => {
    update((state) => ({
      ...state,
      fullscreenAnchor: anchor,
    }));
  },
