...
function createPermissionDialogStore() {
  const { subscribe, set, update } =
    writable<PermissionDialogState>(initialState);
  ...
  return {
    ...
    showDialog(request: PendingPermissionRequest) {
      console.info(
        "[PermissionDialogStore] Showing dialog for request:",
        request.requestId,
      );
      update((state) => ({
        ...state,
    ...
    toggleCategory(categoryKey: string) {
      update((state) => {
        ...
        const updatedCategories = state.currentRequest.categories.map((cat) => {
          ...
          // IMPORTANT: Create new array AND new entry objects for Svelte reactivity
          ...
          console.debug(
            `[PermissionDialogStore] Toggled category ${categoryKey}: ${!newSelected} -> ${newSelected}. ` +
              `Entries: ${updatedEntries ? `${updatedEntries.filter((e) => e.selected).length}/${updatedEntries.length} selected` : "none"}`,
          );
...
export function initPermissionDialogListener() {
  ...
  const handleDismissDialog = (
    event: CustomEvent<DismissDialogEventDetail>,
  ) => {
  ...
  window.addEventListener(
    "showAppSettingsMemoriesPermissionDialog",
    handleShowDialog as EventListener,
  );
  window.addEventListener(
    "dismissAppSettingsMemoriesPermissionDialog",
    handleDismissDialog as EventListener,
  );

  console.info(
    "[PermissionDialogStore] Initialized event listeners for permission dialog",
  );
