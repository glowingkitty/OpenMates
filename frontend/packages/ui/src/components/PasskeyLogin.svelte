<script lang="ts">
    ...
    async function handlePasskeyLogin() {
        ...
        try {
            ...
            // Step 11: Derive email encryption key
            // If email not available yet, we'll decrypt it from backend response
            // For now, we need email for key derivation - get from backend if needed
            if (!userEmail && verifyData.encrypted_email) {
                // We'll decrypt it after we have the email encryption key
                // For now, we need to get email from userHandle or backend
                // Backend should provide user_email in response for resident credentials
                if (verifyData.user_email) {
                    userEmail = verifyData.user_email;
                }
            }
            
            // If still no email, we need it for key derivation
            // This should not happen with resident credentials - backend should provide email
            if (!userEmail) {
            ...
            // Step 12: Unwrap master key using PRF-derived wrapping key
            ...
            // Step 13: Store email encryption key and salt
            cryptoService.saveEmailEncryptionKey(emailEncryptionKey, stayLoggedIn);
            cryptoService.saveEmailSalt(emailSalt, stayLoggedIn);
            
            // Step 14: Decrypt and store email with master key
            ...
            // Step 15: Store WebSocket token if provided
            ...
            // Step 16: Update user profile if user data is available
            ...
            // Step 17: Check if user is in signup flow
