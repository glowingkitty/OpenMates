...
<style>
  ...
  .field{display:grid;align-content:start;gap:.4rem;min-width:0;text-align:start}.field label,legend{font-size:.78rem;font-weight:650}.type{font-size:.65rem;background:var(--color-primary);color:var(--color-font-button);border-radius:.2rem;padding:.1rem .3rem;margin-inline-end:.3rem}.type[data-type="number"],.type[data-type="integer"]{background:var(--color-error)}
  input,select{width:100%;box-sizing:border-box;min-height:2.5rem;border:1px solid var(--color-grey-25);border-radius:.8rem;padding:.5rem .7rem;background:var(--color-grey-0);color:var(--color-font-primary);font:inherit;font-size:.85rem;box-shadow:var(--shadow-sm)}
  .object{grid-column:1/-1;min-width:0;border:1px solid var(--color-grey-20);border-radius:.8rem;padding:.75rem;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.8rem}.object legend{padding:0 .3rem}.array-entry{grid-column:1/-1;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:.5rem}.array-entry>.object{grid-column:auto}.quiet,.variable{background:transparent;color:var(--color-primary);border:0;box-shadow:none;font:inherit;font-size:.75rem;cursor:pointer;min-height:2rem}.quiet{align-self:start}.variable{padding:0;text-align:start;min-height:1.5rem}details{font-size:.7rem;color:var(--color-font-secondary)}summary{cursor:pointer}p{margin:.4rem 0;white-space:pre-wrap;user-select:text}input:focus-visible,select:focus-visible,button:focus-visible{outline:2px solid var(--color-button-primary);outline-offset:2px}
