...
<script lang="ts">
  ...
  import { record, label, schemaDefault, normalizeSchema, isCheck, isTrigger, isMessage, messageDestinationConfig, capabilityFor, outputsBefore, insertNode, removeNode, type Capability, type Insertion, type Output } from './workflowBuilder';
  ...
  async function deleteNode(): Promise<void> { if (!draft || !onSave || busy) return; busy = true; try { await onSave({ ...removeNode(graph, draft.id), version: 2 }); closeEditor(); } catch (error) { failForUser(error, 'save_failed'); } finally { busy = false; } }
...
{#snippet choice(icon: string, title: string, action: () => void, testId?: string)}
  {@const Icon = getLucideIcon(icon)}{@const asset = ({ blocks: 'app', 'messages-square': 'chat', 'calendar-clock': 'workflow', 'calendar-days': 'calendar', 'git-branch': 'workflow-check' } as Record<string, string>)[icon]}<button type="button" class="choice" data-testid={testId} onclick={action}>{#if asset}<AppIcon name={asset} size="24px" color="var(--color-primary, #536de6)" noMargin ariaHidden/>{:else}<Icon size={23}/>{/if}<span>{title}</span></button>
{/snippet}
...
{#snippet editor()}
  {#if draft}
    ...
    <div class="editor" class:skill-editor={draft.type === 'app_skill_action'} style={style(draft)} data-testid="workflow-node-expanded">
      <div class="panel-top"><span></span><span class="editor-kind">{#if isCheck(draft)}<AppIcon name="workflow-check" size="20px" color="var(--color-font-secondary)" noMargin ariaHidden/>{/if}{kind(draft)}</span><button type="button" class="quiet" disabled={busy || testStatus === 'processing'} aria-label={tr('close')} onclick={closeEditor}><Close size={19}/></button></div>
      {#if isTrigger(draft)}
      ...
      {:else if draft.type === 'app_skill_action'}
        <div class="skill-heading" style={style(draft)}><AppIcon name={appIcon(draft)} size="30px" color="white" noMargin ariaHidden/><h3>{summary(draft)}</h3>{#if location(draft)}<span>{location(draft)}</span>{/if}</div>
        <h4>↓ {tr('input')}</h4>
      ...
      {:else if isCheck(draft)}
        ...
        <div class="check-fields"><select aria-label={tr('select_output')} value={String(predicate.left ?? '')} onchange={event => predicatePatch({ left: event.currentTarget.value, op: '', right: '' })}><option value="">{tr('select_output')}</option>{#each outputs.filter(item => !['array','object'].includes(item.schema.type ?? '')) as output}<option value={output.reference}>{output.label}</option>{/each}</select>
          ...
          {#if predicate.op}<span class="type" data-value-type={valueType(sourceSchema(predicate.left))}>{tr(`output_type_${valueType(sourceSchema(predicate.left))}`)}</span>{#if sourceSchema(predicate.left)?.type === 'boolean'}<select aria-label={tr('compare_value')} value={String(predicate.right)} onchange={event => predicatePatch({ right: event.currentTarget.value === 'true' })}><option value="true">{tr('true')}</option><option value="false">{tr('false')}</option></select>{:else}<input aria-label={tr('compare_value')} type={['number','integer'].includes(sourceSchema(predicate.left)?.type ?? '') ? 'number' : 'text'} value={String(predicate.right ?? '')} oninput={event => predicatePatch({ right: ['number','integer'].includes(sourceSchema(predicate.left)?.type ?? '') ? Number(event.currentTarget.value) : event.currentTarget.value })}/>{/if}{/if}
        </div>
  ...
  {#if node && node.type !== 'end' && !visited.includes(nodeId) && nodeId !== stopAt}
    ...
    <article class="flow-node" data-node-id={node.id} data-node-type={node.type} data-testid="workflow-node-card">
      {#if draft?.id === node.id}{@render editor()}{:else}
        <button type="button" class="node-summary" class:skill={node.type === 'app_skill_action'} class:expanded={readOnly && expandedReadOnly === node.id} style={style(node)} data-testid="workflow-node-summary" aria-expanded={expandedReadOnly === node.id} onclick={() => edit(node)}><span class="kind">{kind(node)}</span><span class="node-app-icon">{#if node.type === 'app_skill_action'}<AppIcon name={appIcon(node)} size="28px" color="white" noMargin ariaHidden/>{:else if isCheck(node) && checkSource(node)}<AppIcon name={appIcon(checkSource(node)!)} size="28px" color="var(--color-primary, #536de6)" noMargin ariaHidden/>{:else if isTrigger(node) || isMessage(node)}<AppIcon name={isTrigger(node) ? 'calendar' : 'chat'} size="28px" color="var(--color-primary, #536de6)" noMargin ariaHidden/>{:else}<Icon size={26}/>{/if}</span>{#if isCheck(node) && checkSource(node)}<span class="check-source">{summary(checkSource(node)!)}</span>{/if}<strong data-testid="workflow-node-title-label">{summary(node)}</strong>{#if node.type === 'app_skill_action' && location(node)}<span class="location">{location(node)}</span>{/if}{#if run}<span data-testid="workflow-run-node-status" data-node-status={run.status}>{String(isMessage(node) && run.output_summary?.status ? run.output_summary.status : run.status).replaceAll('_',' ')}</span>{/if}<Down size={16}/></button>
...
<style>
  ...
  .skill-editor>.panel-top{background:var(--node-gradient);color:white;margin-inline:-1.5rem;padding-inline:.6rem;border-radius:1rem 1rem 0 0}.skill-editor>.panel-top .quiet{color:white}.skill-heading{margin-top:-1rem;border-radius:0;padding-block:.8rem 1.2rem}.skill-heading span{font-size:16px}.node-app-icon{display:grid;place-items:center}.message-preview section{display:grid;gap:.55rem}.graph-panel{margin-block:1.75rem}.node-summary.skill .node-app-icon{color:white}@media(max-width:730px){.skill-editor>.panel-top{margin-inline:-.8rem}.output-fields>div{grid-template-columns:1fr;gap:.35rem}}
  ...
  .editor-kind,.branch-label{display:flex;align-items:center;justify-content:center;gap:.4rem}
