...
<script lang="ts">
  ...
  function assetIconStyle(name: string, size: number, color = 'var(--color-primary, #536de6)'): string { return `--workflow-icon:var(--icon-url-${name});--workflow-icon-size:${size}px;color:${color}`; }
...
{#snippet editor()}
  {#if draft}
    ...
    <div class="editor" class:skill-editor={draft.type === 'app_skill_action'} style={style(draft)} data-testid="workflow-node-expanded">
      <div class="panel-top"><span></span><span class="editor-kind">{#if isCheck(draft)}<span class="workflow-icon" style={assetIconStyle('workflow-check', 20, 'var(--color-font-secondary)')} aria-hidden="true"></span>{/if}{kind(draft)}</span><button type="button" class="quiet" disabled={busy || testStatus === 'processing'} aria-label={tr('close')} onclick={closeEditor}><Close size={19}/></button></div>
      {#if isTrigger(draft)}
      ...
      {:else if draft.type === 'app_skill_action'}
        <div class="skill-heading" style={style(draft)}><span class="workflow-icon" style={assetIconStyle(appIcon(draft), 30, 'var(--color-font-button)')} aria-hidden="true"></span><h3>{summary(draft)}</h3>{#if location(draft)}<span>{location(draft)}</span>{/if}</div>
        <h4>↓ {tr('input')}</h4>
  ...
  {#if node && node.type !== 'end' && !visited.includes(nodeId) && nodeId !== stopAt}
    ...
    <article class="flow-node" data-node-id={node.id} data-node-type={node.type} data-testid="workflow-node-card">
      {#if draft?.id === node.id}{@render editor()}{:else}
        <button type="button" class="node-summary" class:skill={node.type === 'app_skill_action'} class:expanded={readOnly && expandedReadOnly === node.id} style={style(node)} data-testid="workflow-node-summary" aria-expanded={expandedReadOnly === node.id} onclick={() => edit(node)}><span class="kind">{kind(node)}</span><span class="node-app-icon">{#if node.type === 'app_skill_action'}<span class="workflow-icon" style={assetIconStyle(appIcon(node), 32, 'var(--color-font-button)')} aria-hidden="true"></span>{:else if isCheck(node) && checkSource(node)}<span class="workflow-icon" style={assetIconStyle(appIcon(checkSource(node)!), 32)} aria-hidden="true"></span>{:else if isTrigger(node) || isMessage(node)}<span class="workflow-icon" style={assetIconStyle(isTrigger(node) ? 'calendar' : 'chat', 32)} aria-hidden="true"></span>{:else}<Icon size={30}/>{/if}</span>{#if isCheck(node) && checkSource(node)}<span class="check-source">{summary(checkSource(node)!)}</span>{/if}<strong data-testid="workflow-node-title-label">{summary(node)}</strong>{#if node.type === 'app_skill_action' && location(node)}<span class="location">{location(node)}</span>{/if}{#if run}<span class="run-status" class:success={runStatus === 'completed'} class:failed={runStatus === 'failed'} data-testid="workflow-run-node-status" data-node-status={run.status} role="img" aria-label={$text(`workflows.runs.status_${runStatus}`)}>{#if runStatus === 'completed'}<span class="workflow-icon" style={assetIconStyle('check', 16, 'var(--color-font-button)')} aria-hidden="true"></span>{:else}{$text(`workflows.runs.status_${runStatus}`)}{/if}</span>{/if}<span class="node-chevron" aria-hidden="true"><Down size={20}/></span></button>
