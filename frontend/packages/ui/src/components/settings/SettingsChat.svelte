...
<script lang="ts">
    ...
    let notificationStatus = $derived(
        pushEnabled || emailEnabled
            ? $text('settings.chat.notifications.enabled', { default: 'Enabled' })
            : $pushNotificationStore.permission === 'denied'
                ? $text('settings.chat.notifications.blocked', { default: 'Blocked by browser' })
                : $text('settings.chat.notifications.disabled', { default: 'Disabled' })
    );
    ...
    function navigateToNotifications() {
        dispatch('openSettings', {
            ...
            direction: 'forward',
            icon: 'notification',
            title: $text('settings.chat.notifications', { default: 'Notifications' })
        });
    }
</script>
