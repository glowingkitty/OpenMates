    ...
    function selectPeriod(periodKey: string) {
        ...
        setTimeout(() => {
            dispatch('openSettings', {
                ...
                direction: 'backward',
                icon: 'privacy',
                title: $text('settings.privacy.privacy.text')
            });
        }, 200);
    }
</script>
...
<div class="auto-deletion-description">
    <p class="description-text">
        {$text(`settings.privacy.privacy.auto_deletion.${category}.description.text`)}
    </p>
</div>
...
<SettingsItem
    type="heading"
    icon={categoryIcon}
    title={$text(`settings.privacy.privacy.auto_deletion.select_period.text`)}
/>
...
{#each periodOptions as option}
    ...
    >
        <span class="period-label">
            {$text(`settings.privacy.privacy.${option.translationKey}.text`)}
        </span>
