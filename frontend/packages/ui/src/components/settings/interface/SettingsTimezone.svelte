...
<script lang="ts">
    ...
    async function handleTimezoneChange(newTimezone: string) {
        ...
        try {
            ...
            successMessage = $text('settings.interface.timezone_saved.text');
...
<div class="settings-timezone">
    {#if isLoading}
        <div class="loading">{$text('settings.interface.timezone_loading.text')}</div>
    {:else}
        ...
        <div class="current-timezone-section">
            <SettingsItem
                type="nested"
                icon="clock"
                title={$text('settings.interface.timezone_current.text')}
            ...
            {#if browserTimezone && browserTimezone !== currentTimezone}
                <div class="browser-timezone-hint">
                    <button 
                        ...
                        disabled={isSaving}
                    >
                        {$text('settings.interface.timezone_use_browser.text')} ({getTimezoneLabel(browserTimezone)})
                    </button>
                </div>
            {/if}
        </div>
        ...
        {#if currentTimezone && !isKnownTimezone(currentTimezone)}
            <div class="timezone-group">
                <h3 class="region-header">{$text('settings.interface.timezone_other.text')}</h3>
                <div class="timezone-list">
                    <SettingsItem 
                        type="quickaction"
                        icon="clock"
        ...
        <div class="info-box">
            <p>{$text('settings.interface.timezone_info.text')}</p>
        </div>
    {/if}
</div>
