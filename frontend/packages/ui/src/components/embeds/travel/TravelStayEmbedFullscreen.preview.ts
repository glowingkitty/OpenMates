 ...
 *
 * Uses the data-driven shape (EmbedFullscreenRawData) — the stay fields
 * are flattened into decodedContent, matching what the component extracts via
 * `data.decodedContent`. The old `{ stay: {...} }` direct-prop format
  ...
  budget: {
    data: {
      decodedContent: {
        ...
        description:
          "A modern hostel in a prime location with shared and private room options.",
