...
<script lang="ts">
  ...
  let routeHeaderWithFlags = $derived.by(() => {
    ...
    // Extract city name from "Berlin (BER)" format
  ...
  let startAirportName = $derived.by(() => {
    ...
    const firstSeg = conn?.legs?.[0]?.segments?.[0];
    if (!firstSeg) return '';
    // Try to get full airport name from origin display string
   ...
   * Currently unused — EntryWithMapTemplate doesn't expose onDownload.
   * Kept for future re-integration.
   */
  ...
  /**
   * Draw the flight path on the map. Called from onMapReady and when
   * flight track data loads asynchronously.
   */
...
>
  {#snippet embedHeaderCta()}
    <!-- Booking CTA button (three-state: idle → loading → loaded) -->
  ...
  {#snippet detailContent(_childContext)}
    <div class="flight-card" data-testid="flight-details-card">
      <!-- Route header pill: "🇩🇪 Berlin → 🇹🇭 Bangkok" -->
      ...
      {#if connection.legs && connection.legs.length > 0}
                ...
                <div class="segment-carrier-row">
                  {#if segment.airline_logo}
                    <img
                      ...
                      src={proxyImage(segment.airline_logo, MAX_WIDTH_AIRLINE_LOGO_FULLSCREEN)}
                      alt={segment.carrier}
                    />
                  {/if}
        ...
        {/each}
      {:else}
        <!-- No leg details - summary fallback -->
...
<style>
  /* ===========================================
     CTA Button (in gradient header)
     Figma: booking button inside the header area
     =========================================== */
  ...
  /* ===========================================
     Flight Details Card
     Figma: node 4956-45277
     White card with route header, segment cards, layovers
     =========================================== */
  ...
  /* ===========================================
     Segment Card
     Figma: grey rounded card per flight segment
     Layout: [left: times+duration] [center: airline+codes]
     =========================================== */
  ...
  /* Left column: time badges + duration */
  .segment-left {
    display: flex;
    flex-direction: column;
  ...
  /* Time badge: colored pill with sun/moon icon + time */
  .time-badge {
    display: inline-flex;
    align-items: center;
    gap: 4px;
  ...
  /* Night: blue gradient */
  .time-badge.nighttime {
    background: linear-gradient(to right, #365dad, #1745a1);
  }

  /* Day: amber/yellow gradient */
  .time-badge.daytime {
    background: linear-gradient(to right, #f5bb12, #e79600);
  }
  ...
  /* Duration in blue gradient text */
  .segment-duration-text {
    font-size: 1.25rem;
    font-weight: 700;
    background: linear-gradient(164deg, rgb(72, 103, 205) 9%, rgb(90, 133, 235) 90%);
  ...
  /* Center: airport codes + carrier info */
  .segment-center {
    display: flex;
    flex-direction: column;
    gap: 8px;
  ...
  /* ===========================================
     Layover Section (between segment cards)
     Figma: duration text + overnight badge + airport name
     =========================================== */
  ...
  /* ===========================================
     Summary Fallback (no leg details)
     =========================================== */
  ...
  /* ===========================================
     Booking Info Footer (CTA content area)
     =========================================== */
  ...
  /* ===========================================
     Map marker overrides (via EntryWithMapTemplate)
     =========================================== */

  :global(.travel-route-marker) {
    background: transparent !important;
    border: none !important;
  }

  :global(.travel-route-marker .marker-dot) {
    width: 12px;
    height: 12px;
    border-radius: 50%;
