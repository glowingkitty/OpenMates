<!--
  frontend/packages/ui/src/components/embeds/social_media/SocialMediaSearchEmbedPreview.svelte

  Preview card for Social Media / Search parent embeds.
  It keeps the card lightweight and relies on child post embeds for the
  detailed grid in fullscreen.
...
<script lang="ts">
  ...
  let localQuery = $state('Social media search');
  ...
  $effect(() => {
    localQuery = queryProp || 'Social media search';
