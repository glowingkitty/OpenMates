...
function parseTableRow(line: string): string[] {
  ...
  const content = withoutPipes.endsWith('|') ? withoutPipes.slice(0, -1) : withoutPipes;
 ...
 */
function parseAlignments(separatorLine: string, colCount: number): Array<'left' | 'center' | 'right' | undefined> {
...
export function parseSheetEmbedContent(
  ...
  if (typeof content === 'object' && content !== null) {
    ...
    markdown = (contentObj.code || contentObj.table || contentObj.content || '') as string;
  ...
  if (!title) {
    ...
    if (titleMatch) {
      ...
      markdown = markdown.replace(/<!--\s*title:\s*"[^"]+"\s*-->\n?/, '').trim();
    }
  }
...
export function countTableColumns(markdownTable: string): number {
  ...
  const withoutLeadingPipe = headerLine.startsWith('|') ? headerLine.slice(1) : headerLine;
