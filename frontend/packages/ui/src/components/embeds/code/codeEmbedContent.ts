...
function parseLangPathHeader(
  firstLine: string,
): { language: string; filename: string } | null {
  ...
  // Reject numeric-only languages (e.g., "22" from timestamps like "22:56:19.011 Build...")
  if (/^\d+$/.test(language)) return null;

  return { language, filename };
}

export function parseCodeEmbedContent(
  rawCodeContent: string,
  hints: ParseHints = {},
): ParsedCodeEmbedContent {
  ...
  const firstLine = (
    firstNewlineIndex === -1
      ? normalized
      : normalized.slice(0, firstNewlineIndex)
  ).trimEnd();
  ...
  const strippedCode = header
    ? firstNewlineIndex === -1
      ? ""
      : normalized.slice(firstNewlineIndex + 1)
    : normalized;

  const resolvedLanguage =
    !isBlank(hints.language) &&
    !PLAIN_LANGS.has((hints.language || "").toLowerCase())
      ? hints.language
      : header?.language;

  const resolvedFilename = !isBlank(hints.filename)
    ? hints.filename
    : header?.filename;
