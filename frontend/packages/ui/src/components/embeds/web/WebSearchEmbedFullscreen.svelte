<!--
  ...
  - Website embeds in a responsive grid
  - Overlay drill-down into WebsiteEmbedFullscreen with sibling navigation
...
<script lang="ts">
  import SearchResultsTemplate, { type ChildNavContext } from '../SearchResultsTemplate.svelte';
  ...
  interface Props {
    ...
    previewData?: { query?: string; provider?: string; status?: string; results?: unknown[] };
    onClose: () => void;
    embedId?: string;
  ...
  function handleEmbedDataUpdated(data: { status: string; decodedContent: Record<string, unknown> }) {
    ...
    const s = data.status;
    if (s === 'processing' || s === 'finished' || s === 'error' || s === 'cancelled') localStatus = s;
