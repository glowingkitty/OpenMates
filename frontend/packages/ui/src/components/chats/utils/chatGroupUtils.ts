...
export function groupChats(
  ...
  return chatsToGroup.reduce<Record<string, ChatType[]>>((groups, chat) => {
    ...
    if (!chatTimestamp || chatTimestamp === 0) {
      ...
      console.warn(
        `[ChatGroupUtils] Chat ${chat.chat_id} has invalid/zero timestamp (${chatTimestamp}), using current time for grouping`,
      );
    ...
    if (!chatDateSource || isNaN(chatDateSource.getTime())) {
      console.warn(
        `[ChatGroupUtils] Chat ${chat.chat_id} has invalid date for grouping. Placing in 'today'.`,
      );
    ...
    const chatDateOnly = new Date(
      chatDateSource.getFullYear(),
      chatDateSource.getMonth(),
      chatDateSource.getDate(),
    );
