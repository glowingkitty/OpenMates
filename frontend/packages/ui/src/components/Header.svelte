...
<script lang="ts">
  ...
  let {
    ...
    isSidebarOpen = false,
    /** Optional public publication label rendered below the OpenMates logo. */
    publicationLabel = undefined as string | undefined,
  ...
  let activeWorkspaceIndex = $derived(
    Math.max(
      0,
      webappWorkspaceTabs.findIndex((item) => item.active),
    ),
  );
  let activeWorkspaceTab = $derived(
  ...
  const handleWorkspaceSelectChange = async (event: Event) => {
    ...
    const selectedTab = webappWorkspaceTabs.find(
      (item) => item.href === target.value,
    );
  ...
  onMount(() => {
    ...
    if (!publicationLabel) {
      ...
      initializeFeatureAvailability().catch((error) => {
        console.warn(
          "[Header] Failed to initialize feature availability:",
          error,
        );
      });
    }
  ...
  function _slideFade(node: HTMLElement, { duration = 200 }) {
      ...
      <nav class:webapp={context === "webapp"}>
        <div class="left-section">
          ...
          <div class="logo-container">
            <a
              ...
              target={!publicationLabel &&
              (serverEdition === "self_hosted" ||
                serverEdition === "development")
                ? "_blank"
                : undefined}
              rel={!publicationLabel &&
              (serverEdition === "self_hosted" ||
                serverEdition === "development")
                ? "noopener noreferrer"
                : undefined}
            >
              <strong
                ><mark>Open</mark><span style="color: var(--color-grey-100);"
                  >Mates</span
                ></strong
              >
              <span class="mobile-logo-icon" aria-hidden="true"></span>
            </a>
            {#if !publicationLabel && (serverEdition === "self_hosted" || serverEdition === "development")}
              <div
        ...
        {:else if WORKSPACE_SWITCHER_ENABLED && context === "webapp" && isLoggedIn && webappWorkspaceTabs.length >= 2}
          ...
          <div class="workspace-select-shell" aria-label="Workspace switcher">
            <span
              class={`workspace-select-icon ${activeWorkspaceTab?.iconClass ?? "chat-icon"}`}
              aria-hidden="true"
            ></span>
            <select
...
<style>
  ...
  .menu-button-container {
    ...
    transition:
      ...
      min-width var(--duration-normal) var(--easing-default);
  }
