...
<script lang="ts">
  ...
  let canPause = $derived(isPlaying || isLoading);
    ...
    <div class="assistant-speech-primary-controls" class:paused={isPaused || isFailed || isBlocked}>
      {#key canPause}
        <button
          ...
          aria-label={isFailed ? $text('common.retry') : isBlocked ? $text('chat.assistant_speech.continue_playback') : canPause ? 'Pause voice response' : 'Play voice response'}
          onclick={() => canPause ? controller.pause() : isBlocked ? void controller.continueAfterUserGesture() : void controller.play()}
        >
          <span
            class="assistant-speech-icon"
            class:pause={canPause}
            class:play={!canPause}
            data-testid="assistant-speech-primary-icon"
            data-icon={canPause ? 'pause' : 'play'}
          ></span>
        </button>
      {/key}
