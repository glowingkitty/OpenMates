...
export async function handleSend(
  ...
  // =========================================================================
  // UPLOADED PDF EMBED REGISTRATION
  // For each 'pdf' embed that has been successfully uploaded (uploadEmbedId
  // present), ensure it is registered in EmbedStore with the current session's
  // encryption key. This is critical for deduplicated PDFs: the server reuses
  // an old embed_id, and the EmbedStore may contain a stale version encrypted
  // with a previous session's key — causing decryption failures that block
  // message sending. _performPdfUpload already stores the embed, but this
  // is a safety net for edge cases (old drafts, race conditions).
  // =========================================================================
  try {
    const { encode: toonEncodePdf } = await import("@toon-format/toon");
    const { embedStore: pdfEmbedStore } =
      await import("../../../services/embedStore");

    interface UploadedPdfNode {
      attrs: Record<string, unknown>;
    }
    const uploadedPdfNodes: UploadedPdfNode[] = [];
    editor.view.state.doc.descendants((node) => {
      if (
        node.type.name === "embed" &&
        node.attrs.type === "pdf" &&
        (node.attrs.status === "finished" || node.attrs.status === "processing") &&
        node.attrs.uploadEmbedId
      ) {
        uploadedPdfNodes.push({ attrs: { ...node.attrs } });
      }
      return true;
    });

    for (const { attrs } of uploadedPdfNodes) {
      const pdfUploadEmbedId = attrs.uploadEmbedId as string;

      const embedContent = {
        app_id: "pdf",
        skill_id: "read",
        type: "pdf",
        status: attrs.status || "processing",
        filename: attrs.filename || null,
      ...
      let toonContent: string;
      try {
        toonContent = toonEncodePdf(embedContent);
      } catch {
        toonContent = JSON.stringify(embedContent);
      }

      const now = Date.now();
      const textPreview = (attrs.filename as string) || "PDF";

      await pdfEmbedStore.put(
        `embed:${pdfUploadEmbedId}`,
        {
          embed_id: pdfUploadEmbedId,
          type: "pdf-read",
          status: (attrs.status as string) || "processing",
          content: toonContent,
          text_preview: textPreview,
          createdAt: now,
          updatedAt: now,
      ...
      console.debug(
        `[handleSend] Registered uploaded PDF embed ${pdfUploadEmbedId} in EmbedStore`,
      );

      // Update the embed node's contentRef if not already set
      if (!attrs.contentRef) {
        const { state: pdfState, dispatch: pdfDispatch } = editor.view;
        const pdfTr = pdfState.tr;
        pdfState.doc.descendants((node, nodePos) => {
          if (
            node.type.name === "embed" &&
            node.attrs.uploadEmbedId === pdfUploadEmbedId
          ) {
            pdfTr.setNodeMarkup(nodePos, undefined, {
              ...node.attrs,
              contentRef: `embed:${pdfUploadEmbedId}`,
            });
            return false;
          }
          return true;
  ...
  } catch (pdfEmbedRegError) {
    console.error(
      "[handleSend] Error registering PDF embeds:",
      pdfEmbedRegError,
    );
    throw pdfEmbedRegError;
  }
  embedRegSpan.end();
