...
export async function handleSend(
  ...
  console.log(
    "[handleSend] SUGGESTION DEBUG 1: consumeClickedSuggestion result:",
    {
      hasValue: !!encryptedSuggestionToDelete,
      value: encryptedSuggestionToDelete
        ? `${encryptedSuggestionToDelete.substring(0, 20)}...`
        : null,
    },
  );
  ...
  if (encryptedSuggestionToDelete) {
    console.debug(
      "[handleSend] New chat suggestion was used, will delete from client and server:",
    ...
    try {
      ...
      console.log(
        "[handleSend] SUGGESTION DEBUG 2: Getting all suggestions from DB...",
      );
      const allSuggestions = await chatDB.getAllNewChatSuggestions();
      console.log(
        "[handleSend] SUGGESTION DEBUG 3: Found total suggestions in DB:",
        {
      ...
      console.log(
        "[handleSend] SUGGESTION DEBUG 4: Search result for encrypted match:",
        {
          found: !!suggestionToDelete,
          suggestionId: suggestionToDelete?.id,
          matchedEncrypted: suggestionToDelete
            ? `${suggestionToDelete.encrypted_suggestion.substring(0, 20)}...`
            : null,
        },
      );
      ...
      if (suggestionToDelete) {
        console.log(
          "[handleSend] SUGGESTION DEBUG 5: Found suggestion to delete, will delete by ID...",
        );

        // Delete directly by ID instead of re-encrypting text
        // This avoids encryption mismatch issues
        const deleteResult = await chatDB.deleteNewChatSuggestionById(
        ...
        if (deleteResult) {
          ...
          console.log(
            "[handleSend] SUGGESTION DEBUG 9: Verification after deletion:",
            {
              totalAfterDelete: suggestionAfterDelete.length,
              stillExists: suggestionAfterDelete.some(
        ...
        console.warn(
          "[handleSend] SUGGESTION DEBUG 4B: Suggestion NOT found in DB!",
          {
            searchedFor: `${encryptedSuggestionToDelete.substring(0, 20)}...`,
            dbHas: allSuggestions.map((s) =>
              s.encrypted_suggestion.substring(0, 20),
            ),
          },
        );
      }
      ...
      console.error(
        "[handleSend] SUGGESTION DEBUG ERROR: Failed to delete new chat suggestion from local IndexedDB:",
        {
    ...
    console.log(
      "[handleSend] SUGGESTION DEBUG 1B: No suggestion was tracked for deletion (encryptedSuggestionToDelete is null/undefined)",
    );
  }
