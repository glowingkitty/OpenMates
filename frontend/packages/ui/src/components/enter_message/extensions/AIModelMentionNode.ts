...
/**
 * Formats a display name for reliable parsing.
 * Replaces spaces with hyphens so "@Claude Haiku 4.5" becomes "@Claude-Haiku-4.5".
 * This ensures the mention can be reliably parsed when serialized to backend format.
 */
...
export const AIModelMentionNode = Node.create<AIModelMentionNodeOptions>({
  ...
  // Uses hyphens instead of spaces for reliable parsing
  renderText({ node }) {
    return `@${formatDisplayName(node.attrs.displayName)}`;
  },
  ...
  renderHTML({ HTMLAttributes }) {
    return [
      ...
      `@${formatDisplayName(HTMLAttributes.displayName)}`,
    ];
  },
