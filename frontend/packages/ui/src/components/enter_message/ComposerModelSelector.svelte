...
<style>
    ...
    .selector-icon > img, .menu-icon, .model-icon > img { width: 1.75rem; height: 1.75rem; box-sizing: border-box; padding: 0.25rem; object-fit: contain; border-radius: var(--radius-2); background: var(--color-white-fixed, #fff); }
    .selector-icon .icon_ai, .menu-icon .icon_ai { width: 100%; height: 100%; }
    .model-selector-label { min-width: 0; max-width: 12rem; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-weight: 600; }
    .model-selector-menu { position: absolute; z-index: var(--z-index-dropdown); bottom: calc(100% + var(--spacing-4)); left: 0; width: min(22rem, calc(100vw - 2rem)); max-height: 22rem; overflow-x: hidden; overflow-y: auto; padding: var(--spacing-2); background: var(--color-grey-0); border-radius: var(--radius-8); box-shadow: var(--shadow-lg); }
