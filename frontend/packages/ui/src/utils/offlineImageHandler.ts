/**
 * Offline-aware image loading utility.
 *
...
/**
 * Handle an image load error with offline awareness.
 * Call this from an image's `onerror` handler instead of simply hiding the image.
 *
 * Behaviour:
 * - If online: hides the image (same as current behaviour — server genuinely can't serve it)
 * - If offline: shows a placeholder and registers for retry on reconnect
 *
 * @param img - The HTMLImageElement that failed to load
 * @param originalSrc - The original src URL to retry (optional, defaults to img.src)
 */
...
function retryAllPendingImages(): void {
  ...
  for (const img of images) {
    ...
    // the component's onerror handler will call handleImageError again
  }
