...
export async function loadCommunityDemos(
  ...
  if (forceLanguageReload && demoReloadAbortController) {
    console.debug(
      `${LOG_PREFIX} Aborting previous demo reload in favor of new language reload`,
    );
  ...
  try {
    ...
    if (abortController.signal.aborted) {
      console.debug(
        `${LOG_PREFIX} Demo reload aborted (superseded by newer reload)`,
      );
      return;
    }
    ...
    console.debug(
      `${LOG_PREFIX} Loading community demos for language: ${currentLang}`,
    );

    if (forceLanguageReload) {
      console.debug(
        `${LOG_PREFIX} Language changed - clearing cache and reloading demos`,
      );
      try {
    ...
    if (!forceLanguageReload && !communityDemoStore.isCacheLoaded()) {
      console.debug(
        `${LOG_PREFIX} Loading community demos from IndexedDB cache...`,
      );
    ...
    if (abortController.signal.aborted) {
      console.debug(
        `${LOG_PREFIX} Demo reload aborted before fetch (superseded by newer reload)`,
      );
      return;
    }
    ...
    if (!response.ok) {
      ...
      console.warn(
        `${LOG_PREFIX} Failed to fetch demo chats list:`,
        response.status,
      );
      communityDemoStore.markAsLoaded();
      return;
    ...
    if (demoChatsList.length === 0) {
      console.debug(
        `${LOG_PREFIX} No community demo chats available from server`,
      );
      communityDemoStore.markAsLoaded();
      return;
    }
    ...
    for (const demoChatMeta of demosToFetch) {
      ...
      try {
        ...
        if (!chatDataObj || !chatDataObj.chat_id) {
          console.warn(
            `${LOG_PREFIX} Invalid community demo chat data for ${demoId}`,
          );
          continue;
        }
        ...
        const parsedEmbeds = rawEmbeds.map(
          (emb: {
            ...
            embed_id:
              emb.embed_id || `${demoId}-embed-${rawEmbeds.indexOf(emb)}`,
            chat_id: chatId,
        ...
        console.error(
          `${LOG_PREFIX} Error loading community demo ${demoId}:`,
          error,
        );
      }
    }
    ...
    console.error(
      `${LOG_PREFIX} Error loading community demo chats from server:`,
      error,
    );
    communityDemoStore.markAsLoaded();
  }
