...
/**
 * Group consecutive website embeds into website-preview-group containers
...
export function groupConsecutiveWebsiteEmbeds(embedNodes: EmbedNodeAttributes[]): EmbedNodeAttributes[] {
  ...
  for (let i = 0; i < embedNodes.length; i++) {
    ...
    if (embed.type === 'web' || embed.type === 'video') {
...
/**
 * Flush a group of website embeds, creating a group if there are multiple items
