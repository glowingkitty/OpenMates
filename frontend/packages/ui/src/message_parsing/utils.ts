...
export const EMBED_PATTERNS = {
  ...
  // Filename comment pattern: <!-- filename: "Name.docx" -->
  FILENAME_COMMENT: /^<!--\s*filename:\s*["'](.+?)["']\s*-->$/,
  ...
  YOUTUBE_URL:
    /(?:https?:\/\/)?(?:www\.|m\.)?(?:youtube\.com\/(?:watch\?v=|embed\/|shorts\/|v\/)|youtu\.be\/)([a-zA-Z0-9_-]{11})/g,
...
export function classifyCodeFenceLine(line: string): CodeFenceClassification {
  ...
  if (afterBackticks.length > 0 && !afterBackticks.includes(" ")) {
    return {
      ...
      filename: afterBackticks.includes(":")
        ? afterBackticks.split(":").slice(1).join(":")
        : undefined,
    };
  }
