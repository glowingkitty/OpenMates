...
/**
 * Static import map for locale files - this ensures Vite can statically analyze
 * all imports at build time. We use a static object with all possible locale imports,
 * then filter it based on LANGUAGE_CODES from languages.json at runtime.
 * 
 ...
 * to import them.
 * 
 * We create a static object with all locale imports so Vite can see them all at build time.
 * Then we filter it based on LANGUAGE_CODES from languages.json to use only supported languages.
 */
// Static import map with ALL possible locale imports - Vite can statically analyze these
// This ensures all imports are bundled even if not all languages are in LANGUAGE_CODES
const allLocaleImports: Record<string, () => Promise<any>> = {
 ...
 * This ensures we only use languages that are in languages.json while still
 * having all imports statically analyzable by Vite
 */
function createLocaleImportMap(): Record<string, () => Promise<any>> {
    const map: Record<string, () => Promise<any>> = {};
    
    // Always ensure 'en' is in the map as a fallback
    if (allLocaleImports['en']) {
        map['en'] = allLocaleImports['en'];
    } else {
        // Fallback if 'en' is somehow missing
        map['en'] = () => import('../i18n/locales/en.json');
    }
    ...
    // Filter allLocaleImports to only include languages from LANGUAGE_CODES
    for (const langCode of LANGUAGE_CODES) {
        if (allLocaleImports[langCode]) {
            map[langCode] = allLocaleImports[langCode];
        } else {
            console.warn(`Locale import not found for language code: ${langCode}`);
        }
    }
