...
import { createApiKeyCryptoMaterial, encryptBytesWithAesGcm } from "../src/crypto.ts";
...
describe("OpenMates npm SDK cleartext boundary", () => {
  // contract-test: direct surface=sdks.npm assertions=plans.content.client-encrypted,plans.project-links.encrypted
  it("accepts cleartext asks and sends encrypted durable storage payloads", async () => {
    const masterKey = Buffer.alloc(32, 11);
    const projectKey = Buffer.alloc(32, 12);
    ...
    await withServer(
      (request, body) => {
        ...
        if (request.url === "/v1/user-plans/ask") {
          ...
          assert.deepEqual(body.encrypted_create.linked_project_ids, [PROJECT_ID]);
      ...
      async (apiUrl) => {
        ...
        const plan = await client.plans.ask(`create ${CLEAR_PUBLIC_PLAN}`, { projectIds: [PROJECT_ID] });
