...
test("another worker persists its intent while an earlier request is in flight", async () => {
  const directory = mkdtempSync(join(tmpdir(), "mutation-concurrent-"));
  let releaseFirst!: () => void;
  let started!: () => void;
  const firstStarted = new Promise<void>(resolve => { started = resolve; });
  const waiting = new Promise<void>(resolve => { releaseFirst = resolve; });
  const writes: string[] = [];
  const client = {createUserTask: async (input: any) => {
    writes.push(input.task_id);
    if (input.task_id === "first") { started(); await waiting; }
    return {...input, status: "todo"};
  }} as unknown as OpenMatesClient;
  const store=taskMutationStore(["api","account","personal"],directory);
  const operation=(taskId: string): TaskMutation=>({kind:"create",taskId,input:{task_id:taskId,version:1,encrypted_title:"cipher-"+taskId} as never});
  try {
    const first=store.deliver("b".repeat(64),async()=>operation("first"),client);
    await firstStarted;
    await assert.rejects(store.deliver("c".repeat(64),async()=>operation("second"),client),TaskDeliveryPending);
    releaseFirst(); await first;
    await store.flush(client);
    assert.deepEqual(writes,["first","second"]);
    await store.deliver("c".repeat(64),async()=>{throw new Error("must retain the second worker intent");},client);
    assert.deepEqual(writes,["first","second"]);
  } finally { releaseFirst?.(); rmSync(directory,{recursive:true,force:true}); }
});
