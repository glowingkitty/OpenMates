...
OpenMates shows the short product line, for example `v0.19`, in the web app.
The npm package uses exact artifact versions:

- `0.19.0-alpha.N` is a prerelease from the `dev` branch published under the
  `alpha` npm tag.
- `0.19.0` is a stable release from `main` published under the `latest` npm tag.
