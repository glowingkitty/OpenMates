...
import { TaskDeliveryPending, TaskDeliveryRejected, withTaskDeliveryAdmission, lockTaskDeliveryRecord } from "./taskDelivery.js";
    ...
    async flush(client: OpenMatesClient, onNotice?: (message: string) => void) {
      ...
      try {
        ...
        for (const file of files) {
          ...
          try {
            const unlock = await lockTaskDeliveryRecord(directory, file.slice(0, -5));
            try { await send(path, JSON.parse(readFileSync(path, "utf8")), client); }
