...
export function reconcileMutation(mutation: TaskMutation, current: UserTaskRecord | null): "accepted" | "retry" {
  ...
  if (mutation.kind === "update" && mutation.ownerHash && current.external_chat_lookup_hash !== mutation.ownerHash) {
    ...
    async deliver(id: string, build: () => Promise<TaskMutation>, client: OpenMatesClient) {
      ...
      try {
        ...
        try { record = JSON.parse(readFileSync(path, "utf8")); }
          ...
          if (record.mutation.kind === "delete" && scope[2] !== "personal") throw new TaskDeliveryRejected("Team deletion is not supported by the current Task deletion API.");
          save(path, record);
        }
