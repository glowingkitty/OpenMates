...
export class ProjectTaskCache {
  ...
  constructor(root: string, accountScope: string, projectId: string) {
    ...
    try {
      ...
      this.state = { ...state, account_scope: accountScope.startsWith("[") ? JSON.parse(accountScope) : undefined };
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code !== "ENOENT") throw error;
      this.state = { schema_version: 1, account_scope: accountScope.startsWith("[") ? JSON.parse(accountScope) : undefined, project_id: projectId, cursor: "", synced_at: "", connection: "reconnecting", tasks: [] };
    }
  }
...
export async function decryptProjectTask(record: ProjectTaskFrame["tasks"][number], masterKey: Uint8Array, projectKey: Uint8Array, projectId: string): Promise<CachedTask> {
  ...
  return {
    task_id: task.taskId, encrypted: readable,
    assignee_type: task.assigneeType, assignee_identity: task.assigneeIdentity,
    ai_execution_state: task.aiExecutionState, queue_state: task.queueState,
    short_id: task.shortId, title: task.title,
