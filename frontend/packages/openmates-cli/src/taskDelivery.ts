...
export function failureDisposition(error: unknown, state: RetryState | null, now: number, random = Math.random): RetryState {
  ...
  if ([400, 401, 403, 404, 409, 410, 422].includes(status)) {
    throw new TaskDeliveryRejected((error as { ownerConflict?: string })?.ownerConflict ?? `Task delivery requires action (HTTP ${status}); the operation was retained.`);
  }
...
/** Serialize the same intent while allowing other workers to persist new ones. */
export async function lockTaskDeliveryRecord(directory: string, id: string): Promise<() => Promise<void>> {
  if (!/^[a-f0-9]{64}$/.test(id)) throw new Error("Invalid Task delivery identity");
  const target = join(directory, "locks", id);
  mkdirSync(target, {recursive: true, mode: 0o700});
  try { return await lockfile.lock(target, {retries: 0, stale: 240000}); }
  catch (error) {
    if ((error as {code?: string}).code !== "ELOCKED") throw error;
    throw new TaskDeliveryPending(Date.now() + 5000);
  }
}
