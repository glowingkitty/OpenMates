...
export class SecretRegistry {
  ...
  /**
   * Add a known original → placeholder pair generated by another owner-local
   * redaction layer. Unlike add(), this preserves the supplied token exactly.
   */
  addKnownMapping(mapping: KnownSecretMapping): void {
    if (!mapping.original || !mapping.placeholder) return;

    const regEntry: SecretRegistryEntry = {
      value: mapping.original,
      token: mapping.placeholder,
      source: mapping.source ?? "custom",
      name: mapping.placeholder,
      type: mapping.type,
      length: mapping.original.length,
    };

    this.entries.set(mapping.original, regEntry);
    this.usedPlaceholders.set(mapping.placeholder, mapping.original);
    this.automaton = null;
  }
