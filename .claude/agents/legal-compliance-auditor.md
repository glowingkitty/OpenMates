...
- `COOKIES_YAML` — contents of `docs/architecture/compliance/cookies.yml` (**cookies only**, auto-generated by the E2E suite via the cookie-audit Playwright fixture). This is the **empirical evidence** that backs the cookie-banner exemption claim. If empty or missing, treat that as a Top 10 finding ("no runtime evidence — run the Playwright suite"). If populated, verify each entry has `consent_exempt: true` AND an `exemption_basis` that maps to "strictly necessary" under ePrivacy Art. 5(3) / TTDSG §25; flag any non-essential third-party cookie or any entry missing human-maintained fields.
- `BROWSER_STORAGE_YAML` — contents of `docs/architecture/compliance/browser-storage.yml` (localStorage / sessionStorage / IndexedDB **key names only** — no values). **Informational only — does NOT affect the cookie-banner decision.** Use it to spot engineering/privacy concerns: flag any key whose name suggests analytics, tracking, or fingerprinting (e.g. `_ga`, `mp_*`, `amplitude_*`, `_fbp`, `mixpanel.*`, `ajs_*`), and flag keys with missing `purpose` for human documentation.
