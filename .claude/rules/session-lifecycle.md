...
- **Use session worktrees for edits:** source edits belong in the automatic worktree from `python3 scripts/sessions.py worktree ensure --session <ID>`. The root checkout is the control plane for orchestration, diagnostics, and the short `dev` integration window. OpenCode blocks root-checkout source edits by default; set `OPENMATES_ROOT_GUARD=off` only for an explicit manual emergency.
