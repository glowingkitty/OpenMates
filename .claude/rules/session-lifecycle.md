...
- **Demonstration review is a completion gate:** For specs with `demonstration.eligibility.status: required`, continue after normal green checks through caption rendering, local privacy scanning, frame-only review, and publication attempt. A verification deploy may happen before this gate when it is needed to produce the real Playwright recording. Never put the full video in model context; Discord `publication_pending` does not reopen passed implementation evidence.
