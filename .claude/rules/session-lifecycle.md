...
- **OpenCode Web restarts are Zellij-only:** use the existing `code` Zellij session and its `scripts/start-opencode-server.sh` workflow. Do not launch or restart `opencode web` through systemd, `systemd-run`, `nohup`, detached shells, tmux, or another terminal session.
