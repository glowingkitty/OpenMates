...
3. **Run spec conformance when applicable:**
   ...
   - A scoped verification deploy may precede final conformance when live Playwright evidence is required. After that run, complete exact captions, local privacy scanning, frame-only demonstration review, and OpenCode response-media embedding before final completion.
