...
import { readProjectChanges, unlinkDeletedExternalChat } from './operations.js';
...
export default {
  ...
  handler(router, { database, env, logger }) {
    ...
    router.post('/external-chat-deleted', async (req, res) => {
      if (!authorized(req.headers, env.INTERNAL_API_SHARED_TOKEN)) return res.status(401).json({error: 'internal_auth_failed'});
      try { return res.json({data: await unlinkDeletedExternalChat(database, req.body)}); }
      catch (error) {
        logger.warn({code: 'task_chat_deletion_deferred'}, 'Task chat deletion reconciliation deferred');
        return res.status(error.message === 'invalid_sync_scope' ? 400 : 503).json({error: 'task_chat_deletion_deferred'});
      }
    });
