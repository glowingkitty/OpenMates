...
# Usage record retention is NOT handled by a Celery task.
#   - Directus rows: the existing monthly usage archive task
#     (usage.archive_old_entries) moves rows older than 3 months to S3, keeping
#     Directus lean. Nothing ever ages Directus rows beyond that.
#   - S3 archive objects: retention is enforced by the S3 bucket lifecycle
#     policy on `usage_archives` (3 years). See s3/config.py.
# A previous per-user usage auto-delete task was removed because its S3
# DeleteObject loop triggered bucket-wide throttling and stalled the API
# event loop (sync boto3 in async context).
