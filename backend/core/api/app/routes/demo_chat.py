...
class CreateDemoChatRequest(BaseModel):
    """Request model for creating a demo chat from a shared chat"""
    demo_id: str  # The demo chat ID generated by admin
    chat_id: str
    encryption_key: str
    title: str
...
@limiter.limit("5/hour")  # Strict rate limit since this is admin functionality
async def create_demo_chat(
    request: Request,
    payload: CreateDemoChatRequest,
    current_user: User = Depends(get_current_user),
    directus_service: DirectusService = Depends(get_directus_service)
) -> Dict[str, Any]:
    """
    Create a demo chat from a shared chat.

    This endpoint is called when an admin decides to create a demo chat from
    a community-shared chat. It stores the chat ID and encryption key so that
    non-authenticated users can view it.

    Requires admin authentication.
    """
    try:
        # TODO: Add admin role check here
        # For now, allow any authenticated user to create demo chats
        # In production, this should check if the user has admin privileges

        # Verify the original chat exists and is shared
        original_chat = await directus_service.chat.get_chat_metadata(payload.chat_id)
        if not original_chat:
            raise HTTPException(status_code=404, detail="Original chat not found")

        if original_chat.get("is_private", True):
            raise HTTPException(status_code=400, detail="Chat is not shared publicly")
        ...
        demo_chat = await directus_service.demo_chat.create_demo_chat(
            ...
            approved_by_admin=True  # Auto-approve for now
        )

        if not demo_chat:
            raise HTTPException(status_code=500, detail="Failed to create demo chat")

        logger.info(f"Created demo chat {payload.demo_id} from chat {payload.chat_id} by user {current_user.id}")
        ...
        logger.error(f"Error creating demo chat: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail="Failed to create demo chat")

@router.post("/chat/approve")
@limiter.limit("10/hour")  # Rate limit for approval actions
async def approve_demo_chat(
    ...
    try:
        ...
        success = await directus_service.demo_chat.approve_demo_chat(payload.demo_id)

        if not success:
            raise HTTPException(status_code=404, detail="Demo chat not found or already approved")

        logger.info(f"Approved demo chat {payload.demo_id} by user {current_user.id}")
