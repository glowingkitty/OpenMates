...
class CreditNoteTemplateService(BasePDFTemplateService):
    ...
    def generate_credit_note(self, credit_note_data, lang="en", currency="eur"):
        ...
        # Get the unused credits amount and total credits
        unused_credits = credit_note_data.get('unused_credits', 0)
        total_credits = credit_note_data.get('total_credits', 0)
        
        # Format the credits for display
        formatted_unused_credits = format_credits(unused_credits)
        formatted_total_credits = format_credits(total_credits)
        
        # Get the unit price for the unused credits
        ...
        # Store the calculated values in the credit note data
        credit_note_data['unit_price'] = unit_price
        credit_note_data['refund_amount'] = refund_amount
        ...
        # Add left indent to credit_note_text and adjust widths to match inner tables
        header_table = Table([[Spacer(self.left_indent, 0), credit_note_text, openmates_text]], 
        ...
        # Fix credit note details alignment to match other elements
        # Calculate dynamic width based on text length
        credit_note_number_text = self.t["invoices_and_credit_notes"]["credit_note_number"]["text"] + ":"
        date_issue_text = self.t["invoices_and_credit_notes"]["date_of_issue"]["text"] + ":"
        referenced_invoice_text = self.t["invoices_and_credit_notes"]["referenced_invoice"]["text"] + ":"
        
        # Estimate appropriate column width based on longest text plus some padding
        label_texts = [credit_note_number_text, date_issue_text, referenced_invoice_text]
        ...
        credit_note_table = Table([
            [Spacer(self.left_indent, 0), Paragraph(credit_note_number_text, self.styles['Normal']), 
             Paragraph(credit_note_data['credit_note_number'], self.styles['Normal'])],
            [Spacer(self.left_indent, 0), Paragraph(date_issue_text, self.styles['Normal']), 
             Paragraph(format_date_for_locale(credit_note_data['date_of_issue'], self.current_lang), self.styles['Normal'])],
            [Spacer(self.left_indent, 0), Paragraph(referenced_invoice_text, self.styles['Normal']), 
             Paragraph(credit_note_data['referenced_invoice'], self.styles['Normal'])]
        ], colWidths=[self.left_indent, label_col_width, doc.width-self.left_indent-label_col_width])
        
        credit_note_table.setStyle(TableStyle([
            ('ALIGN', (1, 0), (1, -1), 'LEFT'),
            ('ALIGN', (2, 0), (2, -1), 'LEFT'),
        ...
        if credit_note_data.get('receiver_country'):
            translated_receiver_country = self._get_translated_country_name(credit_note_data['receiver_country'])
        ...
        if credit_note_data.get('receiver_name'):
            receiver_fields.append(credit_note_data['receiver_name'])
            
        # Add address if present
        if credit_note_data.get('receiver_address'):
            receiver_fields.append(credit_note_data['receiver_address'])
            
        # Add city if present
        if credit_note_data.get('receiver_city'):
            receiver_fields.append(credit_note_data['receiver_city'])
        ...
        if credit_note_data.get('receiver_email'):
            receiver_fields.append(credit_note_data['receiver_email'])
            
        # Add VAT if present
        if credit_note_data.get('receiver_vat'):
            receiver_fields.append(f"{self.t['invoices_and_credit_notes']['vat']['text']}: {credit_note_data['receiver_vat']}")
        ...
        # Format the credits text using the translation with both unused and total credits
        refunded_credits_text = sanitize_html_for_reportlab(
            self.t["invoices_and_credit_notes"]["refunded_credits_item"]["text"]
                .replace("{amount_unused_credits}", formatted_unused_credits)
        ...
        totals_inner_table.setStyle(TableStyle([
            ...
            ('LINEABOVE', (0, 2), (-1, 2), 1, self.separator_color), # Line above total refunded row only
            ('LINEBELOW', (0, 2), (-1, 2), 1, self.separator_color), # Line below total refunded row
            ('LEFTPADDING', (0, 0), (-1, -1), 0),
            ('RIGHTPADDING', (0, 0), (-1, -1), 0),
        ]))
