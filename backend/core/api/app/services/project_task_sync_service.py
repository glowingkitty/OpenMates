...
class ProjectTaskSyncService:
    ...
    async def _authorize(self, subscription: Subscription) -> bool:
        if subscription.team_id:
            await self.directus.team.require_team_role(
                subscription.team_id, subscription.user_id, TEAM_ROLES
            )
    ...
    async def subscribe(self, websocket: Any, user_id: str, payload: dict) -> None:
        ...
        if team_id is not None and (
            not isinstance(team_id, str) or not team_id or len(team_id) > 128
        ):
        ...
        for binding in bindings:
            ...
            for field in ("project_id", "source_id"):
                if (
                    ...
                    or not 0 < len(binding[field]) <= 128
                ):
                    raise ValueError("invalid_task_sync_binding")
            cursor = binding.get("cursor")
            if cursor is not None and (
                not isinstance(cursor, str) or len(cursor) > 128
            ):
                raise ValueError("invalid_task_sync_cursor")
            item = Subscription(
                user_id, team_id, binding["source_id"], binding["project_id"], cursor
            )
        ...
        self.scope_sockets.setdefault(
            scope_key(user_id, team_id), weakref.WeakSet()
        ).add(websocket)
    ...
    async def unlink_deleted_external_chat(
        self, user_id: str, lookup_hash: str, event_id: str, team_id: str | None = None
    ) -> dict:
        ...
        response = await self.directus._make_api_request(
            "POST",
            f"{self.directus.base_url.rstrip('/')}/project-task-sync/external-chat-deleted",
            headers={"X-Internal-Service-Token": token},
            json={
                "scope": scope_key(user_id, team_id),
                "provider": "codex",
        ...
        if response.status_code != 200:
            raise RuntimeError(
                "Task chat deletion reconciliation temporarily unavailable"
            )
    ...
    async def _read(self, item: Subscription) -> dict:
        ...
        response = await self.directus._make_api_request(
            "POST",
            f"{self.directus.base_url.rstrip('/')}/project-task-sync",
            headers={"X-Internal-Service-Token": token},
            json={
                "scope": scope_key(item.user_id, item.team_id),
        ...
        if (
            ...
            or not isinstance(result.get("cursor"), str)
        ):
    ...
    async def _deliver(self, websocket: Any, item: Subscription) -> None:
        try:
            while item.dirty:
                ...
                count = max(
                    1, (len(records) + RECORDS_PER_FRAME - 1) // RECORDS_PER_FRAME
                )
                for index in range(count):
                    await websocket.send_json(
                        {
                            "type": "project_task_sync",
                            "payload": {
                                ...
                                "tasks": records[
                                    index * RECORDS_PER_FRAME : (index + 1)
                                    * RECORDS_PER_FRAME
                                ],
                                "removed_task_ids": result.get("removed_task_ids", [])
                                if index == 0
                                else [],
                            },
                        }
            ...
            permanent = (
                ...
                or type(exc).__name__ == "TeamPermissionError"
            )
            logger.warning(
                "Project Task sync delivery deferred: %s", type(exc).__name__
            )
            if permanent:
    ...
    async def listen(self) -> None:
        while True:
            try:
                async for event in self.cache.subscribe_to_channel(
                    "project_task_sync:*"
                ):
                ...
                logger.warning(
                    "Project Task sync listener reconnecting: %s", type(exc).__name__
                )
