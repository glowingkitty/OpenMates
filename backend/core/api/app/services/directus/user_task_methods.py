...
class UserTaskMethods:
    ...
    async def _update_task_unlocked(
        ...
        *,
        ...
        if previous_owner and not next_owner:
            update["queue_state"] = "none"
            update["ai_execution_state"] = None
            if update.get("status", existing.get("status")) == "in_progress":
                update["status"] = "todo"
        ...
        # that chat wrapper while preserving master/team/project access.
        relinks_context = (next_chat_hash is not None and next_chat_hash != existing_chat_hash) or next_project_hashes != existing_project_hashes
