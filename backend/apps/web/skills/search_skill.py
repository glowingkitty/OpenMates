...
def _allowlisted_object(value: Any, *fields: str) -> Optional[Dict[str, Any]]:
    """Preserve only UI metadata fields that cannot carry provider-owned extras."""
    if not isinstance(value, dict):
        return None
    allowed = {field: value[field] for field in fields if value.get(field) is not None}
...
class SearchSkill(BaseSkill):
    ...
    async def _process_single_search_request(
        ...
        try:
            ...
            task_id = f"search_{request_id}_{search_query[:50]}"
            for result in results:
                ...
                previews.append({
                    ...
                    "profile": _allowlisted_object(result.get("profile"), "name"),
                    "meta_url": _allowlisted_object(result.get("meta_url"), "favicon"),
                    "thumbnail": _allowlisted_object(result.get("thumbnail"), "src", "original"),
        ...
        **kwargs
        ...
        sanitized = await sanitize_app_skill_output(
            ...
            AppSkillOutputSafetyContext(
                ...
                secrets_manager=secrets_manager,
                cache_service=cache_service,
                log_prefix="[WebSearch direct execute] ",
            ),
        )
        return SearchResponse.model_validate(sanitized)
