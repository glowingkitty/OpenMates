...
MINDMAP_INTENT_PATTERN = re.compile(
...
def _contains_mindmap_intent_in_user_history(message_history: List[AIHistoryMessage]) -> bool:
    """Return True when user text explicitly asks for an OpenMates mind map."""
    for message in message_history:
        role = message.role if hasattr(message, "role") else (message.get("role") if isinstance(message, dict) else None)
        if role != USER_ROLE:
            continue
...
async def handle_preprocessing(
    ...
    if not user_requested_skills_only:
        ...
        # --- mindmaps direct embed ---
        # Mind Maps are generated by app instructions as fenced OpenMates JSON,
        # not by image generation. Suppress only conflicting image-generation
        # tools when explicit Mind Maps wording is present.
        if _contains_mindmap_intent_in_user_history(request_data.message_history):
            conflicting_skills = [
                skill
                for skill in validated_relevant_skills
                if skill in MINDMAP_CONFLICTING_GENERATIVE_SKILLS
            ]
            if conflicting_skills:
                validated_relevant_skills = [
                    skill
                    for skill in validated_relevant_skills
                    if skill not in MINDMAP_CONFLICTING_GENERATIVE_SKILLS
                ]
                logger.info(
                    f"{log_prefix} [RULE_BASED] Removed {conflicting_skills} from preselected skills: "
                    "detected explicit Mind Maps intent. "
                    "(Mind Maps direct embeds are created from openmates_mindmap fences, not image generation.)"
                )
            else:
                logger.debug(
                    f"{log_prefix} [RULE_BASED] Mind Maps intent detected; no conflicting image generation skills were preselected."
                )
