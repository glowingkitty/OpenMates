...
async def check_rate_limit(
    ...
    try:
        ...
        if not client:
            logger.warning("Cache client not available for rate limit check, allowing request")
            return (True, None)
...
async def wait_for_rate_limit(
    ...
    while True:
        ...
        if retry_after < CELERY_THRESHOLD:
            logger.debug(
                f"Rate limit active for provider '{provider_id}', skill '{skill_id}'. "
                f"Waiting {retry_after:.2f}s before retrying check."
            )
            await asyncio.sleep(retry_after)
            continue

        # Long wait: Use Celery scheduling if available, otherwise fall back to direct wait loop
        if celery_producer and celery_task_context:
            ...
            if app_id and skill_id_for_celery and arguments is not None:
                try:
                    ...
                    task_kwargs = {"arguments": arguments}
                    if chat_id:
                    ...
                    raise RateLimitScheduledException(
                        ...
                        message="Skill execution scheduled via Celery due to rate limit",
                    )
                except Exception as e:
                    logger.warning(
        ...
        logger.info(
            f"Rate limit active for provider '{provider_id}', skill '{skill_id}'. "
            f"Long wait ({retry_after:.2f}s), waiting and rechecking."
        )
        await asyncio.sleep(retry_after)
