...
class SearchResponse(BaseModel):
    ...
    results: List[Dict[str, Any]] = Field(
        ...
        description="List of place search results. These will be flattened and encoded to TOON format by main_processor."
    )
    provider: str = Field(
        default="Google Maps",
...
class SearchSkill(BaseSkill):
    ...
    async def execute(
        ...
        for i, req in enumerate(search_requests):
            ...
            req_page_size = req.get("pageSize", 20)  # Default to maximum (20 results per request)
            req_language_code = req.get("languageCode", "en")  # Default from schema
            req_location_bias = req.get("locationBias")  # Optional
            req_included_type = req.get("includedType")  # Optional
            req_min_rating = req.get("minRating")  # Optional
            req_open_now = req.get("openNow", False)  # Default from schema
            req_include_reviews = req.get("includeReviews", False)  # Default to False to keep response size manageable
            
            logger.debug(f"Executing search {i+1}/{len(search_requests)}: query='{search_query}', page_size={req_page_size}")
            ...
            try:
                ...
                provider_id = "google_maps"  # Google Maps provider
                skill_id = "search"
                ...
                search_result = await search_places(
                    ...
                    include_reviews=req_include_reviews  # Pass include_reviews parameter
                )
                ...
                # (unlike web search results which may contain user-generated content)
                # We now include Enterprise + Atmosphere tier fields (description, summaries, etc.)
                # at no extra cost since Enterprise and Enterprise + Atmosphere both cost $0.032
                places = search_result.get("results", [])
                
                for place in places:
                    # Build preview with place data
                    # Includes all available fields from Google Places API
                    # Structure is flattened for efficiency (open_now at root, single phone_number field)
                    preview = {
                        "type": "place_result",
                        ...
                        # Contact information - single phone number (prefers international format)
                        "phone_number": place.get("phone_number"),
                        # Pricing information
                        "price_level": place.get("price_level"),
                        "price_range": place.get("price_range"),  # Will be ignored for LLM inference
                        # Opening hours - flattened to root level
                        "opening_hours": place.get("opening_hours"),  # Array of weekday descriptions (e.g., "Monday: 11:30 AM – 10:00 PM")
                        "open_now": place.get("open_now"),  # Boolean indicating if place is currently open
                        "next_close_time": place.get("next_close_time"),  # ISO timestamp of next closing time
                        # Business status
                        "business_status": place.get("business_status"),
                        # Descriptions and summaries
                        "description": place.get("description"),  # Editorial summary
                        "hash": self._generate_result_hash(place.get("place_id", ""))
                    }
                    # Only include generative_summary if it exists and is not empty
                    generative_summary = place.get("generative_summary")
                    if generative_summary:
                        preview["generative_summary"] = generative_summary
                    # Only include reviews if they were requested (they significantly increase response size)
                    reviews = place.get("reviews")
                    if reviews:
                        preview["reviews"] = reviews
                    all_results.append(preview)
                
                logger.info(f"Search {i+1}/{len(search_requests)} completed: {len(places)} results for '{search_query}'")
        ...
        # Build response:
        # - results: actual place results (will be flattened and encoded to TOON by main_processor)
        ...
        logger.info(f"Maps search skill execution completed: {len(all_results)} total results, {len(errors)} errors")
        return response
