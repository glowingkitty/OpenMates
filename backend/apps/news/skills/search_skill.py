...
class SearchSkill(BaseSkill):
    ...
    async def execute(
        ...
        for i, req in enumerate(search_requests):
            ...
            # Users can override this by explicitly providing a different freshness value
            # Valid values: "pd" (past day), "pw" (past week), "pm" (past month), "py" (past year)
            ...
            logger.debug(f"Executing news search {i+1}/{len(search_requests)}: query='{search_query}', country='{req_country}'")
            ...
            try:
                ...
                if not is_allowed:
                    logger.info(f"Rate limit hit for news search '{search_query}', waiting for reset...")
                    try:
                        ...
                        logger.warning(
                            f"News search '{search_query}' was scheduled via Celery (task_id: {e.task_id}) "
                            f"due to rate limit. This search will be processed asynchronously."
                        )
                        # Skip this search request as it's been scheduled
                        continue
                
                # Call Brave News Search API using dedicated /news/search endpoint
                ...
                task_id = f"news_search_{i+1}_{search_query[:50]}"  # Limit length for logging
                        ...
                        try:
                            ...
                            for idx, metadata in enumerate(result_metadata):
                                ...
                                if not sanitized_title or not sanitized_title.strip():
                                    logger.warning(f"[{task_id}] News search result {idx} title empty after sanitization, using original: {metadata.get('url', 'unknown')}")
                                ...
                                if not sanitized_title or not sanitized_title.strip():
                                    logger.warning(f"[{task_id}] News search result {idx} title blocked due to prompt injection risk: {metadata.get('url', 'unknown')}")
                                    continue
                ...
                logger.info(f"News search {i+1}/{len(search_requests)} completed: {len(results)} results for '{search_query}'")
        ...
        logger.info(f"News search skill execution completed: {len(all_previews)} total results, {len(errors)} errors")
        return response
