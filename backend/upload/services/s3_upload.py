...
class UploadsS3Service:
    ...
    def __init__(self) -> None:
        ...
        # Per-environment bucket names for legacy public profile images.
        # LEGACY: kept for backward compat — existing users still have images here.
        # New uploads go to profile_private_bucket_name_*.
        self.profile_bucket_name_prod: str = "openmates-profile-images"
        self.profile_bucket_name_dev: str = "dev-openmates-profile-images"
        # Per-environment bucket names for new private, AES-256-GCM encrypted profile images.
    ...
    async def initialize(self) -> None:
        ...
        # ARCHITECTURE: Chatfiles and new private profile-images buckets are PRIVATE.
        ...
        #
        # Profile images (LEGACY): ACL 'public-read' — existing users keep their
        # unencrypted public-read URLs until they re-upload.
        #
        # Profile images (NEW private bucket): ACL 'private' — bytes are AES-256-GCM
        ...
        for bucket_name in (self.profile_bucket_name_dev, self.profile_bucket_name_prod):
            try:
                ...
                logger.info(f"[S3Upload] Set public-read ACL on legacy profile bucket '{bucket_name}'")
            except ClientError as acl_e:
                logger.warning(
                    f"[S3Upload] Failed to set public-read ACL on legacy profile bucket '{bucket_name}': {acl_e}."
                )
