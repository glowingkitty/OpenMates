...
class UploadsS3Service:
    ...
    async def initialize(self) -> None:
        ...
        # ARCHITECTURE: Chatfiles buckets are now PRIVATE. Encrypted blobs are served
        # to the client via time-limited presigned URLs generated by the API server.
        # Profile-images buckets remain public-read (unencrypted user avatars).
        #
        # Chatfiles: ACL set to 'private' — no anonymous access. The client requests
        # a presigned URL from GET /v1/embeds/presigned-url, then fetches the ciphertext
        # from S3 and decrypts client-side (AES-256-GCM via Web Crypto API).
        #
        # Profile images: ACL remains 'public-read' — these are unencrypted and must be
        # directly accessible as user avatar URLs.
        for bucket_name in (self.profile_bucket_name_dev, self.profile_bucket_name_prod):
            try:
                self.client.put_bucket_acl(Bucket=bucket_name, ACL="public-read")
                logger.info(f"[S3Upload] Set public-read ACL on profile bucket '{bucket_name}'")
            except ClientError as acl_e:
                logger.warning(
                    f"[S3Upload] Failed to set public-read ACL on profile bucket '{bucket_name}': {acl_e}."
                )
