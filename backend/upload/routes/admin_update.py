# backend/upload/routes/admin_update.py
#
...
# upload server: git pull + docker compose build + docker compose up.
#
...
#   3. docker compose build app-uploads (rebuilds the image with new code)
#   4. docker compose up -d app-uploads (restarts the container)
#
...
_SERVICE_NAME = "app-uploads"

# Compose file path (relative to project root)
_COMPOSE_FILE = "backend/upload/docker-compose.yml"
...
async def _run_update() -> asyncio.Queue:
    ...
    async def _produce() -> None:
        ...
        await queue.put("[AdminUpdate] Starting update for upload server\n")
...
@router.post(
    ...
    summary="Pull latest code and rebuild/restart upload server",
    description=(
        "Triggers a self-update of the upload server:\n"
        "1. git pull\n"
        ...
        "Called by: `admin_debug_cli.py upload-update`\n\n"
...
async def post_admin_update(
    ...
    """
    Trigger a full self-update of the upload server.
