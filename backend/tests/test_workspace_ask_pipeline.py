...
class FakeModelSelector:
    ...
    def select_models(self, **kwargs):
        ...
        return SimpleNamespace(
            primary_model_id="google/gemini-3.7-flash" if complexity == "complex" else "google/gemini-3-flash-preview",
...
async def test_task_pipeline_uses_preprocessor_selected_main_model_and_description_generator() -> None:
    ...
    async def fake_llm(**kwargs):
        ...
        if task_id.endswith("-main"):
            assert kwargs["model_id"] == "google/gemini-3.7-flash"
            return {
                "tasks": [
    ...
    assert result.processing["model_selection"]["primary_model_id"] == "google/gemini-3.7-flash"
    assert result.processing["intent_frame"]["selected_main_llm_model_id"] == "google/gemini-3.7-flash"
