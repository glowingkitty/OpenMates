...
def test_execute_skill_web_read(api_client):
    ...
    response = api_client.post("/v1/apps/web/skills/read", json=payload, timeout=30.0)
    ...
    assert len(page.get("markdown", "")) > 0, "Markdown content should not be empty"
...
def test_execute_skill_news_search(api_client):
    ...
    payload = {
        "requests": [
            {"query": "artificial intelligence", "count": 3}
        ]
    }

    print("\n[NEWS SEARCH] Searching for 'artificial intelligence' news...")
    response = api_client.post("/v1/apps/news/skills/search", json=payload, timeout=30.0)
...
def test_execute_skill_maps_search(api_client):
    ...
    payload = {
        "requests": [
            {"query": "coffee shops in Munich", "pageSize": 3}
        ]
    }

    print("\n[MAPS SEARCH] Searching for 'coffee shops in Munich'...")
    response = api_client.post("/v1/apps/maps/skills/search", json=payload, timeout=30.0)
    ...
    assert place.get("formatted_address"), "Place should have a 'formatted_address'"

    print(f"[MAPS SEARCH] Found {len(places)} place(s)")
    print(f"[MAPS SEARCH] First place: {place.get('name')} - {place.get('formatted_address')}")
...
def test_execute_skill_videos_search(api_client):
    ...
    response = api_client.post("/v1/apps/videos/skills/search", json=payload, timeout=30.0)
    assert response.status_code == 200, f"Videos search failed: {response.text}"
...
def test_execute_skill_videos_transcript(api_client):
    ...
    Fetches transcript for a short TED talk - these tend to pass sanitization
    since they contain natural speech rather than code/command snippets.

    NOTE: Sanitization may block videos whose transcripts contain code-like content
    (e.g., programming tutorials). The test accepts this as a valid response
    (success=True but empty results with sanitization error) and checks it gracefully.
    The critical assertion is that the skill returns a valid HTTP 200 with the
    expected response structure — not that content is always returned.
    """
    # Short TED talk (~5 min, English captions, natural speech — lower injection risk)
    youtube_url = "https://www.youtube.com/watch?v=UF8uR6Z6KLc"

    payload = {
        "requests": [
            {"url": youtube_url, "languages": ["en"]}
        ]
    }

    print(f"\n[VIDEOS TRANSCRIPT] Fetching transcript for: {youtube_url}")
    response = api_client.post("/v1/apps/videos/skills/get_transcript", json=payload, timeout=30.0)
    assert response.status_code == 200, f"Videos transcript failed: {response.text}"
    ...
    if transcripts:
        # Happy path: transcript successfully fetched and sanitized
        transcript = transcripts[0]
        assert transcript.get("url") or transcript.get("video_id"), "Transcript should reference the video"
        transcript_text = (
            transcript.get("transcript") or
            transcript.get("text") or
            transcript.get("content", "")
        )
        assert len(transcript_text) > 100, f"Transcript content too short: {len(transcript_text)} chars"
        print(f"[VIDEOS TRANSCRIPT] Transcript length: {len(transcript_text)} chars")
        print(f"[VIDEOS TRANSCRIPT] Preview: {transcript_text[:200]}")
    else:
        # Acceptable failure: sanitization blocked this specific video's transcript.
        # The skill worked correctly (200 OK, valid structure, graceful error) but the
        # content sanitizer flagged this transcript. This is expected behavior when the
        # video transcript contains code-like content triggering prompt injection protection.
        assert sanitization_error, "Empty results should include an error message"
        print(f"[VIDEOS TRANSCRIPT] Sanitization blocked transcript (expected behavior): {sanitization_error}")
        print("[VIDEOS TRANSCRIPT] NOTE: This is valid behavior - content sanitizer protects against prompt injection")
...
def test_execute_skill_reminder_lifecycle(api_client):
    ...
    # Set a reminder 1 hour in the future (UTC)
    trigger_dt = (datetime.now(timezone.utc) + timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M:%S")
    ...
    set_resp = api_client.post("/v1/apps/reminder/skills/set-reminder", json=set_payload, timeout=20.0)
    assert set_resp.status_code == 200, f"set-reminder failed: {set_resp.text}"

    set_data = set_resp.json()
    assert set_data["success"] is True, f"set-reminder success=False: {set_data}"
    assert "data" in set_data
    reminder_result = set_data["data"]
    assert reminder_result.get("success") is True, f"Inner success=False: {reminder_result}"
    assert reminder_result.get("reminder_id"), "Expected a reminder_id in response"
    ...
    list_resp = api_client.post("/v1/apps/reminder/skills/list-reminders", json=list_payload, timeout=15.0)
    assert list_resp.status_code == 200, f"list-reminders failed: {list_resp.text}"

    list_data = list_resp.json()
    assert list_data["success"] is True, f"list-reminders success=False: {list_data}"
    assert "data" in list_data
    list_result = list_data["data"]
    assert list_result.get("success") is True, f"Inner list success=False: {list_result}"
    ...
    print(f"[REMINDER] Found {len(reminders)} pending reminder(s), our reminder is listed")

    # --- 3. Cancel reminder ---
    print(f"[REMINDER] Cancelling reminder {reminder_id}...")
    cancel_payload = {"reminder_id": reminder_id}
    cancel_resp = api_client.post("/v1/apps/reminder/skills/cancel-reminder", json=cancel_payload, timeout=15.0)
    assert cancel_resp.status_code == 200, f"cancel-reminder failed: {cancel_resp.text}"

    cancel_data = cancel_resp.json()
    assert cancel_data["success"] is True, f"cancel-reminder success=False: {cancel_data}"
    assert "data" in cancel_data
    cancel_result = cancel_data["data"]
    assert cancel_result.get("success") is True, f"Inner cancel success=False: {cancel_result}"

    print("[REMINDER] Lifecycle test PASSED - set/list/cancel all successful!")
...
def test_execute_skill_travel_search_stays(api_client):
    ...
    response = api_client.post("/v1/apps/travel/skills/search_stays", json=payload, timeout=30.0)
    assert response.status_code == 200, f"travel/search_stays failed: {response.text}"
...
def test_execute_skill_code_get_docs(api_client):
    ...
    response = api_client.post("/v1/apps/code/skills/get_docs", json=payload, timeout=30.0)
    assert response.status_code == 200, f"code/get_docs failed: {response.text}"
    ...
    # The skill returns library info and documentation content
    assert "documentation" in skill_data, f"Response missing 'documentation'. Keys: {list(skill_data.keys())}"
    documentation = skill_data.get("documentation") or ""
    assert len(documentation) > 50, f"Documentation too short: {len(documentation)} chars"
...
def test_execute_skill_openmates_share_usecase(api_client):
    ...
    """
    payload = {
        "summary": "This is an automated test submission from the integration test suite. "
                   "The user wants to use OpenMates to search the web and summarize results.",
        "language": "en"
    }

    print("\n[SHARE USECASE] Submitting anonymous use-case summary...")
    response = api_client.post("/v1/apps/openmates/skills/share-usecase", json=payload, timeout=20.0)
    assert response.status_code == 200, f"openmates/share-usecase failed: {response.text}"
    ...
    assert skill_data.get("success") is True, f"Inner success=False: {skill_data}"
    assert not skill_data.get("error"), f"Unexpected error: {skill_data.get('error')}"
