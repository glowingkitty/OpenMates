...
from backend.core.api.app.services.project_task_sync_service import ProjectTaskSyncService, scope_key
...
def service():
    directus = SimpleNamespace(project=SimpleNamespace(
        get_project=AsyncMock(return_value={'id': 'project'}), get_source=AsyncMock(return_value={'status': 'connected'})),
        team=SimpleNamespace(require_team_role=AsyncMock()))
    instance = ProjectTaskSyncService(directus, SimpleNamespace())
    instance._read = AsyncMock(return_value={'reset': True, 'cursor': 'epoch:1',
        'tasks': [{'task_id': str(i)} for i in range(45)], 'removed_task_ids': []})
    return instance
...
async def test_authorized_subscription_sends_all_records_and_commits_cursor():
    instance = service()
    websocket = Socket()
    await instance.subscribe(websocket, 'user', {'bindings': [{'project_id': 'project', 'source_id': 'source'}]})
    ...
    assert [frame['payload']['final'] for frame in websocket.frames] == [False, False, True]
...
async def test_committed_hint_only_reads_the_affected_workspace():
    ...
    await asyncio.gather(instance.subscriptions[first]['project'].delivery,
