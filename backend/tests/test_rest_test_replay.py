...
def boundary(monkeypatch):
    ...
    return SimpleNamespace(
        get_user_by_id=AsyncMock(return_value={"email": "runner@example.test"})
    )


def prepare(cache, text="hello <<<TEST_LIVE_MOCK:web_search_cli>>>", **kwargs):
    return asyncio.run(
        prepare_rest_replay_messages(
            [{"role": "user", "content": text}], "owner", cache, **kwargs
        )
    )
...
@pytest.mark.parametrize(
    "setting,value",
    [("SERVER_ENVIRONMENT", "production"), ("MOCK_EXTERNAL_APIS", "false")],
)
...
@pytest.mark.parametrize(
    "marker",
    [
        "<<<TEST_LIVE_REAL:daily_canary_20260908>>>",
        "<<<TEST_LIVE_RECORD:web_search_cli:run>>>",
        "<<<TEST_LIVE_MOCK:web_search_cli:run>>>",
        "<<<TEST_LIVE_MOCK:bad group>>>",
    ],
)
def test_other_modes_and_malformed_markers_rejected(boundary, marker):
    with pytest.raises(ValueError):
        prepare(boundary, "hello " + marker)
...
def test_foreign_or_expired_signature_is_not_resigned(boundary, owner, ttl):
    signed = sign_live_marker(
        "<<<TEST_LIVE_MOCK:web_search_cli>>>",
        owner,
        is_allowlisted_test_account=True,
        ttl_seconds=ttl,
    )
    with pytest.raises(ValueError, match="signature"):
        prepare(boundary, "hello " + signed)
...
def test_cold_profile_comes_from_authenticated_user_lookup(boundary):
    ...
    cms = SimpleNamespace(
        get_user_profile=AsyncMock(
            return_value=(True, {"email": "runner@example.test"}, None)
        )
    )
    assert detect_live_marker(
        prepare(boundary, directus_service=cms)[0]["content"], "owner"
    )
...
def test_history_and_assistant_markers_cannot_activate_replay(boundary):
    for messages in [
        [{"role": "assistant", "content": "<<<TEST_LIVE_MOCK:web_search_cli>>>"}],
        [
            {"role": "user", "content": "<<<TEST_LIVE_MOCK:web_search_cli>>>"},
            {"role": "user", "content": "next"},
        ],
    ]:
        with pytest.raises(ValueError):
            asyncio.run(prepare_rest_replay_messages(messages, "owner", boundary))


@pytest.mark.parametrize("route", ["sdk", "apps"])
@pytest.mark.asyncio
async def test_actual_rest_submission_signs_before_dispatch(
    boundary, monkeypatch, route
):
    ...
    class Registry:
        async def dispatch_skill(self, app, skill, payload):
            ...
            if route == "apps":
                assert is_central_app_skill_dispatch()
            return {"content": "fixture result"}

        def get_metadata(self, app):
    ...
    if route == "sdk":
        monkeypatch.setattr(
            sdk, "_authenticate_sdk_request", AsyncMock(return_value=identity)
        )
        request = SimpleNamespace(
            app=SimpleNamespace(
                state=SimpleNamespace(cache_service=boundary, directus_service=None)
            )
        )
        await sdk.create_sdk_chat(
            request, sdk.SdkChatCreateRequest(message=text, save_to_account=False)
        )
    else:
        ...
        monkeypatch.setattr(
            ...
            lambda *args: exposure.append(args),
        )
        ...
        await apps_api.call_app_skill(
            ...
            {},
            identity,
            cache_service=boundary,
        )
        assert exposure and safety.await_count == 1
    assert len(captured) == 1
    assert captured[0]["_user_id"] == "owner"
    assert (
        detect_live_marker(captured[0]["messages"][-1]["content"], "owner").mode
        == "mock"
    )
