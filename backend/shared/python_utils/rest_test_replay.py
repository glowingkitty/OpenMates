...
async def prepare_rest_replay_messages(
    messages, user_id, cache_service, directus_service=None
):
    ...
    for index, message in marked:
        ...
        if (
            len(matches) != 1
            or content.count("<<<TEST_LIVE_") != 1
            or matches[0].group(1) != "MOCK"
            or matches[0].group(3)
            or message.get("role") != "user"
            or index != len(messages) - 1
            or content[matches[0].end() :].strip()
        ):
            raise ValueError(
                "REST supports only a trailing replay marker on the current user turn"
            )
        match = matches[0]
        ...
        signed = sign_live_marker(
            match.group(0), user_id, is_allowlisted_test_account=True
        )
        if not signed:
