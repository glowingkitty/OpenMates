...
# Video registry keys → the generated TS file imports from videos.ts.
# When meta.yml sets ``video.intro_fullscreen: true`` the publisher writes an
# import for the "intro" key from the central video registry
# (frontend/packages/ui/src/demo_chats/data/videos.ts).
INTRO_VIDEO_KEY = "intro"
...
def write_demo_chat_ts(inputs: Dict[str, Any]) -> Path:
    ...
    if video_meta.get("intro_fullscreen"):
        video_key = INTRO_VIDEO_KEY
    elif video_meta.get("video_key"):
        video_key = video_meta["video_key"]

    if video_key:
        video_import_line = 'import { OPENMATES_VIDEOS } from "./videos";\n'
        var_name = f"_video_{video_key.replace('-', '_')}"
        video_lines.append(f'\nconst {var_name} = OPENMATES_VIDEOS["{video_key}-en"];\n')
        video_lines.append(f'    video_key: "{video_key}",')
        video_lines.append(f"    video_hls_url: {var_name}.hls_url,")
        video_lines.append(f"    video_mp4_url: {var_name}.mp4_url,")
        video_lines.append(f"    video_thumbnail_url: {var_name}.thumbnail_url,")
        video_lines.append(f"    video_start_time: {var_name}.start_time,")
        video_lines.append(f"    background_frames: {var_name}.background_frames,")
    else:
