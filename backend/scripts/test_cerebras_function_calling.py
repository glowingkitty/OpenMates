#!/usr/bin/env python3
# backend/scripts/test_cerebras_function_calling.py
#
# Integration test script for function calling via Cerebras API.
# This script tests function calling with Cerebras models that support tool use.
#
# Run via docker exec:
#   docker compose exec api python /app/backend/scripts/test_cerebras_function_calling.py
#
# Or from the project root:
#   cd backend/core && docker compose exec api python /app/backend/scripts/test_cerebras_function_calling.py
...
async def run_integration_tests():
    """
    Run integration tests for function calling via Cerebras API.
    
    Tests function calling with Cerebras models that support tool use.
    ...
    print("\n" + "="*80)
    print("CEREBRAS FUNCTION CALLING INTEGRATION TESTS (Real API Calls)")
    print("="*80 + "\n")
    ...
    try:
        ...
        print("Run with: cd backend/core && docker compose exec api python /app/backend/scripts/test_cerebras_function_calling.py")
        return 1
    
    # Define models to test (using Cerebras models that support function calling)
    # Note: Update these model IDs based on actual Cerebras models available that support tool use
    models_to_test = [
        "qwen-3-235b-a22b-instruct-2507",  # Alibaba Qwen3 via Cerebras
        "zai-glm-4.7",  # Z.ai GLM 4.7 via Cerebras
        # Add more Cerebras models that support function calling as needed
    ]
    ...
    test_cases = [
        ...
        # Test 5: Required function call with multiple tools (should call one of them)
        (
            "Required: Multiple tools (should call one)",
            [
                {"role": "system", "content": "You are a helpful assistant with access to calculators and weather APIs."},
            ...
            test_tools,  # Both tools available
            "required",
            "calculate"  # With "required", it should call one of the tools (preferably calculate for this query)
        ),
    ]
    ...
    for model_id in models_to_test:
        ...
        for test_name, messages, tools, tool_choice, expected_function_name in test_cases:
            ...
            try:
                ...
                task_id = f"test_{model_id.replace('/', '_').replace('-', '_')}_{test_name.replace(' ', '_').replace(':', '').replace('(', '').replace(')', '')}"
                ...
                # Check if correct function was called (for required, it should match or be one of the available tools)
