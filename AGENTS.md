...
- For eligible full specs and user-visible Tier 1 work, never request the user-confirmation gate from screenshots, a test report, or visual-smoke evidence alone. First use `create-demo-video` to produce an actual captioned narration video from passing real CLI or deployed Playwright evidence, review its bounded frame bundle, and provide the retained video path or delivery link to the user. Screenshots are review inputs, not a substitute for the video. User confirmation follows the reviewed video; Apple parity follows user confirmation. Audio narration remains optional unless explicitly requested, and Discord publication remains a separate delivery status.
