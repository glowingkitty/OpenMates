...
- Every new feature implementation, every new hardcoded example chat, and every nightly/daily/CI failed E2E that is actively debugged in a chat and turns green requires proof-video evidence before session completion. First use `create-demo-video` to produce actual videos from passing real CLI or deployed Playwright evidence, using ElevenLabs `eleven_flash_v2_5` narration audio plus burned-in captions. Web/spec/example-chat proof uses separate phone and laptop videos, Apple proof uses separate iPhone portrait and iPad landscape videos, and CLI proof uses one terminal video. Review the bounded frame bundle plus full narration; objective defects such as clipping, premature truncation, wrong icons or gradients, raw protocol/error text, missing processing animation, stale loading states, or broken navigation must return to failing-test and implementation work. When the Discord proof destination is configured, confirmed Discord delivery is a hard completion gate.
