...
- Temporary Docker, deploy, test, and edit-resource contention is never a terminal chat blocker. Run the intended canonical command directly when it already queues atomically. For a diagnostic/manual gate, run `python3 scripts/sessions.py wait-lock --session <id> --type <docker|vercel> --follow --poll 10` with a sufficiently long tool timeout, wait for the `OPENMATES_WAIT_READY` signal, and continue the interrupted operation in the same response. Do not pre-check a lock once and return a blocked final answer.
