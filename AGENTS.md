...
- For every `*.spec.ts` Playwright run and every real OpenMates CLI E2E run that generates a video, paste the emitted `<video>` HTML in the next assistant progress response. This is required even when the run failed, the proof video is visibly broken, or more debugging remains.
- For Figma-based UI work, after exporting the target frame PNG, upload it with `python3 scripts/opencode_response_media.py <path> --alt "Figma reference: <screen/frame>"` and paste the returned image Markdown before editing or summarizing that screen. Repeat when switching target frames.
