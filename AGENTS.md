...
- For new shared features, app skills, focus modes, embeds, memory types, and provider-backed behavior, implement and test in strict order: real CLI commands on the dev server first, npm SDK and pip SDK locally against the dev server second, web third, user confirmation fourth, Apple parity last. CLI and SDK gates must hit the real dev API/WebSocket path at `https://api.dev.openmates.org` with real auth/test-account state; mocked API calls, mocked SDK clients, stubbed servers, direct function calls, and fixture replay do not satisfy them. Only after local dev-server CLI and SDK tests pass should the same coverage be reproduced or wired into GitHub Actions for CI/daily tests.
...
- For new shared functionality, CLI and SDK tests first run locally as real commands/SDK calls against the dev server with no mocked OpenMates API/WebSocket calls; only after they pass should the same coverage move or wire into GitHub Actions for CI/daily tests. Web, user confirmation, and Apple work wait for real dev-server CLI and SDK proof when those surfaces apply.
