...
- **Requirement examples:** Write natural-language scenarios or concrete code/CLI commands with expected results. Do not use flattened dictionaries of fixture fields and booleans as human approval examples. New/changed requirements must show one or two mapped examples as paragraphs or code blocks in the approval PDF.
...
## Codex communication and Task context

For Codex, this section and `docs/architecture/codex-task-cache.md` govern
orchestration and testing when legacy OpenCode/Claude workflow instructions
elsewhere conflict. The OpenCode-specific sections below remain for OpenCode.
Shared coding, security and repository ownership rules still apply.

Run product REST/WebSocket, CLI/SDK integration and browser tests through the
existing isolated GitHub CI coordinator. Publish a source with
`sessions.py ci-source`, submit selected specs with `ci_coordinator.py submit`,
and retrieve its cached result with `ci_coordinator.py result`. Do not run these
product tests against shared dev or start a second CI scheduler. Focused unit,
...
unlinked work for assignment. Ordinary CLI creation is unlinked. Codex creation
...
approved work during a transient outage; wait for acknowledgement before starting
newly claimed or dependency-blocked work. See
`docs/architecture/codex-task-cache.md` for commands, context examples and rollout.
...
- Daily meetings ask today's priorities **before research**, then review yesterday's saved priorities, yesterday's and today's Codex tasks **and git commits**, OpenMates CLI Tasks/activity, and nightly source-bound coverage/notification receipts. Ask four clarifying questions afterward, one per round with a reply before proceeding, then propose focus and assignments for approval. Keep dated private meeting records in gitignored `logs/daily-meetings/`. Job batches are not test counts. Follow `daily-meeting-and-orchestration` and `docs/architecture/codex-orchestration.md`.
- Orchestrators use cached Task state for routine progress. Inspect a worker with `wait_threads` (`timeoutMs: 0`, its `threadId` and `hostId`) when Task details are insufficient; use `read_thread` for a specific unresolved question. Do not wake workers or the orchestrator for routine activity, or repeatedly copy status tables between chats.
...
- Product integration and Playwright checks run in the existing isolated GitHub CI environment. Publish reviewed candidate source with `sessions.py ci-source --session <id>`, submit it through `ci_coordinator.py submit`, and consume source/harness/profile-bound results. Focused unit checks can run locally. Do not run shared-dev E2E or create another CI scheduler.
...
- OpenCode must not load the Claude Code provider. OpenCode Task subchats use the central `subagent` default in `opencode.json`: GPT-6 Astra at medium reasoning, with `explore` defaulting to low. The main chat can set Task `reasoning_effort` to `low` for exploration, lookups and straightforward edits or `medium` for implementation, debugging, review and verification. Omit Task `model` except for a deliberate model exception. Keep model pins out of generated specialist agents and skills.
...
- Write or update required checks and retain failures when the Plan requires red-phase evidence. Playwright evidence must come from the isolated GitHub runner and match the tested candidate, harness and proof profile; a previous deployment or ancestor commit is insufficient.
- For new shared functionality, run real REST API/WebSocket, CLI and SDK integration checks in the existing isolated GitHub CI stack before dependent web proof. Focused unit checks run locally. Keep source, harness, profile and recording provenance with the returned results.
