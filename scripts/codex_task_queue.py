...
def enqueue(snapshot_path: Path, operation: dict, key: str, thread: str, root: Path) -> dict:
    uuid.UUID(thread)
    if not key or len(key) > 200:
        raise ValueError("Use a stable, short operation ID, such as header-investigation-finished")
    snapshot = json.loads(snapshot_path.read_text())
    scope = snapshot.get("account_scope")
    if not isinstance(scope, list) or len(scope) != 3 or not all(isinstance(part, str) and part for part in scope):
        raise ValueError("This cache lacks an authenticated account scope; refresh remote-access first")
    if snapshot.get("connection") == "revoked":
        raise ValueError("Project access was revoked; no command was queued")
    allowed = {"kind", "task_id", "title", "description", "status", "message", "link_to_chat"}
    ...
    if kind == "create":
        ...
        if "link_to_chat" in operation and not isinstance(operation["link_to_chat"], bool):
            raise ValueError("link_to_chat must be a boolean")
    else:
        task = next((item for item in snapshot["tasks"] if item["task_id"] == operation.get("task_id")), None)
        if not task:
        ...
        if owner.get("provider") != "codex" or owner.get("id") != thread:
            raise ValueError(f'Task is linked to {json.dumps(owner.get("title") or "another chat")}; Codex chat ID: {owner.get("id") or "unavailable"}. Only its owner can enqueue work.')
    for field in ("title", "description", "message"):
        if field in operation and (not isinstance(operation[field], str) or len(operation[field]) > 100_000):
    ...
    if kind in {"create", "complete"} and scope[2] != "personal":
        raise ValueError("This command is not supported in Team scope by the current Task API")
    if "status" in operation and operation["status"] not in {"backlog", "todo", "in_progress", "blocked", "done"}:
        raise ValueError("Unsupported Task status")
    identity = hashlib.sha256(json.dumps([thread, key], separators=(",", ":")).encode()).hexdigest()
    account = hashlib.sha256(json.dumps(scope, separators=(",", ":")).encode()).hexdigest()
    ...
    with (directory / ".enqueue.lock").open("a") as lock:
        ...
        if path.exists():
            ...
            if saved["operation"] != operation:
                raise ValueError("This operation ID already refers to different work; use a new ID")
        else:
            saved = {"schema_version": 1, "id": identity, "scope": scope, "thread": thread,
                     "project_id": snapshot["project_id"], "operation": operation,
                     "task_id": task["task_id"] if task else str(uuid.uuid4()),
                     "task": task, "created_at": time.time(), "state": "pending"}
    ...
    return {"delivery": {"id": identity, "state": saved["state"], "task_id": saved["task_id"]}}
...
def main():
    ...
    parser.add_argument("--operation", help="JSON object using the existing Task fields")
    args = parser.parse_args()
    thread = os.environ.get("CODEX_THREAD_ID")
    if not thread:
        parser.error("Run this inside the owning Codex chat; CODEX_THREAD_ID is required")
    ...
    if args.status:
        ...
        for path in sorted((root / "task-command-delivery" / account).glob("*.json")):
            ...
            if record.get("thread") == thread:
                entries.append({key: record.get(key) for key in ("id", "task_id", "state", "error")})
        print(json.dumps({"deliveries": entries}))
        return
    if not args.id or not args.operation:
        parser.error("Enqueue requires --id and --operation; use --status to inspect existing delivery")
    print(json.dumps(enqueue(args.snapshot, json.loads(args.operation), args.id, thread, root)))
