...
Auto-generated by the E2E test suite via the cookie-audit Playwright fixture
and merged into `docs/architecture/compliance/cookies.yml` by
`scripts/merge_storage_audits.py`. **This file — and only this file — is
the empirical evidence that backs the "essential-only, no consent banner
needed" claim from acknowledgments.yml.** Cookies are the only storage type
that legally gates the banner requirement in the specific way we rely on
(ePrivacy Art. 5(3) / TTDSG §25 strictly-necessary exemption).

```yaml
{{COOKIES_YAML}}
```

**Compliance evaluation rules for cookies.yml:**
- Verify every observed cookie has `consent_exempt: true` AND an
  `exemption_basis` that clearly maps to "strictly necessary for the
  service explicitly requested by the user".
- Flag any entry where `purpose`, `consent_exempt`, or `exemption_basis` is
  null/missing — these need human review before the next scan can re-confirm
  the exemption.
- Flag any non-essential third-party cookie (analytics, advertising,
  fingerprinting) — its presence means a consent banner is now mandatory.
- If the file is empty or missing, treat it as a Top 10 finding: "no runtime
  evidence for cookie-banner exemption — run the Playwright suite".
- If everything checks out, explicitly state in the report that the
  cookie-banner exemption is re-confirmed with runtime evidence as of {{DATE}}.

### Browser-storage inventory (informational — NOT a banner-decision input)

Auto-generated key-name inventory of localStorage, sessionStorage, and
IndexedDB databases the app uses. Values are intentionally never captured.
**Do NOT use this to decide the cookie-banner question** — it does not
affect the banner verdict. Its role here is engineering + privacy awareness:
flag any key name that suggests analytics/fingerprinting/tracking
(e.g. `_ga`, `mp_*`, `amplitude_*`, `_fbp`, `mixpanel.*`), and flag keys
with missing `purpose` so humans can document what the app persists.
