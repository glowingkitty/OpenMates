...
### Cookie & browser-storage runtime inventory (empirical evidence)

This file is auto-generated by the E2E test suite via the cookie-audit
Playwright fixture and merged into `docs/architecture/compliance/cookies.yml`
by `scripts/merge_storage_audits.py`. It is the canonical runtime evidence
that backs the "essential-only, no consent banner needed" claim from
acknowledgments.yml.
...
**Compliance evaluation rules for this section:**
- Verify every observed cookie / localStorage / sessionStorage / IndexedDB
  entry has `consent_exempt: true` AND an `exemption_basis` that clearly
  maps to "strictly necessary for the service explicitly requested by the
  user" under ePrivacy Art. 5(3) and TTDSG §25 (German DDG).
...
- If the file is missing or empty, treat it as a Top 10 finding: "no runtime
