...
### Cookie inventory (banner-decision source of truth)

Empirical evidence from the E2E test suite (auto-generated). This file
alone drives the cookie-banner verdict. For a delta scan, only re-evaluate
if a commit touched cookies, auth routes, frontend storage code, or
analytics SDKs. If the file is missing, flag it as a Top 10 item.

```yaml
{{COOKIES_YAML}}
```

### Browser-storage inventory (informational only)

Key names of localStorage / sessionStorage / IndexedDB — does NOT affect
the banner decision. Re-inspect only if a commit touched frontend storage
code; flag key names suggesting analytics/tracking.
