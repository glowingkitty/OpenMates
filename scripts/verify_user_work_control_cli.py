...
def _contains_identifier(value: Any, key: str, expected: str) -> bool:
    if isinstance(value, dict):
        return value.get(key) == expected or any(_contains_identifier(child, key, expected) for child in value.values())
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="openmates-work-control-cli-") as home_value, tempfile.TemporaryDirectory(prefix="openmates-work-control-recovery-") as recovery_root:
        ...
        try:
            ...
            assumption = cli(home, args.api_url, ["plans", "assumptions", "create", plan_id, "--id", "proof", "--text", "typed proof", "--sub-chat", "codex:verification", "--proof-file", "specifications/features/tasks/specification.yml:1:2", "--proof-url", "https://example.invalid/proof", "--proof-embed", "embed-proof"])
            assumption_id = identifier(assumption, "assumption_id")
            cli(home, args.api_url, ["plans", "assumptions", "update", plan_id, "--assumption", assumption_id, "--status", "checking", "--sub-chat", "codex:verification", "--proof-file", "specifications/features/tasks/specification.yml:1:2", "--proof-url", "https://example.invalid/proof", "--proof-embed", "embed-proof"])
            ...
            scoped_plans = cli(home, args.api_url, ["plans", "list", "--project", project_id])
            scoped_tasks = cli(home, args.api_url, ["tasks", "list", "--project", project_id])
            if not _contains_identifier(scoped_plans, "plan_id", plan_id):
                raise RuntimeError("project-scoped Plan list omitted the created fixture")
            if not _contains_identifier(scoped_tasks, "task_id", task_id):
                raise RuntimeError("project-scoped Task list omitted the created fixture")
            ...
            if dry_run.get("conflicts"):
                raise RuntimeError("recovery dry-run unexpectedly reported conflicts after source deletion")
