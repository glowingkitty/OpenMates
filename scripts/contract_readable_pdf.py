#!/usr/bin/env python3
"""Render a readable Contract presentation PDF.

Contracts are stored as deterministic YAML, but the default human surface is a
...
Approval still uses scripts/contract_approval_pdf.py and scripts/contracts.py.
"""
...
DEFAULT_OUTPUT_ROOT = Path("/tmp/opencode/contract-presentations")
IDENTIFIER_RE = re.compile(r"[^A-Za-z0-9_.:-]+")
NUMBERED_LINE_RE = re.compile(r"^\s*\d+\.\s+(.*)$")
IDENTITY_FIELDS = ("id", "name", "key")
SKIPPED_EXAMPLE_FIELDS = {"schema_version", "contract"}
ICON_PATHS = {
...
class ReadableContractError(ValueError):
...
def _as_mapping(value: Any, field: str) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise ReadableContractError(f"{field} must be a mapping")
    return value


def _as_list(value: Any, field: str) -> list[Any]:
    if not isinstance(value, list):
        raise ReadableContractError(f"{field} must be a list")
    return value
...
def _by_id(items: Any, field: str) -> dict[str, dict[str, Any]]:
    ...
    for index, item in enumerate(_as_list(items, field), start=1):
        ...
        if not isinstance(item_id, str) or not item_id:
            raise ReadableContractError(f"{field}[{index}].id must be a non-empty string")
        result[item_id] = item
    return result
...
def _surface_lookup(contract: dict[str, Any]) -> dict[str, dict[str, Any]]:
    ...
    for index, surface in enumerate(surfaces, start=1):
        ...
        if not isinstance(surface_id, str) or not surface_id:
            raise ReadableContractError(f"project_surface_catalog.surfaces[{index}].id must be a non-empty string")
...
def _ordered_chapters(contract: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not ordered:
        raise ReadableContractError("chapters must contain at least one chapter")
    return ordered
...
def _render_generated_indexes(bundle: contracts.ContractBundle, surface_lookup: dict[str, dict[str, Any]]) -> str:
    ...
    if "history" in requested:
        rows = [
            ["Contract", f'<code>{_escape(bundle.versioned_id)}</code>'],
...
def build_html(bundle: contracts.ContractBundle) -> str:
    contract = bundle.contract
    ...
    title = str(contract.get("title") or bundle.contract_id)
...
<head>
<meta charset="utf-8">
<title>{_escape(title)} readable Contract</title>
<style>
...
<section class="hero" id="contract-outcome">
  <div class="chapter-label">{_icon('outcome', css_class='card-icon')}Capability Contract</div>
  <h1>{_escape(title)}</h1>
  <div class="status">
...
def render_preview(document: str, output: Path, *, anchor: str = "") -> None:
    ...
    try:
        ...
        if anchor:
            ...
            if target.count() != 1:
                raise ReadableContractError(f"preview anchor must resolve exactly once: {anchor}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render and optionally upload a readable Contract presentation PDF")
    parser.add_argument("bundle", help="Contract bundle directory or contract.yml path")
    parser.add_argument("--output", type=Path, help="PDF output path; defaults under /tmp/opencode/contract-presentations")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        output = args.output or DEFAULT_OUTPUT_ROOT / f"{_safe_name(bundle.contract_id)}-{bundle.fingerprint[:16]}-readable.pdf"
        ...
        if not args.no_upload:
            result["publication"] = opencode_response_media.upload_file(
                output,
                alt=f"Read {bundle.versioned_id} readable Contract PDF",
                container=args.container,
                dry_run=args.dry_run_upload,
            )
    except Exception as exc:
        print(f"contract_readable_pdf: {exc}", file=sys.stderr)
        return 1
