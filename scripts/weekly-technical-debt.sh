...
# reports without launching an AI session. Automatic recommendation stages
# were removed; workflow migration is tracked by TASK-8338.
#
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --scan-only) shift ;; # Compatibility: the command always runs only the scan.
    --help|-h)
...
echo "[technical-debt] Deterministic scan complete."
