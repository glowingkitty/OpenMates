...
from scripts import contract_readable_pdf as readable_pdf  # noqa: E402
...
def _change_summary(
    ...
    groups = [
        ("Chapters", "chapter", "chapters", readable_pdf._anchor("chapter", bundle.contract.get("presentation", {}).get("chapter_order", ["chapter"])[0])),
    ...
    for label, icon, field, target in groups:
        changed = _changed_item_ids(contract.get(field), baseline_contract.get(field))
        removed = _removed_items(contract.get(field), baseline_contract.get(field))
        cards.append(
    ...
    changed_example_groups = sum(
        ...
        if key not in {"schema_version", "contract"} and value != baseline_examples.get(key, MISSING)
    )
    cards.append(
...
def _render_examples_appendix(
    ...
    for group_name, cases in examples.items():
        if group_name in {"schema_version", "contract"}:
            continue
...
def _render_technical_appendix(contract: dict[str, Any], baseline_contract: dict[str, Any]) -> str:
    ...
    technical = {
        ...
        "surface_contract": contract.get("surfaces"),
    ...
    baseline = {
        ...
        "surface_contract": baseline_contract.get("surfaces", MISSING),
...
def build_html(
    ...
    *,
    ...
    baseline_contract = baseline_contract or {}
    baseline_examples = baseline_examples or {}
    if not isinstance(bundle.contract.get("presentation"), dict):
    ...
    document = _annotate_readable_changes(document, bundle.contract, baseline_contract)
    ...
    appendices = (
        ...
        + _render_technical_appendix(bundle.contract, baseline_contract)
        + _render_removal_appendix(bundle.contract, baseline_contract)
    )
    document = document.replace(
        '<p class="footer-note">This is a readable Contract presentation, not an approval receipt. Exact approval must use the highlighted approval PDF and matching review artifact.</p>',
        appendices
        + '<p class="footer-note">Approval must match the exact fingerprint printed on page one. Any later Contract or example edit requires a new PDF and approval.</p>',
        1,
    )
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Render and privately upload an inline-diff Contract approval PDF")
