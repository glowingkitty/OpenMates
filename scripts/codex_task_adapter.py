...
def dependency_ready(previous, current):
    ...
    return (previous.get("status") == "blocked"
        and previous.get("ai_execution_state") == "waiting_for_plan_dependency"
...
def observe_tasks(state, tasks, allowed):
    for key, task in tasks.items():
        ...
        state["tasks"][key] = {field: task.get(field) for field in
            ("task_id", "version", "status", "ai_execution_state", "external_chat", "scope")}
...
def observe_notification(state, message, allowed, parents, tasks):
    ...
    if method == "thread/status/changed":
        ...
        if parent in allowed and turn.get("status") == "completed" and owned and all(task["status"] == "done" for task in owned):
            queue(state, parent, "worker_done", [owner, sorted((t["task_id"], t["version"]) for t in owned)],
...
def deliver(state, rpc, allowed, auto_wake, save, tasks):
    ...
    for item in state["deliveries"].values():
        ...
        if status == "active":
            ...
            continue
        if status not in {"idle", "notLoaded"} or thread.get("archived"):
            continue
    ...
    with path.with_suffix(".lock").open("a") as lock:
        ...
        while True:
            ...
            try:
                ...
                allowed = {thread for thread, host in config.get("execution_hosts", {}).items() if host == socket.gethostname() and thread in config.get("threads", [])}
                parents = {}
                ...
                for owner in sorted(allowed - subscribed):
                        # Subscribe/read metadata once per connection, not each tick.
                        if state["threads"].get(owner, {}).get("deleted"):
                            subscribed.add(owner); continue
                        try:
                            meta = rpc.call("thread/read", {"threadId": owner, "includeTurns": False})["thread"]
                        except CodexRPCRejected:
...
def main():
    ...
    if args.action == "serve":
        ...
        if args.thread not in config.get("threads", []): parser.error("Select a registered chat")
