...
try:
    ...
    from scripts.codex_orchestration import canonical_root, transaction
except ModuleNotFoundError:
    from codex_cached_context import _save, state_root
    from codex_rpc import CodexRPC, CodexRPCRejected
    from codex_orchestration import canonical_root, transaction
...
def adapter_path(root):
    return state_root() / "codex-adapters" / identity(str(root.resolve())) / "state.json"


def empty_state():
    return {"schema_version": 1, "tasks": {}, "threads": {}, "deliveries": {}, "ci_seen": {}}


def queue(state, owner, kind, key, data):
    event = identity([owner, kind, key])
    state["deliveries"].setdefault(event, {"id": event, "owner": owner, "kind": kind,
        "data": data, "state": "pending", "message_id": str(uuid.uuid4()), "created_at": time.time()})
...
def dependency_ready(previous, current):
    ...
    return (previous.get("status") == "blocked"
        ...
        and all(edge.get("target_kind") == "task" and edge.get("target_status") == "done" for edge in current["dependencies"]))


def execution_threads(root, config):
    """Explicit pilots or actual local repository bindings, never cache visibility."""
    host = socket.gethostname()
    allowed = {thread for thread, owner_host in config.get("execution_hosts", {}).items()
    ...
    if config.get("all_threads") and path.exists():
        ...
        for record in sessions.values():
            ...
            if thread and record.get("codex_host") == host and worktree.get("status") not in {"merged", "removed"} and location and Path(location).resolve().is_relative_to(root):
                uuid.UUID(thread)
                allowed.add(thread)
    return allowed
...
def observe_tasks(state, tasks, allowed):
    for key, task in tasks.items():
        ...
        if previous and owner in allowed and dependency_ready(previous, task):
            queue(state, owner, "dependency_ready", [key, task["version"]],
                  {"task_id": task["task_id"], "version": task["version"], "title": task["title"]})
        # Store only the metadata needed for routing/replay, never full transcripts.
        state["tasks"][key] = {field: task.get(field) for field in
            ("task_id", "version", "status", "blocked_reason_code", "blocked_reason", "external_chat", "scope")}
...
def observe_ci(root, config, state, allowed):
    ...
    owners = {sid: record.get("codex_task_id") for sid, record in sessions.items()
              if record.get("codex_host") == socket.gethostname() and record.get("codex_task_id") in allowed}
    if not owners:
        return
    with sqlite3.connect(database.as_uri() + "?mode=ro", uri=True) as db:
        db.row_factory = sqlite3.Row
        rows = db.execute("SELECT id,owner,source,state,run_id,url,proof_profile,created FROM jobs WHERE owner IN ("
            + ",".join("?" for _ in owners) + ") AND state IN ('success','failure','cancelled')", list(owners)).fetchall()
    for row in rows:
        job = dict(row)
        if job["id"] in state["ci_seen"] or job["created"] < config.get("activated_at", time.time()):
            continue
        receipt_path = root / "test-results/ci-runs" / job["id"] / "receipt.json"
        evidence = {"request_id": job["id"], "state": job["state"], "source_commit": job["source"],
                    "run_id": job["run_id"], "url": job["url"], "proof_profile": job["proof_profile"],
                    "result_command": f"python3 scripts/ci_coordinator.py result {job['id']}"}
        if receipt_path.exists():
            receipt = json.loads(receipt_path.read_text())
            if receipt.get("source_commit") != job["source"] or str(receipt.get("run_id")) != str(job["run_id"]) or receipt.get("proof_profile", "") != job["proof_profile"] or not receipt.get("harness_commit"):
                evidence["receipt_state"] = "identity_mismatch; do not use as passing proof"
            else:
                evidence.update(receipt=str(receipt_path), harness_commit=receipt["harness_commit"], receipt_state="available")
        else:
            evidence["receipt_state"] = "retrieve through existing coordinator before treating this as proof"
    ...
    if method == "thread/status/changed":
        ...
        for task in state["tasks"].values():
            if (task.get("external_chat") or {}).get("id") != owner:
                continue
            queue(state, owner, "chat_deleted", [owner, task["scope"]], {"scope": task["scope"]})
...
def observe_worker_completion(state, parents, tasks, allowed):
    ...
    for owner, parent in parents.items():
        ...
        owned = [task for task in tasks.values() if (task.get("external_chat") or {}).get("id") == owner]
        if turn and state["threads"].get(owner, {}).get("status") == "idle" and parent in allowed and owner in allowed and owned and all(task["status"] == "done" for task in owned):
            queue(state, parent, "worker_done", [owner, turn, sorted(t["task_id"] for t in owned)],
                  {"worker_id": owner, "tasks": [{"task_id": t["task_id"], "title": t["title"]} for t in owned]})
...
def subscribe_thread(rpc, state, owner):
    ...
    result = rpc.call("thread/resume", {"threadId": owner, "excludeTurns": True,
    ...
    if turns:
        ...
        if turn.get("status") == "interrupted" and (turn.get("completedAt") or time.time()) > runtime.get("control_at", 0):
            runtime["paused"] = True
        if turn.get("status") == "completed":
...
def reconcile_uncertain(rpc, item):
    ...
    for record in page.get("data", []):
        ...
        if payload.get("type") == "functionCallOutput" and payload.get("namespace") == "openmates" and payload.get("name") == "task_event":
            output = payload.get("output")
            try:
                found = found or isinstance(output, str) and json.loads(output).get("delivery_id") == item["message_id"]
            except ValueError:
                pass
        if found:
            item["state"] = "accepted"
    ...
    if not item["reconcile_cursor"]:
        ...
        item["error"] = "Start acceptance could not be proved; the original intent is retained and will not be sent twice."
...
def deliver(state, rpc, allowed, auto_wake, save, tasks):
    ...
    for item in state["deliveries"].values():
        ...
        if owner not in allowed or owner in attempted or item["kind"] == "chat_deleted":
            continue
        if item["state"] not in {"pending", "uncertain"} or time.time() < item.get("retry_at", 0):
            continue
        attempted.add(owner)
        ...
        if item["kind"] == "dependency_ready":
            task = next((t for t in tasks.values() if t["task_id"] == item["data"]["task_id"] and (t.get("external_chat") or {}).get("id") == owner), None)
            if not task:
                continue  # Revoked/disconnected/unselected scope cannot authorize a wake.
            if (task["status"] != "todo" or task["version"] < item["data"]["version"]
                or not task.get("dependencies")
                or not all(edge.get("target_kind") == "task" and edge.get("target_status") == "done" for edge in task["dependencies"])):
                item["state"] = "superseded"; save(); continue
        thread = rpc.call("thread/read", {"threadId": owner, "includeTurns": False})["thread"]
        ...
        try:
            result = rpc.call("turn/start", {"threadId": owner, "clientUserMessageId": item["message_id"],
                "turnTrigger": "openmates_tasks", "input": [], "toolOutput": {"name": "task_event", "namespace": "openmates",
                "output": json.dumps({"delivery_id": item["message_id"], "event": item["kind"], "data": item["data"],
            ...
            item.update(state="needs_review", error="Codex rejected the continuation; review the retained event before retrying.")
            save()
            raise
...
def run(root, config):
    ...
    with path.with_suffix(".lock").open("a") as lock:
        ...
        while True:
            ...
            readable = select.select([sys.stdin, *([rpc.socket] if rpc else [])], [], [], 5)[0]
            if sys.stdin in readable and not sys.stdin.readline():
                if rpc: rpc.close()
                return
            try:
                current_config = json.loads((state_root() / "codex-adapter.json").read_text())["repositories"].get(str(root))
                ...
                for owner in allowed:
                    ...
                    if control_path.exists():
                        ...
                        if control["updated_at"] > runtime.get("control_at", 0):
                            runtime.update(paused=control["paused"], control_at=control["updated_at"])
                    receipt_path = state_root() / "codex-context-receipts" / f"{owner}.json"
                    if receipt_path.exists():
                        receipt = json.loads(receipt_path.read_text())
                        for delivered_id in receipt.get("adapter_events", []):
                            item = state["deliveries"].get(delivered_id)
                            if item and item["state"] == "pending" and item["owner"] == owner:
                ...
                for owner in sorted(allowed - subscribed):
                        ...
                        if state["threads"].get(owner, {}).get("deleted"):
                            subscribed.add(owner); continue
                        try:
                            meta = subscribe_thread(rpc, state, owner)
                        except CodexRPCRejected:
                            state["threads"].setdefault(owner, {})["status"] = "unavailable"
                            subscribed.add(owner); continue
                        state["threads"].setdefault(owner, {})["status"] = meta.get("status", {}).get("type", "unknown")
                ...
                for item in state["deliveries"].values():
                    ...
                    directory = state_root() / "codex-chat-deletions" / hashlib.sha256(json.dumps(scope, separators=(",", ":")).encode()).hexdigest()
                    delivery_path = directory / f"{item['id']}.json"
                    if not delivery_path.exists():
                        _save(delivery_path, {"schema_version": 1, "event_id": item["id"], "scope": scope,
                            "thread": item["owner"], "evidence": "thread/deleted", "state": "pending"})
                    item["state"] = json.loads(delivery_path.read_text())["state"]
                save()
                deliver(state, rpc, allowed, config.get("auto_wake") is True, save, tasks)
                state["connection"] = {"state": "connected"}
                failures = 0; first_failure = None
                save()
                ...
                state["connection"] = {"state": "retrying", "failures": failures, "retry_at": retry_at}
                if failures >= 5 and time.time() - first_failure >= 300:
                    state["connection"]["notice"] = "Codex event delivery remains unavailable; pending events are retained."
                save()
...
def main():
    ...
    config = json.loads((state_root() / "codex-adapter.json").read_text()).get("repositories", {}).get(str(root))
    ...
    if args.action == "serve":
        ...
        state = json.loads(path.read_text()) if path.exists() else empty_state()
        print(json.dumps({"threads": state["threads"], "connection": state.get("connection"),
            "deliveries": [{k: v for k, v in item.items() if k != "data"} for item in state["deliveries"].values() if item["state"] not in {"accepted", "superseded", "provided_in_context"}]}))
    else:
        if args.thread not in execution_threads(root, config): parser.error("Select a locally registered chat")
        ...
        _save(controls / f"{args.thread}.json", {"paused": args.action == "pause", "updated_at": time.time()})
