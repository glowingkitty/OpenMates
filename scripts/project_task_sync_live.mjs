...
import { buildBlockUserTaskInput, buildCreateUserTaskInput, buildUpdateUserTaskInput, decryptUserTask } from '../frontend/packages/openmates-cli/src/tasksCli.ts';
...
try {
  ...
  const firstCreation = await promisify(execFile)('node', [resolve('frontend/packages/openmates-cli/dist/cli.js'), 'tasks', 'create',
    '--title', `Task sync fixture 0 ${randomUUID()}`, '--external-chat', `codex:${owner}`, '--project', projectId, '--json'], {
    env:{...process.env,CODEX_THREAD_ID:owner}, timeout:30_000,
  });
  const first = JSON.parse(firstCreation.stdout).task;
  ...
  for await (const line of input) {
    ...
    try {
      ...
      ws.send('encrypted_chat_metadata', {chat_id:nativeChatId,
        ...
        versions:{messages_v:0,last_edited_overall_timestamp:now}});
      await stored;
    } finally { ws.close(); }
    ...
    for (const index of [8,9,10]) {
      ...
      let linked = await client.updateUserTask(current.task_id,
        await buildUpdateUserTaskInput(plain,master,{chatId:nativeChatId,status:index===8?'in_progress':'todo'}),{personal:true});
    ...
    const plan = await client.createUserPlan(await buildCreateUserPlanInput(master,
      {title:'Native deletion Plan fixture',status:'active',primaryChatId:nativeChatId,primaryChatKey:chatKey,linkedProjectIds:[projectId]}));
    ...
    assert.equal((await decryptUserPlan(detachedPlan,master)).title,'Native deletion Plan fixture');
    stage = 'confirmed_codex_deletion';
    ...
    await promisify(execFile)('node',[resolve('frontend/packages/openmates-cli/dist/cli.js'),'tasks','connect',tasks[11].task_id,'--thread',disposable.thread_id,'--json'],
      {env:{...process.env,CODEX_THREAD_ID:disposable.thread_id},timeout:30_000});
  ...
  emit({event: 'failure', stage, error: error?.name ?? 'Error', status: error?.status ?? null, exit_code: error?.code ?? null,
