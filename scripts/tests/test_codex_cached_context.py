...
def test_complete_start_then_silent_unchanged_tools_and_one_task_delta(tmp_path, monkeypatch):
    monkeypatch.setenv('OPENMATES_STATE_DIR', str(tmp_path/'cli'))
    snapshot = tmp_path/'snapshot.json'
    tasks = [{'task_id': str(i), 'short_id': f'TASK-{i}', 'title': f'Work {i}', 'status': 'todo',
              'external_chat': {'provider': 'codex', 'id': THREAD}, 'description': 'detail '*500,
              'latest_activity': 'activity '*500, 'blocked_reason': 'Approval pending'} for i in range(12)]
    data = {'schema_version': 1, 'project_id': 'project', 'connection': 'connected', 'tasks': tasks}
...
def test_orchestrator_sees_its_workers_but_no_unrelated_chat(tmp_path, monkeypatch):
    ...
    registry.write_text(json.dumps({'coordinator': THREAD, 'workers': {WORKER: {'title': 'Landing - Header', 'status': 'idle'}}}))
    snapshot = tmp_path/'snapshot.json'
    snapshot.write_text(json.dumps({'schema_version': 1, 'project_id': 'project', 'connection': 'connected', 'tasks': [
        {'task_id': '1', 'title': 'Header work', 'status': 'todo', 'external_chat': {'provider': 'codex', 'id': WORKER}},
        {'task_id': '2', 'title': 'Unrelated work', 'status': 'todo', 'external_chat': {'provider': 'codex', 'id': 'elsewhere'}},
    ]}))
    output = cached.context(tmp_path, 'session', THREAD, 'SessionStart', {'snapshots': [str(snapshot)]})
    assert '"Landing - Header"' in output and WORKER in output and 'Header work' in output
...
def test_new_user_turn_after_tools_gets_full_context_but_initial_overlap_does_not(tmp_path, monkeypatch):
    monkeypatch.setenv('OPENMATES_STATE_DIR', str(tmp_path/'cli'))
    path = tmp_path/'snapshot.json'
    path.write_text(json.dumps({'schema_version': 1, 'project_id': 'project', 'connection': 'stopped', 'tasks': []}))
    ...
    assert cached.context(tmp_path, 'session', THREAD, 'PostToolUse', config) == ''
    assert 'Split complex workflows' in cached.context(tmp_path, 'session', THREAD, 'UserPromptSubmit', config)
