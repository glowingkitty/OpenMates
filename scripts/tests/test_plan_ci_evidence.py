...
def test_isolated_playwright_contract():
    plan_validate._validate_tests(
        {
            "tests": [
                {
                    "id": "T-SYNC",
                    "type": "playwright",
                    "file": "project-task-sync.spec.ts",
                    ...
                    "assertions": ["synchronizes a committed update"],
                    "covers": ["AC-1"],
                    "red_phase": {
                        "required": True,
                        "expected": "fail",
...
def evidence(**changes):
    return dict(
        ...
        command="ci_coordinator.py submit",
        run_id="123",
        subject_commit="a" * 40,
        execution_environment="isolated_github_ci",
        harness_commit="b" * 40,
