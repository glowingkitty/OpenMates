...
class RPC:
    ...
    def call(self, method, args):
        ...
        if method == self.fail: raise TimeoutError('reply lost')
        return {'thread':{'id':'worker'},'turn':{'id':'turn'}}

def intent():
    return dict(action='create',root='/repo',title='Worker',prompt='Do approved work',full_access=True)

# contract-test: tooling
def test_create_registers_before_start_and_sets_explicit_permissions():
    rpc=RPC(); record={}; checkpoints=[]
    dispatch(rpc,record,intent(),lambda r:checkpoints.append(dict(r)),lambda t:checkpoints.append({'registered':t}))
...
def test_lost_reply_preserves_known_identity_and_does_not_retry():
    rpc=RPC('turn/start'); record={}
    with pytest.raises(TimeoutError):dispatch(rpc,record,intent(),lambda r:None,lambda t:None)
...
def test_operation_reuse_with_changed_prompt_rejected():
    rpc=RPC();record={};dispatch(rpc,record,intent(),lambda r:None,lambda t:None)
    with pytest.raises(ValueError):dispatch(rpc,record,{**intent(),'prompt':'different'},lambda r:None,lambda t:None)

# contract-test: tooling
def test_default_does_not_escalate_permissions():
    rpc=RPC();dispatch(rpc,{},dict(intent(),full_access=False),lambda r:None,lambda t:None)
