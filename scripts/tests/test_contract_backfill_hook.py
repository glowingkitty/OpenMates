#!/usr/bin/env python3
"""Contract post-edit hook behavior and approval instruction tests.

Temporary git repositories prove warning behavior without mutating OpenMates
contract state. Architecture: docs/specs/contract-driven-development/spec.yml
"""
...
HOOK = ROOT / ".claude" / "hooks" / "contract-test-impact.sh"
...
def test_hook_warns_for_unmapped_changed_test(tmp_path):
    ...
    assert result.returncode == 0
    assert "contracts.py check-test" in result.stderr
    assert "search existing contracts" in result.stderr
    assert "define-contract" in result.stderr
...
def test_hook_requires_full_new_contract_confirmation(tmp_path):
    result = run_hook(tmp_path, "contracts/features/example/contract.yml", "id: feature.example\n")

    assert result.returncode == 0
    assert "quote the complete contract.yml" in result.stderr
...
def test_hook_treats_examples_as_approval_bound(tmp_path):
    result = run_hook(tmp_path, "contracts/features/example/examples.yml", "valid_inputs: []\n")

    assert result.returncode == 0
    assert "contract.yml or examples.yml" in result.stderr
