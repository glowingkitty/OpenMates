...
def test_green_e2e_requires_run_bound_cleanup(tmp_path, fault):
    ...
    if fault == "run": data["run_id"] = "8"
    if fault == "harness": data["harness_commit"] = "other"
    if fault == "containers": data["containers_remaining"] = 1
    if fault == "volumes": data["volumes_remaining"] = 1
    if fault == "accounts": data["private_account_files_removed"] = False
    if fault != "missing": (tmp_path / "ci-cleanup.json").write_text(json.dumps(data))
