#!/usr/bin/env python3
"""Tests for readable Specification presentation PDFs.

These tests keep the human-facing Specification renderer deterministic without
launching Chromium or contacting Docker, Vault, or S3. The approval path remains
covered by test_specification_approval_pdf.py; this suite verifies only the separate
chaptered presentation used to show a Specification as a Project document.
"""
...
from scripts import specification_readable_pdf as readable_pdf
...
def test_main_renders_and_publishes_readable_pdf(tmp_path: Path, monkeypatch, capsys) -> None:
    ...
    monkeypatch.setattr(readable_pdf.specifications, "validate_bundle", lambda _path: current)
    monkeypatch.setattr(readable_pdf.specifications, "_resolve_specification_path", lambda path: Path(path))
...
def test_documented_cli_entry_point_loads_from_repository_root() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/specification_readable_pdf.py", "--help"],
        cwd=readable_pdf.specifications.REPO_ROOT,
        capture_output=True,
        text=True,
        check=False,
    )
    ...
    assert "readable Specification presentation PDF" in result.stdout
