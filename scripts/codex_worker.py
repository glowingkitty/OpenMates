...
def dispatch(rpc, record, intent, save, register, retry_rejected=False):
    ...
    try:
        ...
        result = rpc.call(
            ...
            {
                "threadId": thread,
                "input": [{"type": "text", "text": intent["prompt"]}],
                **permissions,
            },
        )
...
def main():
    ...
    p.add_argument(
        "--operation",
        required=True,
        help="Stable name for this one dispatch; reuse to inspect, never duplicate",
    )
    ...
    p.add_argument(
        "--task",
        help="Existing OpenMates Task ID for worker registry; does not claim it",
    )
    p.add_argument(
        "--full-access",
        action="store_true",
    ...
    with (directory / ("create.lock" if a.action == "create" else key + ".lock")).open(
        ...
        if a.action == "receipt":
            print(json.dumps(record or {"state": "not_found"}))
            return
        ...
        def register(thread):
            with transaction(state_path(root, a.session)) as state:
                ...
                state["workers"][thread] = new_worker(
                    thread, a.task, a.title, time.time()
                )
            ...
            subprocess.run(
                [
                    sys.executable,
                    str(root / "scripts/codex_cached_context.py"),
                    "configure",
                    "--repository",
                    str(root),
                    "--thread",
        ...
        if registry.exists() and json.loads(registry.read_text()).get(
            "coordinator"
        ) not in (None, owner):
            raise ValueError("Session registry belongs to another coordinator")
        ...
        print(
            json.dumps(
                {
                    k: result[k]
                    for k in ("state", "thread_id", "turn_id", "error")
                    if k in result
                }
            )
        )
