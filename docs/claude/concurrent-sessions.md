...
| Action | Command |
| --- | --- |
| Start session | `python3 scripts/sessions.py start --task "description" [--tags frontend,debug]` |
...
| Session summary | `python3 scripts/sessions.py summary --session <ID>` |
| Load doc on demand | `python3 scripts/sessions.py context --doc <name>` |
| Load deploy docs | `python3 scripts/sessions.py deploy-docs` |
...
# Tags auto-inferred from task: debug, security (keyword matching)
# Override: --tags "frontend,debug"
```

This command:

1. Generates a random 4-char hex session ID
2. Auto-infers tags from the task description (or uses explicit `--tags`)
3. Registers the session in `.claude/sessions.json` (with tags)
4. Prunes stale sessions older than 24 hours
5. Clears stale locks older than 5 minutes
6. Outputs context to stdout:
   ...
   - Project index (filtered by tags — backend only if backend tag, etc.)
   - Architecture doc index (compact list with descriptions)
7. **Prints full contents of instruction docs** matching the inferred/explicit tags
   - Git/deployment docs are deferred — load with `deploy-docs` before committing
