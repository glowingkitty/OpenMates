...
- [x] Flow and edge-case cards use Design app identity; code model references use the Code app gradient and icon. Flows retain revision-bound required Check links, displayed as shared Check previews in their fullscreen reader. Check color/icon identity remains a design decision; the current example uses provisional Plans styling with a checklist icon.
