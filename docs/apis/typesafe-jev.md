...
| Optional search relevance | one score per bounded, deduplicated candidate using separate web, news, events, or home rubrics | unchanged pre-ranking provider order, followed by the requested public result limit |
...
Search relevance ranking runs only when a nonblank `relevance_criteria` value is present. Web uses at most two twenty-result Brave pages, news one up-to-forty-result request, and events/home use bounded merged provider pools. Hard filters and stable deduplication run before the decision. Candidate text is labeled untrusted, the decision can only reorder existing candidates, and the skill slices back to the requested `count` or `max_results` before formatting, enrichment, output safety, or embed creation. Health appointment search is intentionally excluded.
