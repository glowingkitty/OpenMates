...
- **Verify on dev preview:** After creating or changing a component, verify it renders at `https://app.dev.openmates.org/dev/preview/{component-path}` with the preview data
...
Fall back to manual reproduction on `https://app.dev.openmates.org` using a Firecrawl session. See `docs/contributing/guides/debugging.md` → "Browser-Based Debugging with Firecrawl" for the full workflow.
