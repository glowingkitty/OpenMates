...
# Git and Deployment Workflow (Full Guide)
...
- Product UI and marketing copy show `vMAJOR.MINOR`, for example `v0.18`.
- npm, PyPI, GHCR images, and GitHub release tags use exact SemVer/PEP 440 versions, for example `0.18.0-alpha.1`, `0.18.0a1`, or `v0.18.0-alpha.1` while in alpha.
...
**Current phase:** Alpha — the app currently shows **v0.18** as the user-facing product line. Dev artifacts publish on a fixed alpha train such as `0.18.0-alpha.N` for npm/GHCR and `0.18.0aN` for PyPI. The next minor product line is `v0.18`.

**How to bump the version:**

- **Alpha artifact** (`v0.18.0-alpha.0` → `v0.18.0-alpha.1`): additional dev publishes inside the same product line
- **Minor product line** (`v0.18` → `v0.18`): new features added or significant changes
...
- Any `feat:` commits → consider a new minor product line (e.g. `v0.18` → `v0.18`)
- Major milestone reached → consult the user before bumping major or changing phase

**Note:** Run `python3 scripts/bump_alpha_version_line.py --minor <X>` when bumping the minor product line. Keep `userFacing` aligned with the short product line, e.g. `v0.18`, while `cli.stableBase` / `python.stableBase` define the fixed artifact base, e.g. `0.18.0`. Dev publishes prereleases on that fixed train (`0.18.0-alpha.0`, then `0.18.0-alpha.1` for npm/GHCR; `0.18.0a0`, then `0.18.0a1` for PyPI). Main publishes the configured stable base (`0.18.0`) and skips if that exact version is already published. After the stable base exists on PyPI, bump the product minor line before publishing more Python prereleases because PEP 440 alpha versions sort below the stable base. Keep `frontend/packages/ui/src/i18n/sources/signup/main.yml` → `version_title` aligned with `userFacing`, then regenerate locale JSON files (see `docs/contributing/guides/i18n.md`).
