...
| `user-guide/` | Explaining a feature or workflow to end-users who use the product, including CLI usage under `user-guide/cli/` | The audience is developers (→ `contributing/` or `architecture/`) |
...
├── Yes → user-guide/ (or self-hosting/ for self-hosters)
...
| `apps/` | OpenMates apps model: apps, skills, focus modes, memory types, function calling, and app action confirmation |
| `platforms/` | Platform/client architecture: web app, Apple app, CLI, and platform parity |
