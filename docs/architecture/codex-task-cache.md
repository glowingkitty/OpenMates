...
Activity and create/edit requests accept a stable SHA-256 `--delivery-id`. They persist encrypted input before sending. First attempts perform the requested mutation; uncertain responses use the original ID and a scoped read before any retry. A newer conflicting version requires review rather than overwriting it. Pending JSON is explicit:
...
Use the existing `sessions.py ci-source` and `ci_coordinator.py` pipeline. `project-task-sync.spec.ts` checks real web edits against the foreground CLI cache, twelve Tasks, restart replay and deletion on the isolated GitHub stack. Do not run this probe on shared dev or against a copied user account.
