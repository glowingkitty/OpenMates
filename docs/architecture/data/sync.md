...
3. **Cache Embeds**: Store embeds in Redis cache for client sync:
   ...
   - **Note**: During cache warming, embeds remain client-encrypted (zero-knowledge maintained). They are only vault-encrypted and stored in AI cache (`embed:{embed_id}`) later when client provides them as cleartext during message processing (see [Message Processing Architecture](../messaging/message-processing.md#embed-processing-during-inference))
...
- **AI Cache Population**: Embeds are vault-encrypted and stored in AI cache (`embed:{embed_id}`) later when client provides them as cleartext during message processing (see [Message Processing Architecture](../messaging/message-processing.md#embed-processing-during-inference))
