...
`supervisor-probe` performs only a fixed denied read of /dev/null or (`probe_kind: fork`) a denied fork, never a deletion test. Actual Mac evidence showed SIGSTOP was not delivered for denied reads; SIGKILL was delivered. The render runner therefore uses inherited `(deny file-write-unlink (with send-signal SIGKILL))`. Native child launch additionally denies process-fork, so browser/encoder/compositor cannot create an unobserved native grandchild. Fork denial is an ordinary error, not deletion permission. Browser uses single-process/in-process-GPU mode; incompatibility fails closed. Only the three fixed native executables may spawn.
