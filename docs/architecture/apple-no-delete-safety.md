...
## Guarded render capability check

`supervisor-probe` performs only a fixed denied read of /dev/null or (`probe_kind: fork`) a denied fork, never a deletion test. Actual Mac evidence showed SIGSTOP was not delivered for denied reads; SIGKILL was delivered. The render runner therefore uses inherited `(deny file-write-unlink (with send-signal SIGKILL))`. Browser and encoder launch additionally denies process-fork. The compiler transform service and in-process compositor inherit the Node no-unlink policy. Fork denial is an ordinary error, not deletion permission. Browser uses single-process/in-process-GPU mode; incompatibility fails closed. Only the exact installed compiler and compositor may spawn from Node. Browser and encoder are launched and supervised directly by Python.

`render-check` is a fixed one-frame tooling composition plus one-frame encoding, not the marketing video. It requires successful harmless read/fork proofs bound to the current runner hash. Outputs remain under renders/no-delete-retained/<exclusive UUID>/, including source, bundle, tmp/browser profile, PNG, MP4 and logs. Webpack caching/clean and public-media copying are disabled. Remotion4.0.457 deletion-directory calls become retention only inside this run; download maps explicitly prevent cleanup. Unexpected JS unlink/rm/rmdir/rename stops before dispatch. Native unlink denial kills its directly observed process; supervisor writes a persistent task stop and kills the process group without cleanup. The wrapper mirrors exit77 to its durable local latch. No deletion denial is retried. Remote task stops also reject later typed calls.

This capability check is not render_ready until actual execution passes. Marketing source/props/frame-range/encode contracts remain separate from this fixed probe. Source and hook boundaries above still apply; these are trusted reviewed tooling operations, not protection against arbitrary host tampering. Native-check transport interruption must be treated as unresolved; never assume completion or retry deletion.

Actual bounded check found sandbox_apply EPERM when trying to nest sandbox-exec. The staged runner applies complete profiles once to separate Node/browser/encoder processes; no policy is removed to accommodate nested initialization. Browser CDP connection uses the pinned renderer client without owning its process/profile cleanup. Compiler/renderer failures surface without retry. `render-report` reads only bounded retained logs/outcomes for an exact run UUID; it does not execute commands.
