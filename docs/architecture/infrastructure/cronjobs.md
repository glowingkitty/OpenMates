...
**Release intelligence** (`00:20 daily`, `00:45 Mon`, `01:10 first day`): `scripts/release-intelligence-cron.sh` creates LLM-backed changelog artifacts from git history and prior rollups in an isolated `dev` clone. Daily mode writes `docs/releases/daily/YYYY-MM-DD.yml` for the previous UTC day. Weekly mode refreshes all seven daily artifacts before writing `docs/releases/weekly/YYYY-Www.yml`, so branch reachability classifications cannot remain stale after a release. Each run acquires the shared dev deploy lock for publication, rejects a changed remote base, and performs an atomic fast-forward push of only the generated release artifacts; unrelated root-checkout changes are never staged, and a concurrent remote update makes the push fail closed. Weekly Discord delivery happens only after the exact generated commit is confirmed on `origin/dev`. Monthly mode reads weekly artifacts for the previous month and writes `docs/releases/monthly/YYYY-MM.yml`. Artifacts include deterministic release readiness, feature availability gates, newsletter include/exclude candidates, and LLM summaries; `--no-llm` emits a compact deterministic fallback for safe historical backfills. Env: `GEMINI_API_KEY` or `SECRET__GOOGLE_AI_STUDIO__API_KEY`; optional `DISCORD_WEBHOOK_RELEASE_INTELLIGENCE` with fallback to `DISCORD_WEBHOOK_DEV_NIGHTLY` for weekly summaries. Manual: `scripts/release-intelligence-cron.sh daily|weekly|monthly`.
