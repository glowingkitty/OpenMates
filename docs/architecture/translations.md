...
├── login.yml
├── settings/
│   ├── main.yml          # Top-level settings keys
│   ├── app_store.yml     # App store related translations
│   ├── billing.yml       # Billing related translations
│   └── interface.yml     # Interface settings translations
└── signup/
    ├── main.yml
    └── steps.yml
```
...
welcome_message:
  ...
  de: |
    # Willkommen bei OpenMates! 👋
    
    Ich bin dein KI-Teamkollege - hier, um dir beim Lernen, Problemlösen und Nachdenken zu helfen.
...
new_feature_announcement:
  ...
  es: "¡Echa un vistazo a nuestra nueva función!"
...
- `settings.yml` → `settings/main.yml`, `settings/app_store.yml`, etc.
...
- `settings.app_store.text` → `settings.app_store` namespace, `text` field
- `email.confirm_email.text` → `email` namespace, `confirm_email` key, `text` field
- `signup.steps.profile.text` → `signup` namespace, nested `steps.profile` key, `text` field
...
- `app_store.categories.top_picks` → goes to `settings/app_store.yml` with key `categories.top_picks`
- `app_store` (top-level key) → also goes to `settings/app_store.yml` as `app_store`
...
sources/
├── email/
│   ├── main.yml          # General email translations
│   ├── confirmation.yml  # Email confirmation specific
│   └── notifications.yml # Email notifications
```
