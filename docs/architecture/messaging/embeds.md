...
To enable offline chat sharing while maintaining cross-chat embed usage, each embed's encryption key (`embed_key`) is stored in multiple wrapped forms in the `embed_keys` collection. This follows the same pattern as the master key with multiple login methods (see [Security Architecture](../core/security.md)).
...
Embeds can be shared independently of chats, following the same zero-knowledge architecture as chat sharing. See [Share Chat Architecture](../user-guide/sharing.md) for the underlying principles.
...
- **Sync Architecture**: See [Sync Architecture](../data/sync.md) - embeds must be loaded alongside messages during cache warming
