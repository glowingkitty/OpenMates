...
- **Permanent Storage**: Zero-knowledge (server cannot decrypt) - stored in Directus database
  - Uses client-side `encryption_key_chat` per chat (see [`security.md#chats`](../core/security.md#chats))
...
**Architecture Alignment with [`security.md`](../core/security.md)**:
...
**Model Selection**: See [Preprocessing Model Comparison Report](../ai/preprocessing-model-comparison.md) for detailed analysis of model options and why Mistral Small 3.2 was chosen.

**PII Protection**: Before sending messages to LLMs, sensitive personal information (PII) should be pseudonymized to protect user privacy. See [Sensitive Data Redaction Architecture](../privacy/sensitive-data-redaction.md) for implementation details and options (Presidio, data-anonymizer).
...
- define best fitting LLM for request based on complexity/usecase
  ...
  - **Benchmark Resources**: See [AI Model Selection Architecture](../ai/ai-model-selection.md#benchmark-resources-for-model-comparison) for resources used in model comparison
  - **Output Fields**:
...
**Model Selection**: See [Preprocessing Model Comparison Report](../ai/preprocessing-model-comparison.md) for detailed analysis of model options. The same model (Mistral Small 3.2) is recommended for both preprocessing and postprocessing.
...
- **[Follow-up Suggestions & New Chat Suggestions](../ai/followup-suggestions.md)**: 6 contextual suggestions for each type
...
For detailed specifications on follow-up request suggestions and new chat suggestions (generation, storage, UI behavior, and implementation), see **[`followup_request_suggestions.md`](../ai/followup-suggestions.md)**.
