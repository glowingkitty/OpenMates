...
Examples: `POST /api/v1/apps/web/search`, `POST /api/v1/apps/videos/get_transcript`, `POST /api/v1/apps/images/generate`
...
**Long-running skills** (e.g., image generation): returns `task_id` + `embed_id`. Poll via `GET /api/v1/tasks/{task_id}`. Download files via `GET /api/v1/embeds/{embed_id}/file?format=preview|full|original`.
