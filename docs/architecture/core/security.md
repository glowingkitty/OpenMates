...
- OpenCode response media uses `scripts/response_media.py` to upload intentionally shareable plaintext images/videos to a private 48-hour bucket and returns a 48-hour presigned URL for Markdown rendering. This is an agent/dev utility, not a public product API.
