...
```mermaid
flowchart TD
    H[Yesterday's Codex tasks] --> M[Daily meeting]
    G[Yesterday's git commits] --> M
    N[Nightly CI coverage and notification receipts] --> M
    T[OpenMates Tasks and decisions] --> M
    M --> A[User approves today's priorities]
...
```bash
python3 scripts/codex_meeting.py --timezone Europe/Berlin --output /tmp/meeting.json
...
deduplicates copied fork messages. Inspect relevant full history after the compact
last-message excerpt: it is an index for review, not an exhaustive task summary.
Inaccessible or oversized history is disclosed. A missing day is not an approval
to resume work. Calendar-yesterday and selected-last-working-day are both recorded.
Git commits use the selected working day. Nightly CI uses yesterday through the
current meeting time, independently of a weekend/holiday history lookback.

The legacy daily meeting helper now uses these inputs by default; old diagnostic
collectors remain available for targeted investigations. `MEETING_TIMEZONE` selects
the timezone there (UTC when unset). No provider/production probe is a mandatory
