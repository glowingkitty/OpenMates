...
For encryption architecture, see [security.md#app-settings--memories](../architecture/security.md#app-settings--memories).

## Client-Side Interaction

![App settings and memories](../../images/apps/request_app_settings_memories.png)

As described in [message_processing.md](../architecture/message-processing.md), the client submits an overview of the type of app settings and memories that are available. The assistant then requests the data from the user via WebSocket connection.
...
For detailed information on action confirmation architecture, see [Action Confirmation Architecture](./action-confirmation.md).
...
5. **Main-Processing Skill Execution**: LLM extracts the entry*id and updated fields, then calls the `{app_id}.settings_memories_update*{category_name}` skill with entry_id and fields to update.
...
5. **Main-Processing Skill Execution**: LLM extracts the entry*id and calls the `{app_id}.settings_memories_delete*{category_name}` skill with entry_id.
...
4. **User clicks "Confirm"** on preview → client generates entry_id and metadata, encrypts entry using app-specific encryption key (see [security.md#app-settings--memories](../architecture/security.md#app-settings--memories))
