...
> Find messages, chats, settings, apps, and documentation from one search bar. Search works entirely on your device -- nothing is sent to the server.

## What It Does

Search lets you quickly find content across your entire OpenMates experience. Because your data is encrypted, the search index is built and stored locally on your device. This means search works offline and your queries are never logged or shared.
...
Documentation pages are included in search results. A compact index is bundled with the app, so you can find help pages even when offline. Clicking a documentation result opens the full page.

## Tips

- Search is entirely local. Your search terms are never sent to any server.
- If you have thousands of chats, the first-time indexing may take a moment. A progress indicator shows the status.
