...
Data processing is done via unified `parseMessage()` function described in [message_parsing.md](../../architecture/messaging/message-parsing.md).
...
3. Process each chunk through the prompt injection detection system (see [Prompt Injection Protection](../../architecture/privacy/prompt-injection.md))
...
See also: [App Skills - Prompt Injection Protection for External Data](./skills.md#prompt-injection-protection-for-external-data)
...
- See [LLM Hallucination Mitigation](../../architecture/ai/hallucination-mitigation.md) for details on how this fits into the broader strategy to reduce hallucinations and ensure accuracy.
