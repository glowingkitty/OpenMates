...
Every generated image is stored as an **embed** (see [Embeds Architecture](../../architecture/messaging/embeds.md)). This provides a unified model for both REST API and Web App access, with consistent encryption, status tracking, and cross-chat referencing capabilities.
