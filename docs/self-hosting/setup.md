...
By default the stack pulls OpenMates images from `ghcr.io/glowingkitty` using the CLI version tag, for example `v0.15.0`. Expect the first start to download several GB of compressed images; later updates reuse Docker's image cache.
...
openmates server update --path ~/openmates --image-tag v0.15.0
