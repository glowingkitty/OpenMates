...
class DirectusSchemaManager:
    ...
    def create_collection(self, collection_name: str, fields: list) -> None:
        ...
        try:
            ...
            # Create fields after collection is created, skipping the 'id' field
            for field in fields:
                if field['field'] != 'id':  # Skip the id field
                    logger.debug(f"Processing field: {field['field']}")
                    self.create_field(collection_name, field)
                else:
                    logger.debug("Skipping 'id' field as it's automatically created by Directus")
