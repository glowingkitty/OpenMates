from server.api.models.apps.web.skills_web_view import WebViewOutput
import logging
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                f"http://web_browser:{os.getenv('APP_WEB_PORT')}/view",
                json={"url": url},
                headers={"Authorization": f"Bearer {os.getenv('APP_WEB_SECRET_KEY')}"}
            ) as response:
